...
from artemis.infra.backup.chats import (
...
from artemis.infra.backup.pg_dump import dump_postgres, restore_postgres
from artemis.infra.backup.snapshot import (
...
from artemis.infra.db.migrate import _current_version, _max_discovered_version, run_migrations
from artemis.infra.db.pool import DatabasePool, SqlitePool
...
from artemis.infra.utils.env_file import env_file_path
...
_SQLITE_DB_ARC = f"{_DB_DIR}/artemis.db"
_PG_DUMP_ARC = f"{_DB_DIR}/artemis.dump"
...
def suggested_backup_filename() -> str:
    ...
    return f"artemis-backup-{_timestamp()}.tar.gz"
...
def create_system_backup(
    *,
    ...
    Returns the suggested basename (``artemis-backup-….tar.gz``). Callers that
    ...
    if pool.dialect == "postgresql":
        ...
        if not isinstance(pool, SqlitePool):
            raise ArtemisError(ErrorCode.INTERNAL_ERROR, "sqlite backup requires SqlitePool")
        if not pool.path.is_file():
            raise ArtemisError(ErrorCode.NOT_FOUND, f"database not found: {pool.path}")
...
def _extract_manifest_from_dir(extracted: Path) -> BackupManifest:
    ...
    if not manifest_path.is_file():
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "backup archive missing manifest.json")
    try:
        ...
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, f"invalid manifest: {exc}") from exc
...
def _extract_archive(source: Path | bytes, dest_dir: Path) -> None:
    ...
    if not Path(source).is_file():
        raise ArtemisError(ErrorCode.NOT_FOUND, f"backup not found: {source}")
...
def _is_migration_backup(manifest: BackupManifest) -> bool:
    ...
    return manifest.artemis_version.endswith(_MIGRATION_VERSION_SUFFIX)
...
def restore_system_backup(
    ...
    *source* may be a filesystem path (preferred) or in-memory bytes (tests / legacy).
    ...
    ``preserve_users`` controls whether the *current* Artemis instance's login
    ...
    * ``None`` (default) — auto-detect: preserves credentials when the backup
      was produced by an external migration tool (``artemis_version`` ends with
    ...
    * ``False`` — restore the users/secrets tables as-is from the backup
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if not db_path.is_file():
            raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "backup archive missing database file")
        ...
        if pool.dialect == "postgresql":
            ...
            if isinstance(pool, SqlitePool):
                ...
                raise ArtemisError(ErrorCode.INTERNAL_ERROR, "sqlite restore requires SqlitePool")
    ...
    result: dict[str, Any] = {
        "schema_version": schema_version,
        "artemis_version": manifest.artemis_version,
