...
from artemis.config import BackupConfig, ArtemisConfig, load_config
...
AUTO_BACKUP_JOB_ID = "artemis_auto_backup"
_AUTO_FILENAME_PREFIX = "artemis-auto-backup-"
_MANUAL_FILENAME_PREFIX = "artemis-backup-"
...
def to_auto_backup_filename(suggested: str) -> str:
    """Map ``artemis-backup-*.tar.gz`` → ``artemis-auto-backup-*.tar.gz``."""
    ...
    raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, f"unexpected backup filename: {name!r}")
...
def persist_backup_config(config_path: Path, backup: BackupConfig) -> ArtemisConfig:
...
def backup_config_from_payload(payload: dict[str, Any]) -> BackupConfig:
    ...
    try:
        ...
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, f"invalid backup schedule: {schedule}") from exc
    try:
        retention = int(payload.get("retention_count", defaults.retention_count))
    except (TypeError, ValueError) as exc:
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "retention_count must be an integer") from exc
    if retention < 1:
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "retention_count must be >= 1")
    return BackupConfig(
...
async def run_auto_backup(server: ArtemisServer) -> BackupFileInfo | None:
...
def apply_auto_backup_schedule(cron_manager: CronManager, *, server: ArtemisServer) -> None:
...
def install_auto_backup_job(cron_manager: CronManager, *, server: ArtemisServer) -> None:
    """Boot-time hook: schedule auto backup when enabled in config."""
    apply_auto_backup_schedule(cron_manager, server=server)


def update_server_backup_config(server: ArtemisServer, backup: BackupConfig) -> ArtemisConfig:
