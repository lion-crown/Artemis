...
from artemis.infra.utils.paths import PRIVATE_FILE_MODE, PathLayout, restrict_to_owner
from artemis.infra.utils.posix_compat import chown, geteuid, getuid, is_root
...
# begin.  `artemis run` typically takes several seconds to initialise DB, load
...
_ENV_SCOPE = "ARTEMIS_SERVICE_SCOPE"
...
def detect_service_mode() -> ServiceMode | None:
    """Return the service mode for the current process (from ARTEMIS_SERVICE_MODE)."""
    raw = os.environ.get("ARTEMIS_SERVICE_MODE", "").strip().lower()
...
def _auto_service_scope() -> ServiceScope:
    ...
    when root runs ``artemis service start`` the expected outcome is always a
...
def resolve_service_scope(*, default: ServiceScope | None = None) -> ServiceScope:
    ...
    1. ``ARTEMIS_SERVICE_SCOPE`` env var (``user`` / ``system`` / ``auto``).
...
def launchd_domain(scope: ServiceScope) -> str:
    ...
    `print` — e.g. ``gui/$UID/artemis`` for user scope.  For the `bootstrap`
...
def resolve_artemis_executable(home: Path | None = None) -> Path:
    root = home or PathLayout.from_env().root
    for candidate in (
        root / "bin" / "artemis",
        root / "venv" / "bin" / "artemis",
        root / "venv" / "Scripts" / "artemis.exe",
    ):
        if candidate.is_file():
            return candidate

    # Dev / conda / uv-tool installs: use the Artemis executable on this interpreter's PATH.
    for candidate in (
        Path(sys.executable).parent / "artemis",
        Path(sys.executable).parent / "artemis.exe",
    ):
        if candidate.is_file():
            return candidate

    found = shutil.which("artemis")
    if found:
        return Path(found)

    raise FileNotFoundError(
        f"artemis executable not found under {root} or on PATH; "
        "run `scripts/install.sh` or ensure `artemis` is installed"
    )
...
def resolve_service_home(*, run_as_user: str | None = None) -> Path:
    """Resolve ``ARTEMIS_HOME`` for the service owner, not the installer's euid.

    ``Path.home()`` follows the effective uid, so a ``sudo`` install as root
    would otherwise point at ``/root/.artemis`` while ``User=`` names the
    ...
    return _account_home(user) / ".artemis"
...
def persist_bind_options(home: Path, *, host: str | None = None, port: int | None = None) -> None:
    ...
    if port is not None:
        data["port"] = port
    PathLayout(home).ensure_root()
    config_path.write_text(json.dumps(data, indent=2) + "\n", encoding="utf-8")
    restrict_to_owner(config_path, PRIVATE_FILE_MODE)
...
def build_runtime(
    *,
    ...
    return ServiceRuntime(
        ...
        home=root,
        artemis_bin=resolve_artemis_executable(root),
...
def render_systemd_unit(runtime: ServiceRuntime, *, user_unit: bool | None = None) -> str:
    ...
    # HOME + service mode + config-driven ``run``.  Artemis also
    # sets ``ARTEMIS_HOME`` explicitly.  No ``WorkingDirectory`` — avoids CHDIR
    ...
    env_lines = (
        ...
        f"Environment=ARTEMIS_HOME={runtime.home}\n"
        "Environment=ARTEMIS_SERVICE_MODE=systemd\n"
    )
    if user_unit:
        env_lines += "Environment=ARTEMIS_SYSTEMD_USER=1\n"
    nofile = _user_nofile_cap() if user_unit else SERVICE_NOFILE_LIMIT
    nofile_line = f"LimitNOFILE={nofile}\n" if nofile is not None else ""
    return (
        "[Unit]\n"
        "Description=Artemis AI Server\n"
        ...
        f"{user_line}"
        f"{env_lines}"
        f"ExecStart={runtime.artemis_bin} run\n"
        "Restart=on-failure\n"
        "RestartSec=5\n"
        "TimeoutStopSec=30\n"
        f"{nofile_line}"
...
def render_launchd_plist(runtime: ServiceRuntime) -> str:
    ...
    """
    log_path = runtime.home / "logs" / "artemis.log"
    account_home = _account_home(runtime.run_as_user)
    user_block = (
    ...
    return (
        ...
        "  <array>\n"
        f"    <string>{runtime.artemis_bin}</string>\n"
        "    <string>run</string>\n"
        "  </array>\n"
        ...
        f"    <key>ARTEMIS_HOME</key>\n"
        f"    <string>{runtime.home}</string>\n"
        "    <key>ARTEMIS_SERVICE_MODE</key>\n"
...
def render_nofile_dropin(limit: int = SERVICE_NOFILE_LIMIT) -> str:
    return (
        "# Generated by artemis service. Override with a later drop-in if needed.\n"
        "[Service]\n"
        f"LimitNOFILE={limit}\n"
    )
...
def restart_service(runtime: ServiceRuntime) -> None:
    ...
    if not is_service_installed(runtime.mode, scope=runtime.scope, run_as_user=runtime.run_as_user):
        raise RuntimeError(
            "artemis system service is not installed "
