...
from artemis.infra.setup.tls.acme_issue import issue_certificate_http01
from artemis.infra.setup.tls.listeners import TLS_HTTP_PORT, TLS_HTTPS_PORT
from artemis.infra.setup.tls.modes import (
    TlsIssueMode,
    is_issue_eligible,
    tls_issue_mode,
    validate_issue_domain,
)
from artemis.infra.setup.tls.preflight import PreflightResult, run_preflight
from artemis.infra.setup.tls.store import install_letsencrypt_cert, resolve_tls_paths
...
class TlsManager:
    """In-process TLS issuance state (single Artemis process)."""
    ...
    def status_payload(self, config: ArtemisConfig, paths: PathLayout) -> dict[str, Any]:
        tls = config.tls
