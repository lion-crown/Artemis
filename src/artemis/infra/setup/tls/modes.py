...
def tls_issue_mode(config: ArtemisConfig) -> TlsIssueMode:
...
def is_issue_eligible(config: ArtemisConfig) -> bool:
    return tls_issue_mode(config) != TlsIssueMode.NONE


def is_renewal_mode(config: ArtemisConfig) -> bool:
    return tls_issue_mode(config) == TlsIssueMode.RENEWAL


def validate_issue_domain(domain: str, config: ArtemisConfig) -> str:
