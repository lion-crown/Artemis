"""Artemis-specific storage backend probes (docker + row → harness probe)."""
...
from artemis.infra.backend.adapter import row_to_backend_spec
from artemis.infra.backend.docker_spec import (
    DEFAULT_SANDBOX_PREFIX,
    enrich_docker_backend_spec,
)
from artemis.infra.db.repos.backends import BackendRow

logger = logging.getLogger(__name__)

_PROBE_CONTENT = "artemis-docker-probe"
_PROBE_TEST_ID = "test"
...
def _probe_docker(row: BackendRow) -> dict[str, Any]:
    ...
    workspace = tempfile.mkdtemp(prefix="artemis-docker-probe-")
    test_name = f".artemis-probe-{uuid.uuid4().hex}.txt"
