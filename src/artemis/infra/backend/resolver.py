...
def default_agent_backend_spec(workspace_dir: Path) -> dict[str, Any]:
    ...
    workspace system-files prefix (Artemis: ``.artemis``), so deepagents conversation
    ...
    *,
    ...
    if kind == "composite":
        ...
        return {
            ...
            "routes": {
                _composite_route_prefix(prefix): resolve_agent_backend_spec(sub, repo=repo)
...
def _composite_route_prefix(prefix: object) -> str:
    """Return a CompositeBackend route prefix with its required trailing slash."""
    text = str(prefix)
    return f"{text.rstrip('/')}/" if text.rstrip("/") else "/"
