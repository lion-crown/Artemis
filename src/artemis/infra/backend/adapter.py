"""Map Artemis ``storage_backends`` rows to harness-agent backend specs (no I/O)."""
...
def row_to_backend_spec(row: BackendRow) -> dict[str, Any] | None:
    ...
    if kind == "docker":
        ...
        if not image:
            return None
        from artemis.infra.backend.docker_spec import apply_docker_cfg_keys
...
def storage_spec_previewable(spec: Any) -> bool:
    ...
    if str(spec.get("type") or "").lower() == "opensandbox":
        ...
        return False
    from artemis.infra.backend.docker_spec import docker_spec_previewable
