"""Artemis helpers for harness ``type: "docker"`` backend specs."""
...
DEFAULT_SANDBOX_PREFIX = "artemis_sandbox"
...
def enrich_docker_backend_spec(
    ...
    *,
    ...
    """Inject Artemis defaults into a docker harness spec (does not overwrite set keys).

    - ``sandbox_prefix`` defaults to ``artemis_sandbox``
...
def inject_docker_global_environment(
    ...
    """Point a docker spec at Admin ``~/.artemis/env`` (re-read on each execute)."""
    if str(spec.get("type") or "").lower() != "docker":
        return spec
    out = dict(spec)
