...
from artemis.config import ArtemisConfig, database_env_configured
from artemis.infra.db.pool import DatabasePool, PostgresPool, SqlitePool
from artemis.infra.utils.paths import PathLayout


def resolve_sqlite_db_path(config: ArtemisConfig, paths: PathLayout) -> Path:
...
def should_defer_control_plane_db(config: ArtemisConfig, paths: PathLayout) -> bool:
    ...
    ``ARTEMIS_DATABASE_*`` / PostgreSQL configuration. Password verification and
...
def open_database(config: ArtemisConfig, paths: PathLayout) -> DatabasePool:
    """Return a DB pool for the configured driver.

    When ``config.json`` has no ``database`` section and no ``ARTEMIS_DATABASE_*``
