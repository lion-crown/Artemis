#!/bin/bash
#
# Check Artemis virtual desktop readiness.
...
if [ -d /opt/artemis-desktop ] || [ -d "$CONF_DIR" ]; then
    mark packages ok
else
    mark packages missing
fi

if systemctl is-active --quiet artemis-desktop-xvnc 2>/dev/null; then
    vnc_running=true
    mark xvnc ok
...
if systemctl is-active --quiet artemis-desktop-openbox 2>/dev/null; then
    mark openbox ok
elif pgrep -f "openbox --config-file /opt/artemis-desktop/openbox.xml" >/dev/null 2>&1; then
    mark openbox ok
else
