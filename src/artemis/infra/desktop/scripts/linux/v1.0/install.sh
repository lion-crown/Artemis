#!/bin/bash
#
# Artemis virtual desktop installer for headless Linux servers
# using the TigerVNC + openbox stack.
#
# Usage:
...
SCRIPT_VERSION="v1.0"
# Python interpreter the desktop deps are installed into (the Artemis venv), used to
# pick matching pythonX.Y-dev headers. Falls back to system `python3`.
TARGET_PYTHON="python3"
INSTALL_ROOT="/opt/artemis-desktop"
CONF_DIR="/etc/artemis-desktop"
DISPLAY_NUM=":99"
...
DESKTOP_STATE_DIR="${ARTEMIS_HOME}/desktop"
DESKTOP_ENV="${DESKTOP_STATE_DIR}/desktop.env"

SVC_XVNC="artemis-desktop-xvnc"
SVC_OPENBOX="artemis-desktop-openbox"
SVC_SESSION="artemis-desktop-session"
...
WALLPAPER_FILE="/usr/share/backgrounds/artemis-desktop-wallpaper.svg"
WALLPAPER_PNG="/usr/share/backgrounds/artemis-desktop-wallpaper.png"
...
install_start_menu_logo() {
    # Copy the Artemis PWA logo shipped next to this install.sh into the system
    # install root. Uninstall removes /opt/artemis-desktop; reinstall restores it.
    ...
    cp -f "${INSTALL_ROOT}/icons/start-menu.png" \
        /usr/share/icons/hicolor/48x48/apps/artemis-start-menu.png
    if command -v gtk-update-icon-cache >/dev/null 2>&1; then
        gtk-update-icon-cache -f /usr/share/icons/hicolor >/dev/null 2>&1 || true
    fi
...
#!/bin/bash
# Rewrite the Artemis default panel (appfinder launcher + tasklist + clock).
#
...
mkdir -p /root/.config/xfce4/panel/launcher-1 \
    /root/.config/xfce4/xfconf/xfce-perchannel-xml \
    /opt/artemis-desktop/panel

write_panel_files() {
    local start_icon="/opt/artemis-desktop/icons/start-menu.png"
    if [ ! -s "$start_icon" ]; then
        if [ -e /usr/share/icons/hicolor/48x48/apps/artemis-start-menu.png ]; then
            start_icon="/usr/share/icons/hicolor/48x48/apps/artemis-start-menu.png"
        else
...
LAUNCHER_EOF
    ...
    cat > /opt/artemis-desktop/panel/xfce4-panel.xml << 'PANEL_EOF'
<?xml version="1.0" encoding="UTF-8"?>
...
PANEL_EOF
    cp -f /opt/artemis-desktop/panel/xfce4-panel.xml \
    ...
    chmod 0644 /root/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml \
        /opt/artemis-desktop/panel/xfce4-panel.xml
}
...
if [ -f /tmp/artemis-desktop-dbus-env ]; then
    # shellcheck disable=SC1091
    source /tmp/artemis-desktop-dbus-env || true
fi
...
download_wallpaper() {
    # Install the Artemis wallpaper shipped next to this script (packaged in the
...
configure_desktop_environment() {
    ...
    if [ -n "$browser_bin" ]; then
        ...
        cat > "${INSTALL_ROOT}/artemis-browser.desktop" << BROWSER_EOF
[Desktop Entry]
Type=Application
Name=${browser_name}
...
StartupNotify=true
BROWSER_EOF
        chmod 0644 "${INSTALL_ROOT}/artemis-browser.desktop"
        ...
        cp -f "${INSTALL_ROOT}/artemis-browser.desktop" "${DESKTOP_DIR}/browser.desktop"
...
EDIT_EOF
    ...
    if [ -f "${INSTALL_ROOT}/artemis-browser.desktop" ]; then
        cat > /root/.config/mimeapps.list << 'MIME_EOF'
[Default Applications]
x-scheme-handler/http=artemis-browser.desktop
x-scheme-handler/https=artemis-browser.desktop
text/html=artemis-browser.desktop
MIME_EOF
    fi
...
TRUST_EOF
    ...
    cat > "${AUTOSTART_DIR}/artemis-trust-icons.desktop" << TRUST_AUTO_EOF
[Desktop Entry]
Type=Application
...
export XDG_RUNTIME_DIR=/tmp/runtime-artemis-desktop
mkdir -p "\$XDG_RUNTIME_DIR"; chmod 700 "\$XDG_RUNTIME_DIR"
for i in \$(seq 1 100); do xdpyinfo -display ${DISPLAY_NUM} >/dev/null 2>&1 && break; sleep 0.3; done
eval "\$(dbus-launch --sh-syntax)"
cat > /tmp/artemis-desktop-dbus-env << ENVEOF
export DBUS_SESSION_BUS_ADDRESS="\$DBUS_SESSION_BUS_ADDRESS"
export DBUS_SESSION_BUS_PID="\$DBUS_SESSION_BUS_PID"
ENVEOF
chmod 644 /tmp/artemis-desktop-dbus-env
...
[ -f /tmp/artemis-desktop-dbus-env ] && source /tmp/artemis-desktop-dbus-env || true

WALLPAPER="/usr/share/backgrounds/artemis-desktop-wallpaper.png"
[ -f "$WALLPAPER" ] || WALLPAPER="/opt/artemis-desktop/wallpaper.png"
[ -f "$WALLPAPER" ] || WALLPAPER="/usr/share/backgrounds/artemis-desktop-wallpaper.svg"
[ -f "$WALLPAPER" ] || exit 0
...
if [ ! -f /tmp/artemis-desktop-dbus-env ]; then
    echo "missing /tmp/artemis-desktop-dbus-env (desktop session not ready)"
    exit 1
fi
# shellcheck disable=SC1091
source /tmp/artemis-desktop-dbus-env
...
#!/bin/bash
export DISPLAY=:99 HOME=/root XDG_RUNTIME_DIR=/tmp/runtime-artemis-desktop
...
for i in $(seq 1 50); do
    [ -f /tmp/artemis-desktop-dbus-env ] && { source /tmp/artemis-desktop-dbus-env; [ -n "$DBUS_SESSION_BUS_ADDRESS" ] && break; }
    sleep 0.3
done
...
if [ -x /opt/artemis-desktop/ensure-panel.sh ]; then
    /opt/artemis-desktop/ensure-panel.sh || true
else
    pkill -9 -x xfce4-panel >/dev/null 2>&1 || true
    sleep 0.2
...
(
    ...
    if ! pgrep -x xfce4-panel >/dev/null 2>&1 && [ -x /opt/artemis-desktop/ensure-panel.sh ]; then
        /opt/artemis-desktop/ensure-panel.sh || true
    fi
    ...
    [ -x /opt/artemis-desktop/apply-wallpaper.sh ] && /opt/artemis-desktop/apply-wallpaper.sh
    [ -x /opt/artemis-desktop/apply-icon-size.sh ] && /opt/artemis-desktop/apply-icon-size.sh
    ...
    [ -x /opt/artemis-desktop/trust-desktop-icons.sh ] && /opt/artemis-desktop/trust-desktop-icons.sh
) &>/dev/null &
exec openbox --config-file /opt/artemis-desktop/openbox.xml
SCRIPT_EOF
...
write_systemd_units() {
    local xvnc_bin="$1"
    local vnc_password="${VNC_PASSWORD:-artemis-desktop}"
    local vncpasswd_cmd
...
[Unit]
Description=Artemis virtual desktop - Xvnc
...
[Unit]
Description=Artemis virtual desktop - D-Bus session
...
[Unit]
Description=Artemis virtual desktop - Openbox
...
export ARTEMIS_DESKTOP_DISPLAY=${DISPLAY_NUM}
export ARTEMIS_DESKTOP_GEOMETRY=${GEOMETRY}
EOF
...
start_services() {
    if command -v systemctl >/dev/null 2>&1 && [ -d /run/systemd/system ]; then
        ...
        sleep 2
        systemctl is-active --quiet "$SVC_XVNC" || fail "artemis-desktop-xvnc not active"
        sleep 4
