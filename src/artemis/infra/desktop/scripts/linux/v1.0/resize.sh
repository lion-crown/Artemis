...
DISPLAY_NUM=":99"
ARTEMIS_HOME="${ARTEMIS_HOME:-/root/.artemis}"
...
if command -v systemctl >/dev/null 2>&1 && [ -d /run/systemd/system ]; then
  if [ -f "/etc/systemd/system/artemis-desktop-xvnc.service" ]; then
    sed -i "s/-geometry [0-9]\\+x[0-9]\\+/-geometry ${GEOMETRY}/" \
      /etc/systemd/system/artemis-desktop-xvnc.service
    systemctl daemon-reload
    systemctl restart artemis-desktop-xvnc artemis-desktop-session artemis-desktop-openbox
    sleep 3
