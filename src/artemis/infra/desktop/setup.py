...
from artemis.infra.utils.posix_compat import geteuid
from artemis.infra.utils.runtime_packages import (
...
def artemis_home() -> Path:
    return Path(os.environ.get("ARTEMIS_HOME", Path.home() / ".artemis"))


def desktop_state_dir() -> Path:
    return artemis_home() / "desktop"
...
def system_conf_dir() -> Path:
    return Path("/etc/artemis-desktop")


def system_install_root() -> Path:
    return Path("/opt/artemis-desktop")
...
def resolve_install_script_path() -> Path | None:
    return _script_path_from_env("ARTEMIS_DESKTOP_INSTALL_SCRIPT") or _bundled_script("install.sh")


def resolve_start_script_path() -> Path | None:
    return _script_path_from_env("ARTEMIS_DESKTOP_START_SCRIPT") or _bundled_script("start.sh")


def resolve_stop_script_path() -> Path | None:
    return _script_path_from_env("ARTEMIS_DESKTOP_STOP_SCRIPT") or _bundled_script("stop.sh")


def resolve_resize_script_path() -> Path | None:
    return _script_path_from_env("ARTEMIS_DESKTOP_RESIZE_SCRIPT") or _bundled_script("resize.sh")
...
def read_geometry() -> str:
    ...
    if path.is_file():
        for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
            line = line.strip()
            if line.startswith("export ARTEMIS_DESKTOP_GEOMETRY="):
                value = line.split("=", 1)[1].strip().strip('"').strip("'")
                if _GEOMETRY_RE.match(value):
                    return value
            if line.startswith("ARTEMIS_DESKTOP_GEOMETRY="):
                value = line.split("=", 1)[1].strip().strip('"').strip("'")
                if _GEOMETRY_RE.match(value):
                    return value
    return os.environ.get("ARTEMIS_DESKTOP_GEOMETRY", _DEFAULT_GEOMETRY)
    ...
    path.write_text(
        "\n".join(
            [
                ...
                f"export ARTEMIS_DESKTOP_DISPLAY={display}",
                f"export ARTEMIS_DESKTOP_GEOMETRY={geometry}",
                "",
            ]
        ),
        encoding="utf-8",
...
def _systemd_unit_files() -> tuple[Path, ...]:
    return (
        Path("/etc/systemd/system/artemis-desktop-xvnc.service"),
        Path("/etc/systemd/system/artemis-desktop-session.service"),
        Path("/etc/systemd/system/artemis-desktop-openbox.service"),
    )
...
def _xvnc_service_active() -> bool:
    if shutil.which("systemctl") and Path("/run/systemd/system").is_dir():
        try:
            proc = subprocess.run(
                ["systemctl", "is-active", "--quiet", "artemis-desktop-xvnc"],
                capture_output=True,
                timeout=3,
                check=False,
            )
...
if command -v systemctl >/dev/null 2>&1 && [ -d /run/systemd/system ]; then
  systemctl disable artemis-desktop-xvnc artemis-desktop-openbox artemis-desktop-session 2>/dev/null || true
  systemctl stop artemis-desktop-xvnc artemis-desktop-openbox artemis-desktop-session 2>/dev/null || true
  systemctl reset-failed artemis-desktop-xvnc artemis-desktop-openbox artemis-desktop-session 2>/dev/null || true
  systemctl daemon-reload 2>/dev/null || true
fi
rm -f /etc/systemd/system/artemis-desktop-xvnc.service \\
      /etc/systemd/system/artemis-desktop-openbox.service \\
      /etc/systemd/system/artemis-desktop-session.service
if command -v systemctl >/dev/null 2>&1 && [ -d /run/systemd/system ]; then
  systemctl daemon-reload 2>/dev/null || true
fi
rm -rf /opt/artemis-desktop /etc/artemis-desktop
rm -f /usr/share/backgrounds/artemis-desktop-wallpaper.png \\
      /usr/share/backgrounds/artemis-desktop-wallpaper.svg 2>/dev/null || true
rm -f /usr/share/icons/hicolor/48x48/apps/artemis-start-menu.png 2>/dev/null || true
# Remove artemis-managed Desktop launchers so they are not left as orphans after
# uninstall (they point at the now-removed /opt/artemis-desktop tree). Targeted by
...
pkill -9 -f '/opt/artemis-desktop/' 2>/dev/null || true
pkill -x xfce4-panel 2>/dev/null || true
pkill -f 'xfdesktop --display' 2>/dev/null || true
rm -f /tmp/.X99-lock
rm -rf /tmp/.X11-unix/X99 /tmp/artemis-desktop-dbus-env /tmp/runtime-artemis-desktop
"""
...
async def uninstall_desktop_stream(*, locale: str = "en") -> AsyncIterator[str]:
    from artemis.infra.desktop.session import disconnect_all_streams
