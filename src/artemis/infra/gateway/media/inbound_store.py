...
from artemis.infra.gateway.media.constants import INBOUND_DIR
...
def validate_inbound_media_type(media_type: str, filename: str = "") -> str:
    ...
    raise ArtemisError(ErrorCode.ATTACHMENT_UNSUPPORTED_TYPE, reason, details={"reason": reason})
...
async def _unique_inbound_path(workspace: BackendWorkspace, stored_name: str) -> str:
    ...
    for index in range(2, 1000):
        ...
        if not await workspace.aexists(alt):
            return alt
    raise ArtemisError(ErrorCode.INTERNAL_ERROR, f"cannot allocate unique path for {stored_name!r}")
...
async def read_inbound_bytes(workspace: BackendWorkspace, path: str) -> bytes:
    ...
    if data is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, f"inbound file {rel!r} not found")
    return data
