...
async def wecom_qr_generate() -> dict[str, str]:
    url = f"https://work.weixin.qq.com/ai/qc/generate?source=artemis&plat={wecom_plat_code()}"
...
async def qq_qr_generate(session_id: str) -> dict[str, str]:
    ...
    login = QQBotQRLogin(source="artemis")
...
async def qq_qr_poll(session_id: str, qrcode_token: str) -> dict[str, Any]:
    """Poll the active QQ Bot QR task for an Artemis API or CLI session."""
