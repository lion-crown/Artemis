...
from artemis.i18n.domains.stream import format_stream_error
from artemis.infra.agents.business_workspace import BusinessWorkspaceReader, business_public_scopes
from artemis.infra.agents.execution_profile import BUSINESS_SYSTEM_QA
from artemis.infra.agents.profile import parse_config_json
from artemis.infra.agents.providers.reasoning import reasoning_request_parameters
from artemis.infra.agents.runtime_limits import agent_model_settings
...
from artemis.infra.gateway.media.attachment_hints import content_blocks_need_vision
from artemis.infra.gateway.media.tool_media import (
    attachment_frames_from_tool_result,
    enrich_tool_result_for_dashboard,
    enrich_tool_result_with_backend,
)
from artemis.infra.gateway.process.agent_resolve import (
...
from artemis.infra.gateway.process.harness_request import (
    build_content_from_message,
    build_harness_request,
)
from artemis.infra.gateway.process.history_projection import TurnHistoryTracker, message_inputs
from artemis.infra.gateway.process.message_keys import (
    resolve_user_id_for_message,
    sanitize_im_metadata,
    session_key_from_message,
)
from artemis.infra.gateway.process.stream_project import (
    StreamProjectionState,
    enrich_tool_stream_chunk,
    project_stream,
)
from artemis.infra.gateway.process.usage_record import UsageTracker, record_turn_usage
...
from artemis.infra.gateway.slash.runner import try_handle_slash
from artemis.infra.knowledge.default_open import stamp_turn_knowledge_config
from artemis.infra.trajectory.settings import agent_trajectory_enabled
from artemis.infra.users.preferences import (
...
if TYPE_CHECKING:
    ...
    from artemis.infra.db.repos.agents import AgentRepo
    from artemis.infra.db.repos.audit import AuditRepo
    from artemis.infra.db.repos.connectors import ConnectorRepo
    from artemis.infra.db.repos.sessions import SessionRow
    from artemis.infra.db.repos.users import UserRepo
    from artemis.infra.gateway.slash.dispatcher import SlashDispatcher
    from artemis.infra.gateway.threads import ThreadRegistry
...
class GlobalProcessor:
    ...
    def __init__(
        ...
        *,
        ...
        business_runner: BusinessInvestigationRunner | None = None,
    ) -> None:
        ...
        self._business_runner = business_runner or BusinessInvestigationRunner()

    async def _begin_history(
        self, agent_id: str, thread_id: str, request: dict[str, Any], *, resume: bool = False
    ) -> TurnHistoryTracker:
        from artemis.infra.history.recorder import RecordingTracker  # noqa: PLC0415
    ...
    def _is_business_investigation(self, agent_id: str) -> bool:
        return (
            self._agent_manager.get_config(agent_id).get("execution_profile") == BUSINESS_SYSTEM_QA
        )

    async def _run_business_investigation(
        self,
        *,
        agent_id: str,
        thread_id: str,
        ...
        if workspace is None:
            raise RuntimeError("business workspace is unavailable")
        backend = config.get("backend")
        scopes = business_public_scopes(
            backend if isinstance(backend, dict) else {},
            system_files_path_from_config(config),
        )
        cancelled = self._agent_manager.begin_business_investigation(agent_id, thread_id)
        try:
            return await self._business_runner.answer(
                model=self._agent_manager.get_business_openai_model(agent_id, model_ref),
                reader=BusinessWorkspaceReader(workspace=workspace, scopes=scopes),
                question=question,
                cancelled=cancelled,
                model_settings=agent_model_settings(config),
            )
        finally:
            self._agent_manager.end_business_investigation(agent_id, thread_id)

    async def _record_business_history(self, thread_id: str, question: str, answer: str) -> None:
        tracker = TurnHistoryTracker()
        tracker.observe(
            {
                "type": "state_update",
                "data": {"messages": [HumanMessage(content=question), AIMessage(content=answer)]},
            }
        )
        await self._record_turn_history(thread_id, tracker)

    def _observe_business_trajectory(
        self,
        *,
        agent_id: str,
        thread_id: str,
        result: BusinessInvestigationResult,
        enabled: bool | None = None,
    ) -> None:
        for chunk in business_investigation_trajectory_chunks(result.messages):
    ...
    async def _observe_turn_start_context(
        ...
        *,
        ...
        if service is None:
            return
        try:
            from artemis.infra.trajectory.turn_context import (  # noqa: PLC0415
    ...
    async def _trajectory_workspace_files(self, agent_id: str) -> list[str]:
        ...
        from artemis.infra.trajectory.turn_context import memory_file_order  # noqa: PLC0415
    ...
    async def prepare_peer_session(self, call: PeerCall) -> PeerSession | None:
        ...
        uid = _artemis_user_id(call.user_id)
    ...
    def _slash_ctx(
        ...
        *,
        ...
        return build_slash_ctx(
            ...
            default_timezone=self._agent_manager.artemis_config.default_timezone,
        )
    ...
    async def __call__(self, msg: InboundMessage) -> AsyncIterator[MessageEvent]:
        from artemis.infra.metrics import METRICS  # noqa: PLC0415
        ...
        if mcp_servers:
            from artemis.infra.gateway.process.message_keys import (  # noqa: PLC0415
                COMPOSER_CTX_KEY,
        ...
        if self._is_business_investigation(agent_id):
            try:
                result = await self._run_business_investigation(
                    agent_id=agent_id,
                    thread_id=thread_id,
                    model_ref=model_ref,
                    question=msg.text,
                ...
                yield MessageEvent.completed()
                return
            self._observe_business_trajectory(
                agent_id=agent_id,
                thread_id=thread_id,
                result=result,
            )
            yield MessageEvent.delta(result.answer)
            ...
            await self._record_business_history(thread_id, msg.text, result.answer)
            yield MessageEvent.completed()
            return
        stream_ok = False
        hitl_paused = False
    ...
    async def iter_turn_chunks(self, msg: InboundMessage) -> AsyncIterator[dict[str, Any]]:
        ...
        if self._is_business_investigation(agent_id):
            ...
            await self._record_business_history(thread_id, msg.text, result.answer)
            self._finish_trajectory(thread_id=thread_id, usage=result.usage, enabled=traj_on)
            yield {"type": "done"}
            return
        ...
        try:
            async for chunk in self._agent_manager.stream(agent_id, request):
                ...
                from artemis.infra.history.recorder import flush_tracker  # noqa: PLC0415
                ...
                if chunk.get("type") == "hitl_required":
                    ...
                    if isinstance(request_payload, dict):
                        from artemis.infra.gateway.hitl.coordinator import HitlStreamContext
    ...
    async def _resolve_turn_mcp_servers(
        ...
        *,
        ...
        """
        from artemis.infra.errors import ErrorCode, ArtemisError  # noqa: PLC0415
        ...
        if failed:
            ...
            if raise_on_failure:
                raise ArtemisError(ErrorCode.CONNECTOR_MCP_LOAD_FAILED, detail)
            logger.warning(
    ...
    async def _record_stream_error(self, *, user_id: int, agent_id: str, exc: Exception) -> None:
        from artemis.infra.metrics import METRICS as _M  # noqa: PLC0415
...
def _artemis_user_id(user_id: str | int) -> int | None:
