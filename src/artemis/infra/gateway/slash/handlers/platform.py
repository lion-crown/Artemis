...
from artemis.i18n.domains.slash import localized_rows, tr
from artemis.infra.errors import ErrorCode, ArtemisError
from artemis.infra.gateway.slash.catalog import channel_origin
from artemis.infra.gateway.slash.ctx import SlashCtx, ensure_thread_id, lang_of, resolve_user_agent
from artemis.infra.gateway.slash.formatting import markdown_bullets, markdown_kv_block
from artemis.infra.gateway.slash.help import format_help
...
async def cmd_connectors(
    ...
    if ctx.connector_repo is None:
        ...
        return
    from artemis.infra.connectors.service import list_user_connector_instances
...
async def cmd_exit(d: SlashDispatcher, cmd: SlashCommand, ctx: SlashCtx, sink: SlashSink) -> None:
    raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "/exit handled at CLI level")
