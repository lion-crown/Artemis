...
from artemis.infra.utils.locale import Locale, normalize_locale, resolve_locale

if TYPE_CHECKING:
    from artemis.infra.agents.manager import AgentManager
    from artemis.infra.cron.manager import CronManager
    from artemis.infra.db.repos.agents import AgentRepo, AgentRow
...
def build_slash_ctx(
    *,
    ...
    from artemis.infra.gateway.process.message_keys import COMPOSER_CTX_KEY  # noqa: PLC0415
    ...
    if not timezone and agent_manager is not None:
        timezone = (agent_manager.artemis_config.default_timezone or "").strip()
    if not timezone:
    ...
    return SlashCtx(
        ...
        artemis_version=artemis_version or (meta.version if meta else None),
