...
from artemis.i18n import channel_probe_incomplete, channel_runtime_reason, tr
from artemis.infra.db.repos.channels import ChannelRow
from artemis.infra.db.repos.sessions import SessionRow
from artemis.infra.errors import ErrorCode, ArtemisError
from artemis.infra.gateway.cli import CLI_CHANNEL_ID, CliChannel, CliHub
from artemis.infra.gateway.history_backfill import HistoryBackfillQueue
from artemis.infra.gateway.process import media_backend_for_agent
from artemis.infra.gateway.process.processor import GlobalProcessor
from artemis.infra.gateway.process.response_mode import (
...
from artemis.infra.gateway.ws import (
    WS_CHANNEL_ID,
    WebSocketChannel,
    WebSocketHub,
)
from artemis.infra.utils.locale import DEFAULT_LOCALE, Locale
...
# Artemis owns the supported channel surface even though harness-gateway exposes
# additional transport implementations.  Rows for retired transports remain
# readable so an operator can remove them, but they are never registered.
SUPPORTED_CHANNEL_KINDS = frozenset({"feishu", "dingtalk", "qq", "wecom", "weixin", "mqtt"})
...
class Gateway:
    ...
    async def create_channel(self, spec: ChannelCreateSpec) -> ChannelRow:
        self._ensure_supported_kind(str(spec.kind))
    ...
    async def update_channel(
        ...
        *,
        ...
        if kind is not None:
            self._ensure_supported_kind(kind)
    ...
    async def _preempt_cancel_on_stop(self, _channel_id: str, message: Any) -> None:
        ...
        """
        from artemis.infra.gateway.process.message_keys import session_key_from_message
    ...
    async def probe_channel(
        ...
        if row is None:
            raise ArtemisError(ErrorCode.NOT_FOUND, "channel not found")
    ...
    async def _probe_row(
        ...
        if row.kind not in SUPPORTED_CHANNEL_KINDS:
            raise ArtemisError(ErrorCode.CHANNEL_KIND_UNSUPPORTED, "unsupported channel kind")
        try:
    ...
    async def _register_channel(self, row: ChannelRow) -> None:
        if not self._channel_manager or not self._processor:
            return
        if row.kind not in SUPPORTED_CHANNEL_KINDS:
            self._set_runtime_status(row.channel_id, connected=False, reason="unregistered")
            logger.warning("Skipping retired channel %s (%s)", row.channel_id, row.kind)
            return
        ...
        if row.kind == "wecom" and config.get("delivery_mode") == "final_only":
            # This transport receives a single completed MESSAGE event, so use
            # the processor that retains a complete answer.
            response_mode = "invoke"
        processor = processor_for_response_mode(self._processor, response_mode)
        manager = self._require_channel_manager()
        if row.kind == "wecom" and config.get("delivery_mode") == "final_only":
            from artemis.infra.gateway.channels.wecom_final_only import (  # noqa: PLC0415
            ...
            channel = FinalOnlyWeComChannel(
                processor,
                config=FinalOnlyWeComConfig.from_dict(config),
                channel_id=row.channel_id,
                tenant_id=row.agent_id,
            )
    ...
    def _ensure_supported_kind(kind: str) -> None:
        if kind not in SUPPORTED_CHANNEL_KINDS:
            raise ArtemisError(
                ErrorCode.CHANNEL_KIND_UNSUPPORTED, f"unsupported channel kind {kind!r}"
            )
