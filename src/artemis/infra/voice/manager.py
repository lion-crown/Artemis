...
from artemis.infra.db.repos.voice_providers import VoiceProviderRepo, VoiceProviderRow
from artemis.infra.errors import ErrorCode, ArtemisError
from artemis.infra.voice import adapters
from artemis.infra.voice.presets import is_builtin_preset
...
class VoiceManager:
    ...
    def _validate_provider_name(self, name: str, *, capability: str) -> None:
        ...
        if row is None:
            raise ArtemisError(ErrorCode.NOT_FOUND, f"voice provider {name!r} not found")
        if not row.enabled:
            raise ArtemisError(
    ...
    async def transcribe(
        ...
        *,
        ...
        if kind == "openai":
            if row is None:
                raise ArtemisError(ErrorCode.NOT_FOUND, "OpenAI voice provider is not configured")
            return await adapters.transcribe_openai(row, audio, mime=mime, language=language)
        if kind == "tencent":
            if row is None:
                raise ArtemisError(ErrorCode.NOT_FOUND, "Tencent voice provider is not configured")
            return await adapters.transcribe_tencent(row, audio, mime=mime, language=language)
        if kind == "mimo":
            if row is None:
                raise ArtemisError(ErrorCode.NOT_FOUND, "Mimo voice provider is not configured")
            return await adapters.transcribe_mimo(row, audio, mime=mime, language=language)
        raise ArtemisError(ErrorCode.VOICE_KIND_UNSUPPORTED, f"unsupported STT kind {kind!r}")
        ...
        *,
        ...
        if kind == "mimo":
            ...
            async for chunk in adapters.synthesize_mimo(row, text, voice_id=voice_id, speed=speed):
                yield chunk
            return
        raise ArtemisError(ErrorCode.VOICE_KIND_UNSUPPORTED, f"unsupported TTS kind {kind!r}")
    ...
    async def test_provider(
        ...
        if row is None:
            raise ArtemisError(ErrorCode.NOT_FOUND, "voice provider not found")
        if mode == "stt":
