...
Artemis deployments vary: uv venvs often omit ``pip``, systemd services may not
...
class PackageInstallSpec:
    """Direct PyPI specs plus an optional ``artemis[extra]`` fallback."""
...
def find_uv_binary() -> str | None:
    ...
    override = os.environ.get("ARTEMIS_UV_BIN", "").strip()
    if override:
        path = Path(override)
        if path.is_file():
            return str(path)
