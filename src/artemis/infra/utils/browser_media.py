...
def parse_artemis_user_id(raw: object) -> int | None:
    """Return a positive Artemis ``users.id``, or ``None`` if *raw* is unusable."""
...
def user_browser_profile(user_id: int) -> str:
    """Stable harness-browser profile name for one Artemis user.
...
def legacy_harness_profiles_dir() -> Path:
    """Pre-Artemis default: ``~/.harness-browser/profiles``."""
...
def artemis_browser_profiles_dir(paths: PathLayout | None = None) -> Path:
    """Shared Chrome profiles root: ``~/.artemis/browser-profiles``.

    Profiles live under one Artemis-owned root and are named ``user-<id>``.
    ...
    When the Artemis dir is empty and a legacy harness-browser profiles tree
...
def configure_browser_profiles_dir(profiles_dir: Path | None = None) -> Path:
    ...
    When *profiles_dir* is omitted, uses :func:`artemis_browser_profiles_dir`
    (including legacy migration). Also updates ``BROWSER_USE_PROFILES_DIR``
    and in-process harness-browser settings.
    """
    root = Path(profiles_dir) if profiles_dir is not None else artemis_browser_profiles_dir()
...
def configure_browser_idle_timeout(timeout_minutes: int) -> None:
    """Apply Artemis's browser idle policy to harness-browser."""
