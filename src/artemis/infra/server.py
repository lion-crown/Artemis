"""ArtemisServer — process-level orchestrator."""
...
from artemis.infra.agents.experts.catalog import ExpertCatalog, default_library_root
from artemis.infra.agents.manager import AgentManager
from artemis.infra.agents.plugins.manager import PluginManager
from artemis.infra.agents.subagents.catalog import SubagentCatalog, default_package_root
from artemis.infra.cron.manager import CronManager
from artemis.infra.db.factory import open_database, should_defer_control_plane_db
...
from artemis.infra.mobile.config_probe import ensure_mobile_capabilities_probed
...
from artemis.infra.setup import password_file as _wizard_pw
from artemis.infra.setup.password_file import WIZARD_FILE_NAME
from artemis.infra.setup.wizard_tokens import WizardTokenStore
from artemis.infra.users.manager import UserManager
from artemis.infra.utils.paths import PathLayout

if TYPE_CHECKING:
    from artemis.infra.auth.sso.service import SsoService
    from artemis.infra.trajectory.service import TrajectoryService
...
def _parse_log_compress() -> bool:
    raw = (os.environ.get("ARTEMIS_LOG_COMPRESS", "1") or "1").strip().lower()
...
def delaycompress_rotated_logs(
    ...
    *,
    ...
    plains = [
        entry
        for entry in log_dir.glob("artemis.log.*")
...
def _parse_log_retention_days() -> int:
    raw = os.environ.get("ARTEMIS_LOG_RETENTION_DAYS", str(DEFAULT_LOG_RETENTION_DAYS)) or str(
...
def _parse_log_max_bytes() -> int:
    raw = os.environ.get("ARTEMIS_LOG_MAX_BYTES", str(DEFAULT_LOG_MAX_BYTES)) or str(
...
def _build_log_handler(
    ...
    *,
    ...
    # Rotated files get a date suffix, e.g. artemis.log.2026-07-16
...
def _purge_stale_logs(log_dir: Path, retention_days: int) -> None:
    """Delete rotated artemis log files older than ``retention_days`` (mtime based).
...
class AppRuntime:
    ...
    def replace_services(self, services: SharedServices, config: ArtemisConfig) -> None:
        ...
        if self.trajectory_service is not None:
            from artemis.infra.trajectory.store import TrajectoryStore  # noqa: PLC0415
...
class ArtemisServer:
    def __init__(self, home: Path | None = None) -> None:
        ...
        self.config: ArtemisConfig | None = None
    ...
    def sso_service(self) -> SsoService:
        ...
        from artemis.infra.auth.sso.service import SsoService as SsoServiceCls  # noqa: PLC0415
    ...
    async def start(self) -> None:
        ...
        from artemis.infra.utils.env_file import apply_env_file, env_file_path  # noqa: PLC0415
    ...
    async def _boot_runtime(self, config: ArtemisConfig) -> None:
        ...
        from artemis.infra.trajectory.live import TrajectoryLiveBus  # noqa: PLC0415
        from artemis.infra.trajectory.service import TrajectoryService  # noqa: PLC0415
        from artemis.infra.trajectory.store import TrajectoryStore  # noqa: PLC0415
        ...
        if (
            ...
            from artemis.infra.history.service import HistoryArchive  # noqa: PLC0415
            from artemis.infra.history.store import HistoryStore  # noqa: PLC0415
            from artemis.infra.history.trajectory import ArchiveTrajectoryStore  # noqa: PLC0415
        ...
        from artemis import __version__  # noqa: PLC0415
        ...
        from artemis.infra.cron.delivery import CronDeliveryService  # noqa: PLC0415
        ...
        from artemis.infra.setup.tls.renewal import install_auto_renewal_job

        install_auto_renewal_job(cron_mgr, paths=self.paths)

        from artemis.infra.backup.auto import install_auto_backup_job
        ...
        from artemis.infra.knowledge.jobs import resume_pending_index_jobs  # noqa: PLC0415
    ...
    def _emit_wizard_password(self, *, user_count: int) -> None:
        ...
        if new_pw is not None:
            banner = (
                "\n\033[33m"
                "╔══════════════════════════════════════════════════════════╗\n"
                "║  Artemis first-run wizard password (one-time use):          ║\n"
                f"║  {new_pw:<54}  ║\n"
                "║  Open the dashboard and paste it into the setup wizard.  ║\n"
                "║  File: ~/artemis-login.txt                                   ║\n"
                "╚══════════════════════════════════════════════════════════╝"
                "\033[0m\n"
            )
    ...
    def _setup_logging(self) -> None:
        log_dir = self.paths.ensure_logs_dir()
        log_path = self.paths.log

        # Migrate the legacy single-file log (~/.artemis/artemis.log) into the new logs dir.
        legacy = self.paths.root / "artemis.log"
        if legacy.exists() and not log_path.exists():
            with suppress(OSError):
                legacy.replace(log_path)
        self.paths.ensure_log()
        self.paths.ensure_log_file(datetime.now().astimezone().strftime("%Y-%m-%d") + ".log")
