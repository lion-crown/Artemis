...
Device-code OAuth implementation; constants match the OpenAI wire contract.
Tokens live at ``~/.artemis/codex_oauth.json``.

Artemis is a server (not a local CLI), so it cannot use the PKCE browser
...
completion — so it works regardless of how/where Artemis is deployed.
"""
...
CODEX_ATTRIBUTION_VERSION = (
    os.environ.get("ARTEMIS_CODEX_ATTRIBUTION_VERSION", "").strip() or _OPENCLAW_UPSTREAM_VERSION
)
