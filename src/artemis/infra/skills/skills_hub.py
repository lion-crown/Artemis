...
from artemis.infra.skills.skill_packages import MAX_SKILL_BYTES, MAX_SKILL_FILES
...
def supported_skill_url_prefixes() -> tuple[str, ...]:
    ...
    configured = os.environ.get("ARTEMIS_SKILLS_IMPORT_URL_PREFIXES", "")
...
def _hub_http_timeout() -> float:
    raw = os.environ.get("ARTEMIS_SKILLS_HUB_HTTP_TIMEOUT", "15")
    try:
...
def _hub_http_retries() -> int:
    raw = os.environ.get("ARTEMIS_SKILLS_HUB_HTTP_RETRIES", "3")
    try:
...
def _hub_http_backoff_base() -> float:
    raw = os.environ.get("ARTEMIS_SKILLS_HUB_HTTP_BACKOFF_BASE", "0.8")
    try:
...
def _hub_http_backoff_cap() -> float:
    raw = os.environ.get("ARTEMIS_SKILLS_HUB_HTTP_BACKOFF_CAP", "6")
    try:
...
def _hub_base_url() -> str:
    return os.environ.get("ARTEMIS_SKILLS_HUB_BASE_URL", "https://clawhub.ai")


def _hub_search_path() -> str:
    return os.environ.get(
        "ARTEMIS_SKILLS_HUB_SEARCH_PATH",
        "/api/v1/search",
    )
...
def _hub_version_path() -> str:
    return os.environ.get(
        "ARTEMIS_SKILLS_HUB_VERSION_PATH",
...
def _hub_detail_path() -> str:
    return os.environ.get(
        "ARTEMIS_SKILLS_HUB_DETAIL_PATH",
        "/api/v1/skills/{slug}",
    )
...
def _http_get(
    ...
    req = Request(
        ...
        headers={
            "Accept": accept,
            "User-Agent": "artemis-skills-hub/1.0",
        },
    )
