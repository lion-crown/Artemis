...
from artemis.infra.db.repos.skill_packages import SkillPackageRepo, SkillPackageRow
...
class SkillPackageStore:
    ...
    def assert_can_mutate(self, row: SkillPackageRow, user: User) -> None:
        if self.can_mutate(row, user):
            return
        raise ArtemisError(ErrorCode.FORBIDDEN, "skill package can only be modified by its creator")
