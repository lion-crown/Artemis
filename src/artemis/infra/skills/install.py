...
def with_skillhub_presentation_metadata(
    ...
    *,
    ...
    artemis_meta = {} if not isinstance(artemis_meta, dict) else dict(artemis_meta)

    artemis_meta["source"] = "skillhub"
    if icon_url:
        artemis_meta["icon_url"] = icon_url
    if label:
        artemis_meta["label"] = {key: value for key, value in label.items() if value}
    elif display_name:
        artemis_meta["display_name"] = display_name
    if summary:
        artemis_meta["summary"] = {key: value for key, value in summary.items() if value}
    metadata["artemis"] = artemis_meta
