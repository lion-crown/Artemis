...
def ensure_loopback_no_proxy() -> None:
    """Keep local Chrome CDP traffic out of process-wide HTTP proxies."""
...
def resolve_browser_display() -> str | None:
    ...
    When Artemis's Linux virtual desktop (Xvnc ``:99``) is running, inject
    ...
    try:
        from artemis.infra.desktop.setup import _display_from_env_file  # noqa: PLC0415
...
def _probe_dir_writable(directory: Path) -> bool:
    ...
    try:
        ...
        probe = directory / f".artemis-write-{os.getpid()}"
        probe.write_text("ok", encoding="utf-8")
        probe.unlink()
        if _is_linux():
            link = directory / f".artemis-link-{os.getpid()}"
...
def _under_root_home(path: Path) -> bool:
    ...
    Artemis home (``ARTEMIS_HOME`` / ``~/.artemis``) is exempt so shared profiles
    at ``~/.artemis/browser-profiles`` stay put even when running as root.
    """
    ...
    try:
        ...
        if resolved != root_home and root_home not in resolved.parents:
            return False
        from artemis.infra.utils.paths import PathLayout  # noqa: PLC0415

        artemis_root = PathLayout.from_env().root.resolve()
        return not (resolved == artemis_root or artemis_root in resolved.parents)
    except OSError:
        return False
...
async def prepare_harness_profile_for_launch(
    ...
    *,
    ...
    - Prefers shared ``~/.artemis/browser-profiles`` (cross-agent)
...
def _profiles_root() -> Path:
    try:
        ...
        from artemis.infra.utils.browser_media import artemis_browser_profiles_dir  # noqa: PLC0415

        return artemis_browser_profiles_dir()
...
async def uninstall_browser_stream(*, locale: str = "en") -> AsyncIterator[str]:
    ...
    """
    _ = locale
    yield _sse({"log": "Closing Artemis browser sessions…"})
    ...
    if profiles_root.is_dir():
        yield _sse({"log": "Stopping Artemis-managed browser processes…"})
