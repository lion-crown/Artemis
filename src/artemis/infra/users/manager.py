...
from artemis.infra.db.repos._base import UNSET
from artemis.infra.db.repos.audit import ACTOR_ADMIN
...
from artemis.infra.users.email import normalize_email, parse_optional_email
from artemis.infra.users.identity import Role, User
from artemis.infra.users.password import hash_password, validate_password_policy, verify_password
from artemis.infra.users.permissions import validate_permission_keys
...
class UserManager:
    ...
    async def create(
        ...
        *,
        ...
        if not username:
            raise ArtemisError(ErrorCode.USERNAME_TAKEN, "username must not be empty")
        loc = normalize_locale(locale)
        validate_password_policy(password)
        try:
            keys = validate_permission_keys(permissions or [])
        except ValueError as exc:
            raise ArtemisError(ErrorCode.FORBIDDEN, str(exc), status=400) from exc
    ...
    async def create_from_invite(
        ...
        *,
        ...
        from artemis.infra.users.invites import InviteService
    ...
    async def resolve_or_create_sso_user(
        ...
        *,
        ...
        async with self._lock:
            ...
            if row.disabled:
                raise ArtemisError(ErrorCode.USER_DISABLED, "user is disabled")
    ...
    async def change_password(self, username: str, old: str, new: str) -> None:
        ...
        if row is None or not row.password_hash or not verify_password(old, row.password_hash):
            raise ArtemisError(ErrorCode.AUTH_FAILED, "current password incorrect")
    ...
    async def reset_password(self, username: str, new: str) -> None:
        ...
        row = self._services.user_repo.get_by_username(username)
        if row is None:
            raise ArtemisError(ErrorCode.NOT_FOUND, "user not found")
    ...
    async def set_resource_policy(
        ...
        *,
        ...
        from artemis.infra.users.resource_policy import (
            POLICY_TOKEN_QUOTA,
