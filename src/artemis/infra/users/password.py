...
def validate_password_policy(password: str, *, old_password: str | None = None) -> None:
    """Raise ``ArtemisError`` when ``password`` is too weak or equals ``old_password``.
    ...
    if len(password) < MIN_PASSWORD_LENGTH:
        raise ArtemisError(ErrorCode.PASSWORD_TOO_WEAK, "password too short")
    ...
    if password.lower() in _COMMON_PASSWORDS:
        raise ArtemisError(ErrorCode.PASSWORD_TOO_WEAK, "password is too common")
