...
from artemis.infra.db.repos._base import now_ts
from artemis.infra.db.repos.invites import InviteRepo, InviteRow, invite_status_payload
from artemis.infra.db.services import SharedServices
...
from artemis.infra.utils.locale import normalize_locale
    ...
    def check(self, client_id: str) -> None:
        ...
        with self._lock:
            ...
            if len(bucket) >= RATE_LIMIT_BURST:
                raise ArtemisError(ErrorCode.INVITE_RATE_LIMITED, "too many invite attempts")
...
class InviteService:
        ...
        *,
        ...
        for _ in range(8):
            ...
            if self._repo.get_by_code(code) is None:
                ...
                return row
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "failed to allocate invite code")
    ...
    def revoke(self, invite_id: int, *, actor_username: str) -> dict[str, object]:
        ...
        if before is None:
            raise ArtemisError(ErrorCode.NOT_FOUND, "invite not found")
    ...
    def validate(self, code: str) -> dict[str, object]:
        ...
        if not cleaned:
            raise ArtemisError(ErrorCode.INVITE_INVALID, "invite code required", status=400)
        row = self._repo.get_by_code(cleaned)
        if row is None:
            raise ArtemisError(ErrorCode.INVITE_INVALID, "invite not found")
        status = row.status()
        if status == "used":
            raise ArtemisError(ErrorCode.INVITE_USED, "invite already used")
        if status == "expired":
            raise ArtemisError(ErrorCode.INVITE_EXPIRED, "invite expired")
        if status == "revoked":
            raise ArtemisError(ErrorCode.INVITE_REVOKED, "invite revoked")
    ...
    def redeem(
        ...
        *,
        ...
        if not cleaned_username:
            raise ArtemisError(ErrorCode.USERNAME_TAKEN, "username must not be empty", status=400)
        ...
        try:
            ...
            raise ArtemisError(ErrorCode.INVITE_INVALID, "invite not found") from exc
            ...
            if reason == "used":
                raise ArtemisError(ErrorCode.INVITE_USED, "invite already used") from exc
            if reason == "expired":
                raise ArtemisError(ErrorCode.INVITE_EXPIRED, "invite expired") from exc
            if reason == "revoked":
                raise ArtemisError(ErrorCode.INVITE_REVOKED, "invite revoked") from exc
            raise ArtemisError(ErrorCode.INVITE_INVALID, "invite not usable") from exc
