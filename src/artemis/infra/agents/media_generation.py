    ...
    def save(
        ...
        *,
        ...
        if enabled and image_enabled and not image_model:
            raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "image_model is required")
        if enabled and video_enabled and not video_model:
            raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "video_model is required")
        if enabled and not (api_key or has_stored_key):
            raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "Ark API key is required")
    ...
    async def test_model(
        ...
        *,
        ...
        if not key:
            raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "Ark API key is required")
        if not model:
            raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "media model is required")
