...
from artemis.infra.agents.experts.catalog import ExpertCatalog, build_create_spec_from_expert
from artemis.infra.errors import ErrorCode, ArtemisError
from artemis.infra.utils.host_dirs import host_home_dir, host_path_text
...
async def bootstrap_default_agent(
    ...
    *,
    ...
    if registry.list_agents(user_id):
        return None
    if catalog is None:
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "expert catalog not available")
