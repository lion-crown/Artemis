...
def memory_backend_from_agent_config(
    ...
    * ``{"type": "postgres", "use_control_plane_dsn": true}``
    ...
    if backend is None:
        if artemis_config.database.is_postgresql:
            return {
                "memory_backend": {
                    "type": "postgres",
                    "dsn": artemis_config.database.postgresql_conninfo(),
                }
            }
        return {}
    if not isinstance(backend, dict):
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "memory.backend must be an object")
    ...
    if btype == "postgres":
        ...
        if backend.get("use_control_plane_dsn") or not dsn:
            if not artemis_config.database.is_postgresql:
                raise ArtemisError(
    ...
    raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, f"unsupported memory.backend.type: {btype!r}")
...
def open_memory_kwargs(
    *,
    ...
    resolved = memory_backend_from_agent_config(
        cfg, artemis_config=artemis_config, workspace_dir=workspace_dir
    )
