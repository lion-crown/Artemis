...
COS_ENDPOINT_DEFAULT = "https://artemis-1258344699.cos.ap-guangzhou.myqcloud.com"
COS_PREFIX = "models/embedding"
COS_MANIFEST_NAME = "files.json"
COS_REVISION = "cos-mirror"
_COS_BASE_ENV = "ARTEMIS_ONNX_COS_BASE"

_PROBE_TIMEOUT_S = 4.0
_USER_AGENT = "artemis-onnx-download"
_HF_ALLOW_PATTERNS = (
    "config.json",
    "tokenizer.json",
...
def cos_base_url() -> str:
    """Public COS origin; override with ``ARTEMIS_ONNX_COS_BASE`` for a private bucket."""
...
def export_model_tree_for_cos(
    ...
    *,
    ...
    tqdm_cls = _progress_tqdm(on_progress) if on_progress is not None else None
    with tempfile.TemporaryDirectory(prefix="artemis-onnx-cos-") as tmp:
        snapshot = Path(
            snapshot_download(
                repo_id=hf_repo,
                cache_dir=tmp,
