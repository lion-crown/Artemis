...
# Until orcakit-harness-agent ships these builtins, merge Artemis-side defaults.
_ARTEMIS_BUILTIN_RUNNERS: dict[str, dict[str, Any]] = {
    "kimi_code": {
        "command": "kimi",
        "args": ["acp"],
        "trusted": True,
...
def _runners_response(raw_runners: dict[str, Any]) -> dict[str, Any]:
    ...
    for name, spec in _ARTEMIS_BUILTIN_RUNNERS.items():
