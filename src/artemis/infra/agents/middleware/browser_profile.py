...
from artemis.infra.utils.browser_media import parse_artemis_user_id, user_browser_profile

logger = logging.getLogger(__name__)

_BROWSER_TOOL = "browser_use"
_MISSING_USER = (
    "browser_use blocked: the current turn has no Artemis user id, "
...
def _bind_browser_profile(request: ToolCallRequest) -> ToolCallRequest | ToolMessage:
    ...
    user_id = parse_artemis_user_id(configurable.get("user"))
    if user_id is None:
...
class BrowserProfileMiddleware(AgentMiddleware[Any, Any]):
    """Prevent model-selected profiles from crossing Artemis user boundaries."""
