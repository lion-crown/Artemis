...
* Host-rooted backend → on-disk ``{ARTEMIS_HOME}/agents/<id>/``, persist that
  host absolute path.
* Scoped ``root_dir`` → on-disk ``{root_dir}/.artemis/workspaces/<id>/`` (so files
  land inside the jail), but persist ``/.artemis/workspaces/<id>`` — the
  agent-facing workspace path. Harness gets that persisted string as-is.

User-assigned ``workspace_dir`` always wins (strip only).

:func:`resolve_workspace_host_path` is only for Artemis host FS ops (delete,
memory sqlite path, etc.) when the persisted value is the agent-facing
``/.artemis/workspaces/…`` form.
"""
...
DEFAULT_SYSTEM_FILES_PATH = ".artemis"
...
def resolve_workspace_host_path(raw: str, cfg: dict[str, Any] | None = None) -> Path:
    """Map persisted ``workspace_dir`` to an on-disk path for Artemis host FS ops.

    Harness receives the persisted string as-is (no join here). When the DB
    holds agent-facing ``/.artemis/workspaces/…`` and the backend has a scoped
    ``root_dir``, the on-disk tree is ``{root_dir}/.artemis/workspaces/…``.
    """
    text = raw.strip()
    if not text:
    ...
    if normalized == "/" or normalized.startswith("/.artemis/"):
...
def harness_workspace_path(raw: str, cfg: dict[str, Any] | None = None) -> Path:
    ...
    ``/.artemis/workspaces/<id>`` form has no drive letter, and harness rejects a
...
def agent_facing_workspace_root(
    ...
    *,
    ...
    such as ``/.artemis/workspaces/<id>``. Host-rooted layouts keep the workspace
    path string as-is.
    """
    text = str(workspace_dir).strip().replace("\\", "/")
    if not text:
        return ""
    if text == "/" or text.startswith("/.artemis/"):
...
def workspace_dir_from_config(
    ...
    *,
    ...
    """On-disk workspace for Artemis host ops (may map ``/.artemis/…`` under root_dir).
...
def agent_auth_dir(workspace_dir: Path, cfg: dict[str, Any] | None) -> Path:
    prefix = system_files_path_from_config(cfg)
    legacy = workspace_dir / ".artemis-auth"
    if not prefix:
        return legacy
