...
from artemis.infra.gateway.media.attachment_hints import sniff_image_media_type
...
def sniff_avatar_media_type(data: bytes) -> str:
    ...
    if not data:
        raise ArtemisError(ErrorCode.AVATAR_INVALID, "avatar file is empty")
    if len(data) > MAX_AVATAR_BYTES:
        raise ArtemisError(ErrorCode.AVATAR_TOO_LARGE, "avatar exceeds 5 MiB")
    media_type = sniff_image_media_type(data)
    if media_type is None:
        raise ArtemisError(ErrorCode.AVATAR_INVALID, "avatar content is not a valid image")
    return media_type
...
async def copy_workspace_avatar_to_dir(workspace: WorkspaceAvatarIO, dest: Path) -> str | None:
    """Write the current avatar into a snapshot directory as ``.artemis/avatar.*``."""
...
def read_snapshot_avatar(snapshot_dir: Path) -> tuple[bytes, str] | None:
    """Load ``.artemis/avatar.*`` (or legacy root ``avatar.*``) from a published snapshot."""
    for relpath in _ALL_AVATAR_RELPATHS:
