"""Expert catalog — bundled "scene templates".
...
are written into the agent workspace under ``.artemis/manifest.json``.
...
from artemis.infra.agents.workspace_dir import DEFAULT_SYSTEM_FILES_PATH
...
"""Agent-workspace path for welcome metadata (under ``.artemis/``)."""
...
async def read_workspace_manifest_text(workspace: BackendWorkspace) -> str | None:
    """Read welcome manifest text from the agent workspace (``.artemis`` then legacy root)."""
...
async def read_workspace_manifest_bytes(workspace: BackendWorkspace) -> bytes | None:
    """Read welcome manifest bytes from the agent workspace (``.artemis`` then legacy root)."""
    for rel in _WORKSPACE_MANIFEST_READ_PATHS:
...
async def read_workspace_manifest_data(
    ...
    """Parse workspace ``.artemis/manifest.json`` (legacy root fallback) as a dict."""
...
async def read_workspace_manifest_welcome(
    ...
    """Parse workspace welcome manifest (``.artemis/manifest.json``), if present and valid."""
