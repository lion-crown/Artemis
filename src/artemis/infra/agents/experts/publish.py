...
from artemis.infra.agents.avatar import copy_workspace_avatar_to_dir
...
from artemis.infra.agents.subagents.catalog import slugify
from artemis.infra.db.repos.published_experts import PublishedExpertRepo, PublishedExpertRow
...
def assert_can_mutate_published(row: PublishedExpertRow, user: User) -> None:
    ...
    if user.is_admin or str(user.id) == row.created_by:
        return
    raise ArtemisError(ErrorCode.FORBIDDEN, "published expert can only be modified by its creator")
