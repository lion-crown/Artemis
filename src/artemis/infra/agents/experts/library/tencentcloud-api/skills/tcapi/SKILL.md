...
description: Skill to call Cloud API for Tencent Cloud (腾讯云). Used for cloud automation or resource management. 当用户需要查询、创建、管理腾讯云资源，或执行云 API 自动化操作时触发。优先使用 Artemis 自带 venv 中的 tccli，凭证支持全自动 OAuth 登录。
metadata:
  artemis:
    emoji: "☁️"
    label:
...
**优先使用 Artemis 自带的 Python 虚拟环境（venv）中的 tccli**：与 Artemis 同环境、版本可控、不污染系统 Python。探测顺序：① Artemis venv → ② 系统 PATH → ③ 临时安装进 venv。

```sh
# ① 定位 Artemis venv（通过 artemis 主进程的工作目录；找不到进程则退回常见路径）
...
TCCLI="$ARTEMIS_ROOT/.venv/bin/tccli"

# ② 逐级探测：venv 内 → PATH → 均无则装进 venv
if [ -x "$TCCLI" ]; then :
elif command -v tccli >/dev/null 2>&1; then TCCLI=tccli
else uv pip install --python "$ARTEMIS_ROOT/.venv/bin/python3" tccli; fi

# ③ 验证可运行且凭证有效
"$TCCLI" cvm DescribeRegions >/dev/null 2>&1 && echo "TCCLI_OK" || echo "TCCLI_NEED_CHECK"
```

> 若系统无 `uv`：`"$ARTEMIS_ROOT/.venv/bin/python3" -m ensurepip --upgrade` 后用同路径的 `python3 -m pip install tccli`。
...
| `command not found` / 安装失败 | **未安装** | 按 [references/install.md](references/install.md) 装进 Artemis venv（推荐）或系统安装 |
...
> 环境能打开浏览器时（如桌面版 Artemis），去掉 `BROWSER=echo`，第 ② 步直接提示「浏览器已弹出，请完成授权」即可。

#### 2.3 兜底路径（`AUTH_LOGIN_UNSUPPORTED`，旧版 tccli）

旧版没有 `auth` 子命令。**先自动升级再走 2.2**（装进 Artemis venv，不需要 sudo）：

```sh
uv pip install --python "$ARTEMIS_ROOT/.venv/bin/python3" -U tccli
# 无 uv 时："$ARTEMIS_ROOT/.venv/bin/python3" -m ensurepip --upgrade && ... -m pip install -U tccli
```
...
当直接执行 `tccli` 报错 `bad interpreter`、`No module named tccli` 或 `command not found` 时，通常是 tccli 的 shebang 指向了已卸载的 Python 解释器（环境问题，并非每个用户都会遇到）。此时**优先改用 Artemis venv 的 Python 直接调 `tccli.main`**（venv 里 tccli 与 Artemis 同源，最可靠）；没有 Artemis venv 时才**动态探测**系统 Python 及其 site-packages，**不要硬编码任何平台特定路径**：

```sh
# ① 优先：Artemis venv 的 python（Step 0 已定位 $ARTEMIS_ROOT）
"$ARTEMIS_ROOT/.venv/bin/python3" -c "
import sys
...
- Artemis venv 是第一顺位：tccli 装在 venv 里（Step 0），解释器与包同环境，不存在 shebang 漂移问题
