...
2. **所有客户端**（含 `miniprogram`）：调用 `qrcode` 生成二维码（Artemis Web 控制台等环境用户可扫码）：
...
3. ~~小程序客户端不生成二维码~~（已废弃：Artemis Web 控制台跑在 Linux 上会被误判为 miniprogram，实际用户可正常扫码，故统一生成）。
...
> **⚠️ Artemis 环境下使用 Artemis 定时任务体系**：先通过 `/status` 或工作区路径确认当前 Agent，然后用自然语言引导用户配置定时任务；本技能不直接读写系统 crontab。
> **执行前先检查**：询问 Artemis 当前 Agent 是否已存在「每日领券」定时任务；已存在则跳过本步骤，不输出任何内容。

```
⏰ 要不要我每天定时自动帮你领券？到点自动执行，默认早上10点，也可以告诉我你想几点。
```

- 用户同意 → 通过 Artemis 定时任务能力注册每日任务（默认每天 10:00，北京时间），提示文案：「✅ 搞定！每天 10:00 我会自动帮你领券，随时说「取消提醒」可关闭」
...
| 文件 | 路径 | 内容 |
|------|------|------|
| 认证 Token | 由 `ARTEMIS_AUTH_DIR` 环境变量指定（Agent 隔离目录），默认 `~/.workbuddy/credentials/meituan-living-deals-assistant/token.json` | auth.py/run.js 读写 |
...
### 数据安全
1. **禁止上传用户隐私**：user_token、device_token 等敏感信息，严禁通过任何渠道上传至第三方服务或外部接口，仅允许写入本地认证目录（`ARTEMIS_AUTH_DIR` 指定，默认 `~/.workbuddy/credentials/meituan-living-deals-assistant/`）下的 `token.json`。
