...
// 认证目录：Artemis 多用户/多 Agent 隔离时通过 ARTEMIS_AUTH_DIR 环境变量注入
// （如 ~/.artemis/agents/<id>/credentials/meituan/）；未注入时回退到本机默认目录。
const AUTH_DIR = process.env.ARTEMIS_AUTH_DIR
...
/**
 * Python 解释器解析优先级：
 * 1. ARTEMIS_PYTHON 环境变量（显式指定）
 * 2. Artemis 自托管 venv（裸机安装：$ARTEMIS_HOME/venv/bin/python，默认 ~/.artemis/venv）
 * 3. Artemis Docker 镜像 venv（/app/.venv/bin/python）
 * 4. 回退：PATH 上的 python3 / python（非 Artemis 宿主环境的兼容路径）
 */
function findPython() {
  if (process.env.ARTEMIS_PYTHON && isPython3(process.env.ARTEMIS_PYTHON)) {
    return process.env.ARTEMIS_PYTHON;
  }
  const artemisHome = process.env.ARTEMIS_HOME || path.join(require('os').homedir(), '.artemis');
  const venvCandidates = [
    path.join(artemisHome, 'venv', 'bin', 'python'),            // macOS / Linux
    path.join(artemisHome, 'venv', 'Scripts', 'python.exe'),    // Windows
    '/app/.venv/bin/python',                                   // Artemis Docker 镜像
  ];
...
commands.init = function () {
  ...
  // 2. Python 检查（优先使用 Artemis 自托管 venv，见 findPython 解析顺序）
  let pyVer = '';
  try {
