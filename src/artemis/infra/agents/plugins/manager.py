"""Install, load, and expose plugins under ``~/.artemis/plugins/``."""
...
from artemis.infra.errors import ArtemisError, ErrorCode
from artemis.infra.utils.paths import PRIVATE_FILE_MODE, ensure_private_dir, restrict_to_owner
...
def parse_plugin_ui_meta(plugin_dir: Path) -> dict[str, str] | None:
    ...
    ``ui`` key; Artemis surfaces it for Dashboard dynamic loading.
    """
    try:
...
def parse_plugin_icon(plugin_dir: Path) -> str | None:
    ...
    Harness ignores unknown keys; Artemis surfaces ``icon`` for Dashboard cards.
    """
    try:
        data = _read_plugin_yaml(plugin_dir)
    except Exception:
...
class PluginManager:
    def __init__(self, *, plugins_dir: Path, config_path: Path) -> None:
        self._plugins_dir = ensure_private_dir(plugins_dir)
    ...
    def seed_bundled(self) -> list[str]:
        ...
        from artemis.infra.agents.plugins.bundled import default_bundled_plugins_root
        from artemis.infra.agents.plugins.seed import seed_bundled_plugins
    ...
    def load_missing(self, *, install_deps: bool = False) -> list[LoadedPlugin]:
        ...
        Used after CLI ``artemis plugin install`` while ``artemis run`` is already
    ...
    def set_enabled(self, plugin_id: str, enabled: bool) -> dict[str, Any]:
        ...
        if plugin_dir is None:
            raise ArtemisError(ErrorCode.NOT_FOUND, f"plugin {plugin_id!r} not found")
        try:
    ...
    def resolve_ui_file(self, plugin_id: str, rel_path: str) -> Path:
        ...
        if not cleaned or any(part == ".." for part in cleaned.split("/")):
            raise ArtemisError(ErrorCode.NOT_FOUND, "invalid plugin UI path")
        candidates = [
        ...
        for target in candidates:
            ...
            try:
                ...
                raise ArtemisError(ErrorCode.NOT_FOUND, "invalid plugin UI path") from exc
            if resolved.is_file():
                return resolved
        raise ArtemisError(
            ErrorCode.NOT_FOUND,
    ...
    def sync_skills_to_workspace(
        ...
        *,
        ...
        from artemis.infra.agents.plugin_tool_defaults import agent_plugin_enabled
