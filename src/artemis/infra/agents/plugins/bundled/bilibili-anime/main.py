...
Uses public Bilibili HTTP APIs from the Artemis server (avoids browser CORS).
...
logger = logging.getLogger("artemis.plugins.bilibili_anime")
...
async def bilibili_search_anime(keyword: str, max_seasons: int = 5) -> str:
    ...
    if not keyword:
        return json.dumps(
            {
                "artemis_ui": {"renderer": "bilibili_player", "version": 1},
