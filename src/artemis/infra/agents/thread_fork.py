...
from artemis.infra.db.repos.threads import ThreadRow
...
def find_assistant_fork_index(
    ...
    *,
    ...
    if not answers:
        raise ArtemisError(ErrorCode.NOT_FOUND, "no assistant message to fork from")
    ...
    raise ArtemisError(ErrorCode.NOT_FOUND, "assistant message not found in thread")
...
async def fork_dashboard_thread(
    *,
    ...
    from artemis.infra.history.legacy import checkpoint_wires  # noqa: PLC0415
    ...
    if thread_message_repo is not None:
        from artemis.infra.gateway.process.history_projection import (  # noqa: PLC0415
            message_inputs,
        )
