...
def _skills_dir(workspace: Path) -> Path:
    """Return the writable skills directory (legacy root or ``.artemis/skills``)."""
    env_dir = os.environ.get("ARTEMIS_SKILLS_DIR", "").strip()
    if env_dir:
        path = _workspace(env_dir)
        path.mkdir(parents=True, exist_ok=True)
        return path
    nested_builtin = workspace / ".artemis" / "_builtin_skills"
    nested_skills = workspace / ".artemis" / "skills"
...
def _download(url: str, target: Path) -> None:
    request = urllib.request.Request(url, headers={"User-Agent": "artemis-skill-manager/1.0"})
    total = 0
    try:
...
def _install(workspace: Path, skills: list[dict[str, Any]], *, force: bool) -> list[dict[str, Any]]:
    ...
    if reserved:
        raise SkillManagerError(
            "cannot replace an Artemis-owned built-in skill: " + ", ".join(reserved)
        )
