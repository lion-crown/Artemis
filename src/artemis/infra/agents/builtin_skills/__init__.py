"""Artemis-owned built-in Skills seeded into every agent workspace."""
...
ARTEMIS_BUILTIN_SKILLS_ROOT = "_builtin_skills"
RETIRED_BUILTIN_SKILLS = ("install-skill",)
_WORKSPACE_TOKEN = b"{{ARTEMIS_WORKSPACE}}"
_SKILLS_TOKEN = b"{{ARTEMIS_SKILLS}}"
_BUILTIN_TOKEN = b"{{ARTEMIS_BUILTIN_SKILLS}}"
_PACKAGE = "artemis.infra.agents.builtin_skills"
...
async def sync_artemis_builtin_skills(workspace: Any) -> list[str]:
    """Overwrite Artemis-owned built-ins and remove superseded runtime copies."""
    ...
    for entry in package_root.iterdir():
        ...
        _collect_files(entry, f"{ARTEMIS_BUILTIN_SKILLS_ROOT}/{entry.name}", uploads)

    # Render the agent-facing workspace path into the instructions (not the
    # host ``root_dir`` join that ``resolve_path`` returns for I/O).
    from artemis.infra.gateway.media.inbound_store import (  # noqa: PLC0415
    ...
    builtin_rel = workspace.system_rel(ARTEMIS_BUILTIN_SKILLS_ROOT)
    ...
    for name in RETIRED_BUILTIN_SKILLS:
        path = f"{ARTEMIS_BUILTIN_SKILLS_ROOT}/{name}"
        if await workspace.aexists(path):
            await workspace.adelete(path)
    return sorted(skill_names)


__all__ = ["ARTEMIS_BUILTIN_SKILLS_ROOT", "sync_artemis_builtin_skills"]
