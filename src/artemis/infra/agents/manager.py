...
from artemis.i18n.domains.agents import NO_MODELS_CONFIGURED, format_agent_start_error
from artemis.infra.agents.acp_settings import ACPSettingsStore
from artemis.infra.agents.business_model import BusinessOpenAIModel
from artemis.infra.agents.langfuse import LangfuseSettings, LangfuseSettingsStore
from artemis.infra.agents.media_generation import (
    MediaGenerationSettings,
    MediaGenerationSettingsStore,
)
from artemis.infra.agents.memory_backend import memory_backend_from_agent_config
from artemis.infra.agents.profile import (
    dump_id_list,
...
from artemis.infra.agents.providers import ProviderStore, sync_providers_to_harness
from artemis.infra.agents.runtime_limits import (
...
from artemis.infra.agents.security import SecuritySettingsStore, ToolGuardRulesStore
from artemis.infra.backend.docker_spec import (
    enrich_docker_backend_spec,
    inject_docker_global_environment,
)
from artemis.infra.backend.resolver import (
...
from artemis.infra.db.repos.audit import ACTOR_SYSTEM
from artemis.infra.errors import ErrorCode, ArtemisError
from artemis.infra.skills.presentation import apply_skill_presentation, localize_skill_summary
from artemis.infra.skills.skill_package_store import SkillPackageStore
from artemis.infra.skills.workspace_catalog import list_workspace_skill_summaries
from artemis.infra.utils.locale import Locale
from artemis.infra.utils.ulid import new_short_id

if TYPE_CHECKING:
    from artemis.config import ArtemisConfig
    from artemis.infra.agents.experts.catalog import ExpertCatalog
    ...
    from artemis.infra.db.services import RepoBundle
...
def tools_disabled_set(cfg: dict[str, Any]) -> set[str]:
    ...
    from artemis.infra.agents.tool_catalog import tools_disabled_set as _tools_disabled_set
...
def _resolve_memory_backend_kwargs(
    ...
    *,
    ...
    return memory_backend_from_agent_config(cfg, artemis_config=config, workspace_dir=workspace_dir)
...
async def _fill_missing_subagent_colors(agent: Any, rows: list[dict[str, Any]]) -> None:
    ...
    if not callable(aread):
        return
    from artemis.infra.utils.frontmatter import parse_frontmatter
...
# Custom agent ids become workspace directory names (~/.artemis/agents/<id>/),
...
def validate_custom_agent_id(agent_id: str) -> str:
    """Validate a user-supplied agent id; raise ``ArtemisError`` when invalid."""
    ...
    if lowered in _RESERVED_AGENT_IDS:
        raise ArtemisError(ErrorCode.AGENT_ID_INVALID, f"agent id {agent_id!r} is reserved")
    return agent_id
...
class AgentManager:
    ...
    def __init__(
        ...
        *,
        ...
        self._repos = repos
        self._paths = paths
        from artemis.config import ArtemisConfig as _ArtemisConfig  # noqa: PLC0415

        self._config = config or _ArtemisConfig()
        ...
        self._business_investigation_cancellations: dict[tuple[str, str], asyncio.Event] = {}
    ...
    def replace_persistence(self, repos: RepoBundle, config: ArtemisConfig) -> None:
    ...
    @property
    def artemis_config(self) -> ArtemisConfig:
        return self._config
    ...
    async def create(
        ...
        *,
        ...
        async with self._lock:
            ...
            from artemis.infra.agents.workspace_dir import (  # noqa: PLC0415
                DEFAULT_SYSTEM_FILES_PATH,
                seed_workspace_dir_on_create,
            )
            from artemis.infra.users.resource_policy import raise_if_backend_outside_user_root
    ...
    def resolve_workspace_dir(self, agent_id: str, *, persist_if_missing: bool = True) -> Path:
        """On-disk workspace for Artemis host FS ops (delete, memory path, …).

        May map agent-facing ``/.artemis/workspaces/…`` onto scoped ``root_dir``.
        ...
        # Legacy / incomplete row: classic Artemis layout only (not scoped create default).
    ...
    def find_agents_using_storage_backend(self, backend_name: str) -> list[dict[str, str]]:
        ...
        from artemis.infra.backend.resolver import find_agents_using_storage_backend  # noqa: PLC0415
    ...
    def _unavailable_error(self, agent_id: str) -> ArtemisError:
        """Map a missing live harness handle to the right public error code."""
        row = self.get_row(agent_id)
        if row is None:
            return ArtemisError(ErrorCode.AGENT_NOT_FOUND, f"agent {agent_id!r} not found")
        state = (row.last_state or "").strip().lower()
        if state in ("failed", "error"):
            return ArtemisError(ErrorCode.AGENT_FAILED, f"agent {agent_id!r} failed to start")
        return ArtemisError(ErrorCode.AGENT_NOT_RUNNING, f"agent {agent_id!r} not running")
    ...
    async def delete_thread_checkpoint(self, agent_id: str, thread_id: str) -> bool:
        ...
        Artemis's own ``thread_registry`` only tracks UI metadata (title,
    ...
    def cancel_stream(self, agent_id: str, thread_id: str) -> None:
        ...
        event = self._business_investigation_cancellations.get((agent_id, thread_id))
        if event is not None:
            event.set()
        if self._harness_manager is not None:
            self._harness_manager.cancel(agent_id, thread_id)

    def begin_business_investigation(self, agent_id: str, thread_id: str) -> asyncio.Event:
        """Register one native business investigation for cancellation."""
        event = asyncio.Event()
        self._business_investigation_cancellations[(agent_id, thread_id)] = event
        return event

    def end_business_investigation(self, agent_id: str, thread_id: str) -> None:
        """Release the cancellation handle for a completed business investigation."""
        self._business_investigation_cancellations.pop((agent_id, thread_id), None)

    def get_business_openai_model(
        self,
        agent_id: str,
        model_ref: str | None,
    ) -> BusinessOpenAIModel:
        """Resolve an OpenAI-compatible provider for controlled business investigation."""
        row = self.get_row(agent_id)
        if row is None:
            raise self._unavailable_error(agent_id)
        resolved = (
            model_ref if model_ref and self._providers.is_model_ref_usable(model_ref) else None
        )
        if resolved is None:
            resolved = (
                self._providers.resolve_explicit_default_model(
                    row,
                    self.get_config(agent_id),
                )
                or self.resolve_fallback_model_ref()
            )
        if resolved is None or not self._providers.is_model_ref_usable(resolved):
            raise self._unavailable_error(agent_id)
        provider_name, _, model_id = resolved.partition("/")
        provider = self._repos.provider_repo.get_by_name(provider_name)
        if (
            provider is None
            or provider.kind not in {"openai", "azure", "ollama", "gemini"}
            or not provider.base_url
            or not provider.api_key
            or not model_id
        ):
            raise self._unavailable_error(agent_id)
        from artemis.infra.agents.providers.probe import provider_headers  # noqa: PLC0415

        return BusinessOpenAIModel(
            model=model_id,
            base_url=provider.base_url,
            api_key=provider.api_key,
            headers={str(key): str(value) for key, value in provider_headers(provider).items()},
        )
    ...
    def reload_harness_agents(self) -> None:
        ...
        Does not rebuild Artemis-side agent config from the DB — use :meth:`reload` for that.
        """
        if self._harness_manager is not None:
    ...
    async def reload_connectors(
        ...
        *,
        ...
        """Refresh custom MCP OAuth tokens and reload harness MCP registrations."""
        row = self.get_row(agent_id)
        if row is None:
            return
        ...
        try:
            await self._connector_svc.ensure_fresh_custom_servers(uid)
            await self._reload_agent(agent_id)
        finally:
    ...
    def resolve_tool_display_name_for_chat(
        ...
        *,
        ...
        from artemis.i18n.domains.tools import resolve_tool_display_name
    ...
    async def _get_or_load_mcp_tools(
        ...
        from artemis.infra.connectors.mcp_tool_cache import (
        ...
        async with load_lock:
            ...
            if cached is not None:
                return cached
            from artemis.infra.utils.env_file import (  # noqa: PLC0415
                env_file_path,
                load_env_file,
    ...
    async def apply_persona_mbti(self, agent_id: str, code: str) -> AgentRow:
        ...
        from artemis.infra.agents.persona import PersonaLoader  # noqa: PLC0415
    ...
    async def persist_tools_disabled(self, agent_id: str, disabled: set[str]) -> None:
        ...
        from artemis.infra.agents.tool_catalog import normalize_tools_disabled
    ...
    def validate_mcp_servers(self, user_id: int, mcp_servers: list[str]) -> list[str]:
        ...
        try:
            ...
            raise ArtemisError(ErrorCode.CONNECTOR_NOT_BOUND, str(exc)) from exc
    ...
    async def list_skill_summaries(
        ...
        *,
        ...
        ``kind="workspace"``). Artemis relabels mounted skill-package slugs to
    ...
    def sync_effective_tools_disabled(self, agent_id: str) -> None:
        ...
        from artemis.infra.agents.tool_catalog import effective_tools_disabled
    ...
    async def _post_start_agent(
        ...
        *,
        ...
        ws = agent.workspace
        try:
            from artemis.infra.agents.builtin_skills import (  # noqa: PLC0415
                sync_artemis_builtin_skills,
            )

            synced_skills = await sync_artemis_builtin_skills(ws)
            logger.info(
                "Agent %s: synced Artemis built-in skills=%s",
                row.agent_id,
                synced_skills,
            )
        except Exception:
            logger.warning(
                "Agent %s: failed to sync Artemis built-in skills",
                row.agent_id,
                exc_info=True,
            )
    ...
    def resolved_backend_spec(self, agent_id: str) -> Any:
        """Backend spec for *agent_id* with Artemis docker enrichments applied."""
        row = self._repos.agent_repo.get(agent_id)
        if row is None:
            return None
    ...
    def _prepare_docker_backend(self, backend: Any, row: AgentRow) -> Any:
        """Inject Artemis docker defaults (prefix / agent_id / username) without overwrite."""
        if not isinstance(backend, dict) or backend.get("type") != "docker":
            return backend
        from artemis.infra.utils.env_file import env_file_path  # noqa: PLC0415
    ...
    def _backend_workspace_for_row(
        ...
        *,
        ...
        from artemis.infra.agents.workspace_dir import system_files_path_from_config  # noqa: PLC0415
        from artemis.infra.backend.opensandbox_deps import ensure_opensandbox_deps  # noqa: PLC0415
    ...
    async def _seed_expert_template(self, row: AgentRow, template_name: str) -> None:
        ...
        from artemis.infra.agents.experts.catalog import (  # noqa: PLC0415
            MANIFEST_FILENAME,
    ...
    def _agent_runtime_bundle(
        ...
        configure_browser_profiles_dir(artemis_browser_profiles_dir(self._paths))
        user_display = "User"
        if row.user_id is not None:
            owner = self._repos.user_repo.get(row.user_id)
    ...
    def _peer_manifest_metadata(
        ...
        try:
            ...
            return extra
        for rel in (Path(".artemis") / "manifest.json", Path("manifest.json")):
            path = host / rel
            if not path.is_file():
                continue
            try:
    ...
    def _build_harness_config(self, row: AgentRow) -> HarnessAgentConfig:
        ...
        from artemis.infra.agents.execution_profile import (  # noqa: PLC0415
        ...
        cfg = apply_execution_profile(self._agent_config_dict(row))
        business_investigation = (
            cfg.get("execution_profile") == BUSINESS_SYSTEM_QA
            and cfg.get("investigation_mode") == "business_workspace"
        )
        raw = cfg.get("workspace_dir")
        if isinstance(raw, str) and raw.strip():
            # Persisted value goes to harness as-is; host map is Artemis-local only.
        ...
        if self._cron_manager is not None:
            from artemis.infra.cron.tools import build_cronjob_tools  # noqa: PLC0415

            cron_tools = build_cronjob_tools(self._cron_manager)

        from types import SimpleNamespace  # noqa: PLC0415

        from artemis.infra.knowledge.tools import build_knowledge_tools  # noqa: PLC0415
        ...
        if self._config.capabilities.mobile.enabled:
            from artemis.infra.mobile.tools import build_mobile_tools  # noqa: PLC0415
        ...
        from artemis.infra.agents.plugin_tool_defaults import (  # noqa: PLC0415
        ...
        from artemis.infra.agents.plugin_tool_names import (  # noqa: PLC0415
        ...
        from artemis.infra.agents.middleware.binary_read_guard import BinaryReadGuardMiddleware
        from artemis.infra.agents.middleware.browser_profile import BrowserProfileMiddleware
        from artemis.infra.agents.middleware.reasoning import ReasoningRequestMiddleware
        from artemis.infra.agents.middleware.thread_artifacts import ThreadArtifactsMiddleware
        from artemis.infra.agents.middleware.token_quota import TokenQuotaMiddleware
        from artemis.infra.agents.middleware.workspace_image import (
            WorkspaceImageMaterializeMiddleware,
        )
        from artemis.infra.knowledge.hint import KnowledgeSearchHintMiddleware

        # FilesystemGuard + ModelSettings live in harness-agent (auto-mounted).
        # BinaryReadGuard stays Artemis-specific (inbound/attachment product policy).
        ...
        profile_instruction = cfg.get("execution_profile_instruction")
        if isinstance(profile_instruction, str) and profile_instruction.strip():
            system_prompt = "\n\n".join(
                part for part in [system_prompt, profile_instruction.strip()] if part
            )
            custom_business_instruction = additional_business_reply_instruction(
                cfg.get(BUSINESS_REPLY_GUIDANCE_KEY)
            )
            if custom_business_instruction:
                system_prompt = "\n\n".join(
                    part for part in [system_prompt, custom_business_instruction] if part
                )
        memory: tuple[str, ...] | None = None
        if not business_investigation and not bootstrap_marker_exists(ws):
            system_prompt = None
            memory = ()
        ...
        from artemis.infra.agents.execute_env import inject_agent_execute_env  # noqa: PLC0415
        ...
        investigation_workspace = harness_workspace
        if business_investigation and isinstance(backend, dict):
            from artemis.infra.agents.business_workspace import (  # noqa: PLC0415
            ...
            binding = build_business_workspace_binding(
                backend,
                workspace_dir,
                system_files_path_from_config(cfg),
            )
            backend = binding.backend
            investigation_workspace = binding.workspace_dir
        ...
        harness_cfg = HarnessAgentConfig(
            ...
            workspace_dir=investigation_workspace,
            system_files_path=system_files_path_from_config(cfg),
            ...
            bootstrap_enabled=not business_investigation,
