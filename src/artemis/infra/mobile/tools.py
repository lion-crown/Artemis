...
from artemis.infra.mobile.adb import find_adb, list_devices, screencap_png, shell, swipe, tap
...
_UI_DUMP_PATH = "/sdcard/artemis_ui_dump.xml"
...
def _require_mobile_access(config: ArtemisConfig, ctx: dict[str, Any], user_repo: Any) -> None:
...
def _require_ready(config: ArtemisConfig, *, locale: str) -> None:
...
async def _resolve_device(device: str | None) -> str:
    from artemis.infra.mobile.agent_control import get_mobile_agent_control
...
def build_mobile_tools(
    config: ArtemisConfig, *, user_repo: Any, paths: PathLayout | None = None
) -> list[StructuredTool]:
