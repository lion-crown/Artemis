#!/usr/bin/env bash
# Install / start the Artemis Remote Phone Android container (Redroid).
#
# DinD / docker.sock notes:
# - Prefer --network host so adb 5555 is reachable at 127.0.0.1 from the
#   Artemis process (same netns as dockerd). Falls back to -p 5555:5555.
# - Mount binderfs when present; use --cgroupns=host for nested runtimes.
set -euo pipefail

NAME="${ARTEMIS_MOBILE_CONTAINER:-artemis-mobile-android}"
ADB_HOST="${ARTEMIS_MOBILE_ADB_HOST:-127.0.0.1:5555}"
IMAGE="${ARTEMIS_MOBILE_IMAGE:-redroid/redroid:13.0.0-latest}"
BOOT_WAIT_SECS="${ARTEMIS_MOBILE_BOOT_WAIT_SECS:-45}"
...
# Host networking: Artemis + dockerd share localhost → adb connect 127.0.0.1:5555 works.
