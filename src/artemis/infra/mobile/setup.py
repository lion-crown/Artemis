...
from artemis.infra.mobile.adb import adb_connect, find_adb, list_devices
from artemis.infra.mobile.docker_install import (
...
_CONTAINER_NAME = "artemis-mobile-android"
...
def mobile_capabilities_enabled(config: ArtemisConfig) -> bool:
    return bool(config.capabilities.mobile.enabled)


def mobile_status(config: ArtemisConfig, *, locale: str = "en") -> MobileStatus:
