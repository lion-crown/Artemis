...
from artemis.infra.cron.job import CronJob
...
if TYPE_CHECKING:
    from artemis.infra.db.repos.cron import CronJobRow
    from artemis.infra.db.services import RepoBundle
    from artemis.infra.gateway.gateway import Gateway

from artemis.infra.cron.task_type import normalize_cron_task_type, require_cron_name
    ...
    async def update(
        ...
        *,
        ...
        async with self._lock:
            ...
            if existing is None:
                raise ArtemisError(ErrorCode.NOT_FOUND, f"cron job {cron_id!r} not found")
