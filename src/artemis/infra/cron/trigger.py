...
def build_trigger(spec: str) -> BaseTrigger:
    ...
    if ":" not in spec:
        raise ArtemisError(ErrorCode.CRON_TRIGGER_INVALID, f"trigger spec missing kind: {spec!r}")
    kind, _, value = spec.partition(":")
    value = value.strip()
    if not value:
        raise ArtemisError(ErrorCode.CRON_TRIGGER_INVALID, f"trigger value empty: {spec!r}")
    try:
        if kind == "interval":
    ...
    raise ArtemisError(ErrorCode.CRON_TRIGGER_INVALID, f"unknown trigger kind: {kind!r}")
