...
def assert_agent_access_row(row: Any, user: Any) -> None:
    if not _user_may_access(row, user):
        raise ArtemisError(ErrorCode.FORBIDDEN, "agent not accessible to user")
...
def require_agent_row(
    ...
    *,
    ...
    if as_user is not None and as_user != user.id:
        ...
        if target is None:
            raise ArtemisError(ErrorCode.NOT_FOUND, f"user {as_user} not found")
        if row.user_id is not None and row.user_id != as_user:
            raise ArtemisError(ErrorCode.FORBIDDEN, "agent not owned by as_user")
    else:
