...
from artemis.infra.agents.memory_backend import open_memory_kwargs
from artemis.infra.agents.workspace_dir import host_system_dir
...
def memory_db_path(workspace_dir: Path) -> Path:
    ...
    ``.artemis/_builtin_skills`` tree as the new-layout signal (empty ``.artemis/``
    alone is not enough for legacy agents).
    """
    nested = workspace_dir / ".artemis" / "memory.sqlite"
    root = workspace_dir / "memory.sqlite"
    if nested.exists():
        return nested
    if root.exists():
        return root
    if (workspace_dir / ".artemis" / "_builtin_skills").is_dir():
        return nested
    return root
...
def _open_memory_for_agent(server: Any, agent_id: str) -> tuple[Any, Any]:
    from artemis.api.common.agent_workspace import resolve_agent_workspace_dir  # noqa: PLC0415
    ...
    ns, backend, backend_config = open_memory_kwargs(
        agent_id=agent_id,
        cfg=cfg,
        artemis_config=server.services.config,
    ...
    *,
    ...
    if "error" in response:
        ...
        if code == -32010:  # ERR_PATH_NOT_FOUND
            raise ArtemisError(ErrorCode.NOT_FOUND, message)
        if code == -32602:  # ERR_INVALID_PARAMS
            raise ArtemisError(ErrorCode.INTERNAL_ERROR, message, status=400)
        if code == -32601:  # ERR_METHOD_NOT_FOUND
            raise ArtemisError(
                ErrorCode.INTERNAL_ERROR,
                f"unknown memory dashboard method: {method!r}",
            )
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, message)
