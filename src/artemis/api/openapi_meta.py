...
from artemis.api.deps import is_jwt_exempt_path

API_DESCRIPTION = """\
**Artemis** is a smarter, self-hosted AI assistant for multiple users and agents. All routes are served under `/api`.
...
token in the `X-Artemis-Access-Token` header. Clients should replace the stored
token when present.
...
`/api/auth/invite/redeem`, and `/api/connectors/oauth/callback`.
...
OPENAPI_TAGS: list[dict[str, str]] = [
    ...
    {
        "name": "connectors",
        "description": "User-defined MCP server configuration and OAuth authorization.",
    },
    {
        "name": "knowledge",
    ...
    {
        "name": "channels",
        "description": "Instant-messaging bridges (WeCom, Feishu, QQ, …) per agent.",
    },
    {
        "name": "cron",
    ...
    {
        "name": "envs",
        "description": "Global environment variables (~/.artemis/env) inherited by every agent.",
    },
    {
        "name": "search",
