...
(using the established workspace convention). This router thinly wraps the workspace
...
from artemis.api.common.agent import require_agent_owner_row, require_agent_row
from artemis.api.deps import current_user, get_server, require_permission
from artemis.infra.agents.manager import (
...
from artemis.infra.skills.presentation import apply_skill_presentation
...
from artemis.infra.skills.skill_transfer import (
...
def _parse_frontmatter(text: str) -> tuple[dict[str, Any], str]:
    ...
    handle YAML for simplicity since skills use YAML frontmatter.
...
def _valid_skillhub_icon_url(value: str) -> bool:
    from artemis.infra.skills.install import valid_skillhub_icon_url  # noqa: PLC0415
...
def _map_skillhub_install_error(err_msg: str, skill_name: str) -> Any | None:
    ...
    from artemis.i18n import error_message  # noqa: PLC0415
    from artemis.infra.utils.ssl_errors import looks_like_ssl_error  # noqa: PLC0415
...
async def validate_chat_skills(
    ...
    *,
    ...
    from artemis.api.common.validators import validate_chat_skills as _validate
...
def _skill_transfer_error(exc: SkillPackageError, *, locale: Locale) -> ArtemisError:
    if isinstance(exc, SkillTransferConflict):
        return ArtemisError.localized(
            ErrorCode.SKILL_ALREADY_EXISTS,
            locale,
            name=exc.slug,
        )
    if isinstance(exc, SkillTransferNotFound):
        return ArtemisError(ErrorCode.NOT_FOUND, str(exc))
...
async def get_skill(
    ...
    if resolved is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, f"skill {name!r} not found")
...
async def create_skill(
    ...
    try:
        ...
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, str(exc)) from exc
    except SkillPackageError:
        raise ArtemisError(ErrorCode.NOT_FOUND, "invalid skill name") from None
    await _guard_package_only_skill_write(ctx.workspace, ctx.config, server, name)
    ...
    if existing is not None:
        ...
        if not meta.get("removed") and not body.overwrite:
            raise ArtemisError(ErrorCode.SKILL_ALREADY_EXISTS, f"skill {name!r} already exists")
    with contextlib.suppress(Exception):
...
async def delete_skill(
    ...
    manifest_path, kind, _body = resolved
    if kind == "builtin":
        raise ArtemisError(ErrorCode.NOT_FOUND, f"builtin skill {name!r} cannot be deleted")
    ...
    if err:
        raise ArtemisError(ErrorCode.NOT_FOUND, f"cannot remove {target!r}: {err}")
