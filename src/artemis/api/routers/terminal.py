...
from artemis.api.deps import get_server, require_permission, resolve_user_from_token
from artemis.infra.errors import ErrorCode, ArtemisError
from artemis.infra.users.permissions import user_has_permission
from artemis.infra.utils import posix_compat
...
# Artemis web terminal — reduce stray blank / spacer lines in browser PTYs.
...
artemis_web_terminal_precmd() {
  export PROMPT_EOL_MARK=
  unsetopt prompt_sp 2>/dev/null
}
typeset -ga precmd_functions
precmd_functions=("${precmd_functions[@]}" artemis_web_terminal_precmd)
"""


def _zsh_web_zdotdir() -> str:
    """Return a temp ZDOTDIR whose .zshrc sources the user config then re-applies fixes."""
    d = tempfile.mkdtemp(prefix="artemis-zdotdir-")
...
async def terminal_context(
    ...
    if row is None:
        raise ArtemisError(ErrorCode.AGENT_NOT_FOUND, "no agents for user")
    assert_agent_owner(row, user)
    # AI-facing path: persisted config value (e.g. ``/.artemis/workspaces/<id>``).
...
async def terminal_ws(
    ...
    """
    server = websocket.app.state.artemis_server
