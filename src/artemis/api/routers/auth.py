...
from artemis.api.deps import current_user, get_server, sign_token
from artemis.infra.errors import ErrorCode, ArtemisError
from artemis.infra.users.permissions import effective_permissions
...
async def login(body: LoginBody, server: Any = Depends(get_server)) -> dict[str, Any]:
    ...
    if server.user_manager.count() == 0:
        raise ArtemisError(ErrorCode.SETUP_REQUIRED, "initial admin not created")
    user = await server.user_manager.authenticate(body.username, body.password)
    if user is None:
        raise ArtemisError(ErrorCode.AUTH_FAILED, "invalid credentials")
