...
from artemis.api.deps import get_server, require_permission
from artemis.infra.agents.experts.skillhub_market import (
    SkillHubMarketError,
    SkillHubMarketErrorKind,
    fetch_skillsets,
)
from artemis.infra.db.repos.skill_packages import SkillPackageRow
from artemis.infra.errors import ErrorCode, ArtemisError
from artemis.infra.server import ArtemisServer
from artemis.infra.skills.install import valid_skillhub_icon_url
from artemis.infra.skills.skill_package_from_skillhub import create_package_from_skillhub
from artemis.infra.skills.skill_package_store import (
    SkillPackageStore,
    is_skill_package_name_conflict,
    raise_skill_package_name_taken,
)
from artemis.infra.skills.skillhub_market import SkillHubMarketError as SkillHubDownloadError
from artemis.infra.users.identity import User
from artemis.infra.utils.frontmatter import parse_frontmatter
from artemis.infra.utils.locale import Locale, resolve_request_locale
...
def _files_from_package_skill_body(
    *,
    ...
    if files:
        ...
        return decoded
    if not content:
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "content or files is required")
...
def _store(server: ArtemisServer) -> SkillPackageStore:
    if server.services is None:
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "skill package store not initialized")
...
def _package_or_404(
    ...
    *,
    ...
    if row is None:
        raise ArtemisError.localized(ErrorCode.SKILL_PACKAGE_NOT_FOUND, locale)
    return row
...
def _creator_fields(server: ArtemisServer, created_by: str) -> dict[str, str | None]:
...
def _required(value: str, field: str) -> str:
    value = value.strip()
    if not value:
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, f"{field} is required")
    return value
...
def _icon_url(value: str) -> str:
    ...
    if value and not valid_skillhub_icon_url(value):
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "invalid SkillHub icon URL")
    return value


def _map_skillhub_error(exc: BaseException) -> ArtemisError:
    ...
    if kind in (
        ...
        return ArtemisError(ErrorCode.NOT_FOUND, "skillhub skillset not found")
...
def _skillset_summary(item: Any) -> dict[str, Any]:
    return {
        "slug": item.slug,
        ...
        "skill_count": item.skill_count or len(item.skill_slugs),
    }
...
def _package_skill_or_404(
    ...
    *,
    ...
    for skill in store.list_skill_summaries(
        ...
        if skill["slug"] == slug:
            return skill
    raise ArtemisError(ErrorCode.NOT_FOUND, f"skill {slug!r} not found")


@router.get("", summary="List global skill packages")
async def list_skill_packages(
    writable_only: bool = False,
    server: ArtemisServer = Depends(get_server),
...
async def package_hub_search(
    ...
    from artemis.infra.skills.skillhub_market import (  # noqa: PLC0415
        SkillHubMarketError,
        search_skillhub,
    )
...
@router.get("/hub/skillsets", summary="List SkillHub skillsets for global packages")
async def list_skillhub_skillsets(
    _user: User = Depends(require_permission("skill_packages")),
) -> dict[str, list[dict[str, Any]]]:
    try:
        items = await asyncio.to_thread(fetch_skillsets)
    except SkillHubMarketError as exc:
        raise _map_skillhub_error(exc) from exc
    return {"items": [_skillset_summary(item) for item in items]}
...
async def _refresh_package(server: ArtemisServer, package_id: str) -> None:
    if server.app_runtime is not None:
        await server.app_runtime.agent_registry.refresh_agents_for_package(package_id)
...
async def import_package_skill(
    ...
    from artemis.infra.skills.install import (  # noqa: PLC0415
        SkillAlreadyExistsError,
        commit_skill_install,
        resolve_url_import,
    )
    from artemis.infra.skills.skill_packages import SkillPackageError  # noqa: PLC0415
    from artemis.infra.skills.skills_hub import is_supported_skill_url  # noqa: PLC0415
    ...
    if not bundle_url:
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "bundle_url is required")
    if not is_supported_skill_url(bundle_url):
        raise ArtemisError.localized(ErrorCode.SKILL_IMPORT_UNSUPPORTED_URL, locale)
...
async def hub_install_package_skill(
    ...
    from artemis.infra.skills.skill_packages import (  # noqa: PLC0415
        SkillPackageError,
    ...
    try:
        ...
        transport = "cli"
        from artemis.api.routers.skills import (  # noqa: PLC0415
