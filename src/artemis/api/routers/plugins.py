...
from artemis.api.common.agent import assert_agent_owner as _assert_agent_owner
from artemis.api.deps import current_user, get_server, require_permission
from artemis.infra.agents.plugin_tool_defaults import (
...
def _plugin_manager(server: ArtemisServer) -> PluginManager:
    mgr = server.plugin_manager
    if mgr is None:
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "plugin manager not initialized")
    return mgr
...
async def reload_plugins(
    ...
    """Re-read ``~/.artemis/plugins`` into the process registry and reload agents.

    Use after a CLI ``artemis plugin install`` while ``artemis run`` is already up —
...
async def upload_plugin(
    ...
    raw = await file.read()
    if not raw:
        raise ArtemisError(ErrorCode.PLUGIN_INVALID_ARCHIVE, "empty plugin archive")
    try:
...
async def get_plugin_ui_asset(
    ...
    """Read-only static files from ``~/.artemis/plugins/<id>/`` (typically ``ui/dist/``).
...
async def patch_agent_plugins(
    ...
    if unknown:
        raise ArtemisError(ErrorCode.NOT_FOUND, f"plugin {unknown[0]!r} not found")
