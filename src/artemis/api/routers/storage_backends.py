...
from artemis.infra.backend.adapter import row_to_backend_spec, storage_spec_previewable
...
def _ensure_opensandbox_sdk(kind: str) -> None:
    if kind != "opensandbox":
        return
    from artemis.infra.backend.opensandbox_deps import ensure_opensandbox_deps

    try:
        ensure_opensandbox_deps(allow_install=True)
    except RuntimeError as exc:
        raise ArtemisError(ErrorCode.STORAGE_BACKEND_DEPS_FAILED, str(exc)) from exc
...
async def create_storage_backend(
    ...
    if server.services.storage_backend_repo.get_by_name(body.name) is not None:
        raise ArtemisError(ErrorCode.STORAGE_BACKEND_NAME_TAKEN, f"name {body.name!r} already exists")
...
async def patch_storage_backend(
    ...
    if row is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, "storage backend not found")
...
async def probe_storage_backend_config(
    ...
    from artemis.infra.backend.probe import probe_storage_backend, row_for_probe
...
async def list_storage_backend_tree(
    ...
    if row is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, "storage backend not found")
    from artemis.infra.backend.browse import list_storage_backend_tree as _list_tree

    try:
        return await _list_tree(row, path)
    except ValueError as exc:
        raise ArtemisError(ErrorCode.WORKSPACE_OP_UNSUPPORTED, str(exc)) from exc
...
async def test_storage_backend(
    ...
    if row is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, "storage backend not found")
    from artemis.infra.backend.probe import probe_storage_backend
