...
from artemis.api.deps import get_server, require_permission, sign_token
from artemis.api.routers.auth import _user_json
from artemis.infra.auth.sso.service import SsoService
from artemis.infra.errors import ErrorCode, ArtemisError

router = APIRouter()
_OIDC_STATE_COOKIE = "artemis_oidc_state"
...
def _bad_request(exc: ValueError) -> ArtemisError:
    return ArtemisError.localized(ErrorCode.OIDC_BAD_REQUEST, detail=str(exc))
