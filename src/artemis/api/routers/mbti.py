...
Artemis is multi-agent: the MBTI code lives on ``agents.persona_mbti`` /
``config_json["persona"]`` and is rendered into workspace ``SOUL.md`` on
agent reload.  The active agent is selected via ``X-Artemis-Agent-Id``.
"""
...
from artemis.api.common.agent import assert_agent_owner
from artemis.api.deps import current_user, get_server
from artemis.infra.agents.mbti_profiles import (
    MBTIProfile,
    get_all_profiles,
    get_profile,
)
...
def _resolve_agent_row(server: Any, user: Any, agent_id: str) -> Any:
    ...
    if row is None:
        raise ArtemisError(ErrorCode.AGENT_NOT_FOUND, f"agent {agent_id} not found")
...
async def get_current_mbti(
    x_artemis_agent_id: str = Header(..., alias="X-Artemis-Agent-Id"),
    user: Any = Depends(current_user),
    server: Any = Depends(get_server),
) -> CurrentMBTIResponse:
    """Read the current MBTI type from the active agent's persisted config."""
    row = _resolve_agent_row(server, user, x_artemis_agent_id)
...
async def get_persona_preview(
    ...
    from artemis.infra.agents.persona import PersonaLoader
...
async def submit_test(
    ...
    x_artemis_agent_id: str | None = Header(None, alias="X-Artemis-Agent-Id"),
    user: Any = Depends(current_user),
    server: Any = Depends(get_server),
) -> TestResultResponse:
    ...
    applied = False
    if req.auto_apply:
        if not x_artemis_agent_id:
            raise HTTPException(
                status_code=400,
                detail="X-Artemis-Agent-Id header required when auto_apply is true",
            )
        row = _resolve_agent_row(server, user, x_artemis_agent_id)
        try:
            await _persist_persona(server, x_artemis_agent_id, row, code)
            applied = True
        except Exception:
            logger.exception("Failed to auto-apply MBTI %s to agent %s", code, x_artemis_agent_id)
            applied = False
...
async def apply_type(
    ...
    try:
        ...
        logger.exception("Failed to apply MBTI %s to agent %s", code, x_artemis_agent_id)
