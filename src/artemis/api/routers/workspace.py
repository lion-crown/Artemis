...
from artemis.api.common.agent_workspace import resolve_agent_workspace_dir
from artemis.api.common.content_disposition import content_disposition
from artemis.api.common.workspace import (
    coerce_read_content,
    file_info_to_dict,
    reanchor_entry_path,
...
from artemis.infra.gateway.media.backend_files import (
    backend_workspace_path,
    is_allowed_host_download_abs_path,
    is_host_absolute_path,
    resolve_preview_payload,
)
from artemis.infra.utils.doc_edit import DocConverter, get_doc_converter
...
def _assert_workspace_mutable(path: str) -> str:
    ...
    if rel == ".":
        raise ArtemisError(ErrorCode.FORBIDDEN, "cannot modify workspace root")
    posix = rel.replace("\\", "/").strip("/")
    if (
        posix == _PROTECTED_PREFIX
        or posix.startswith(f"{_PROTECTED_PREFIX}/")
        or posix == f".artemis/{_PROTECTED_PREFIX}"
        or posix.startswith(f".artemis/{_PROTECTED_PREFIX}/")
    ):
        raise ArtemisError(ErrorCode.FORBIDDEN, f"cannot modify {_PROTECTED_PREFIX!r} paths")
    return rel


def _map_workspace_fs_error(exc: Exception, *, operation: str, path: str) -> ArtemisError:
    if isinstance(exc, BackendOperationNotSupportedError):
        return ArtemisError(ErrorCode.WORKSPACE_OP_UNSUPPORTED, str(exc))
    if isinstance(exc, FileNotFoundError):
        return ArtemisError(ErrorCode.NOT_FOUND, f"cannot {operation} {path!r}: not found")
    if isinstance(exc, FileExistsError):
        return ArtemisError(ErrorCode.SLASH_BAD_ARGS, str(exc))
    if isinstance(exc, PermissionError):
        return ArtemisError(ErrorCode.FORBIDDEN, str(exc))
    if isinstance(exc, ValueError):
        return ArtemisError(ErrorCode.SLASH_BAD_ARGS, str(exc))
    return ArtemisError(ErrorCode.INTERNAL_ERROR, f"cannot {operation} {path!r}: {exc}")
...
def _workspace_io_path(path: str, *, from_workspace: bool = False) -> str:
    ...
    if raw.startswith("file://"):
        ...
        if resolved is None:
            raise ArtemisError(ErrorCode.NOT_FOUND, f"cannot resolve {path!r}")
        return resolved
    if from_workspace:
...
async def list_tree(
    ...
    if result is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, f"cannot list {path!r}")
...
async def read_file(
    ...
    if content is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, f"cannot read {path!r}")
...
async def write_file(
    ...
    try:
        ...
        raise ArtemisError(ErrorCode.NOT_FOUND, f"cannot write {path!r}: {exc}") from exc
...
async def upload_file(
    ...
    try:
        ...
        raise ArtemisError(ErrorCode.NOT_FOUND, f"cannot upload to {target!r}: {exc}") from exc
...
async def download_file(
    ...
    outputs (Desktop, ``~/.artemis/agents/…``, workspace tree) but denied for
    ...
    if is_host_absolute_path(io_path) and not is_allowed_host_download_abs_path(
        ...
        raise ArtemisError(ErrorCode.FORBIDDEN, f"cannot download {path!r}: path not allowed")

    try:
        file_blob = await ws.adownload_bytes(io_path)
    except PermissionError as exc:
        raise ArtemisError(ErrorCode.NOT_FOUND, f"cannot download {path!r}") from exc
    if file_blob is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, f"cannot download {path!r}") from None
...
async def read_doc(
    ...
    try:
        ...
        raise ArtemisError(ErrorCode.NOT_FOUND, f"cannot read {path!r}") from exc
    if blob is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, f"cannot read {path!r}")
    try:
        content = converter.to_markdown(blob)
    except Exception as exc:
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, f"cannot parse {path!r}: {exc}") from exc
...
async def preview_media(
    ...
    if payload is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, "preview not available for this source")
    data, mime = payload
...
async def glob_files(
    ...
    if pattern in ("**/*.md", "*.md") and root == ".":
        ...
        if ls_result is None:
            raise ArtemisError(ErrorCode.NOT_FOUND, "glob failed")
...
async def grep_files(
    ...
    if result is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, "grep failed")
...
async def import_workspace_archive(
    ...
    if len(raw) > _MAX_WORKSPACE_ARCHIVE_BYTES:
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "workspace archive too large (max 200MB)")
    if not raw:
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "empty archive")
