...
from artemis.api.deps import get_server, require_database, resolve_user_from_token, sign_token
from artemis.infra.agents.providers.presets import load_provider_presets
from artemis.infra.agents.providers.probe import make_probe_provider_row, probe_provider_row
from artemis.infra.errors import ErrorCode, ArtemisError
from artemis.infra.setup import password_file as _wizard
from artemis.infra.setup.wizard_tokens import RateLimited
from artemis.infra.users.identity import Role
from artemis.infra.utils.locale import normalize_locale, resolve_request_locale
...
def _enforce_wizard_open(server: Any) -> None:
    ...
    if um is not None and um.count() != 0:
        raise ArtemisError(ErrorCode.SETUP_REQUIRED, "setup already completed", status=410)
...
def _extract_bearer(authorization: str | None) -> str:
    if not authorization or not authorization.startswith("Bearer "):
        raise ArtemisError(ErrorCode.SETUP_TOKEN_INVALID, "wizard token required")
...
def _require_wizard_token(authorization: str | None, server: Any) -> str:
    ...
    if not server.wizard_tokens.validate(token):
        raise ArtemisError(ErrorCode.SETUP_TOKEN_INVALID, "invalid or expired wizard token")
    return token
...
async def _bootstrap_default_agent(server: Any, *, user_id: int, locale: str = "zh") -> None:
    ...
    from artemis.infra.agents.default_agent import (
        SETUP_DEFAULT_AGENT_ID,
        bootstrap_default_agent,
    )

    if server.app_runtime is None:
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "app_runtime not ready")
...
async def _apply_provider_draft(server: Any, draft: ProviderDraftBody) -> None:
    ...
    if not api_key:
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "api_key is required", status=400)
    if not base_url:
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "base_url is required", status=400)
    ...
    if not models:
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "at least one model is required", status=400)
...
async def test_database(
    ...
    from artemis.infra.db.probe import probe_database
    from artemis.infra.db.rebind import database_config_from_payload
...
async def apply_database(
    ...
    _enforce_wizard_open(server)
    from artemis.infra.db.probe import probe_database
    from artemis.infra.db.rebind import (
...
async def begin_setup(server: Any = Depends(get_server)) -> dict[str, Any]:
    ...
    if _setup_password_required(server):
        raise ArtemisError(ErrorCode.FORBIDDEN, "setup password required")
...
async def verify_password(
    ...
    if not _setup_password_required(server):
        raise ArtemisError(ErrorCode.FORBIDDEN, "setup password not required")
    client_ip = request.client.host if request.client else "unknown"
    try:
        server.wizard_tokens.record_attempt(client_ip)
    except RateLimited:
        raise ArtemisError(ErrorCode.SETUP_RATE_LIMITED, "too many attempts") from None
    expected = _wizard.read_password(Path.home())
    if expected is None or body.password != expected:
        raise ArtemisError(ErrorCode.SETUP_PASSWORD_WRONG, "wrong password")
    token, ttl = server.wizard_tokens.issue()
    return {"wizard_token": token, "expires_in": ttl}
...
async def resume_wizard(server: Any = Depends(get_server)) -> dict[str, Any]:
    ...
    if server.user_manager.count() == 0:
        raise ArtemisError(ErrorCode.SETUP_TOKEN_INVALID, "admin not created yet", status=400)
    token, ttl = server.wizard_tokens.issue()
    return {"wizard_token": token, "expires_in": ttl}
