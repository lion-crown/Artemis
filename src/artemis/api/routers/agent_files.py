...
from artemis.api.common.workspace import require_running_workspace
...
async def read_daily_memory(
    ...
    if content is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, f"cannot read {target!r}")
...
async def delete_daily_memory(
    ...
    try:
        ...
        raise ArtemisError(ErrorCode.FORBIDDEN, "path escapes workspace") from exc
    if not target.exists():
        raise ArtemisError(ErrorCode.NOT_FOUND, f"daily memory {filename!r} not found")
    target.unlink()
