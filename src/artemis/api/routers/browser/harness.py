...
from artemis.api.deps import current_user
from artemis.infra.errors import ErrorCode, ArtemisError
from artemis.infra.utils.browser_media import user_browser_profile
...
async def resolve_harness_session(
    ...
    *profile_hint* must be a concrete profile name (``user-<id>``). Empty /
    ...
    if not hint or hint == "auto":
        if not create:
            return None
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "browser profile is required")

    if not _locked:
        lock = _SESSION_CREATE_LOCKS.setdefault(hint, asyncio.Lock())
        async with lock:
            return await resolve_harness_session(
                hint,
                server=server,
                agent_id=agent_id,
                create=create,
    ...
    if server is not None and agent_id:
        from artemis.api.common.agent_workspace import (  # noqa: PLC0415
    ...
    from artemis.infra.browser.setup import (  # noqa: PLC0415
        ensure_loopback_no_proxy,
        prepare_harness_profile_for_launch,
        resolve_browser_display,
    )

    # Shared ~/.artemis/browser-profiles (not per-agent workspace).
    ensure_loopback_no_proxy()
    ...
    try:
        ...
        msg = str(exc)
        if (
            "did not receive a valid HTTP response" in msg
            or "opening handshake" in msg
            or "connection closed while reading HTTP status line" in msg
        ):
            # Chrome can expose /json before its selected page target accepts
            # a CDP WebSocket. Retry once instead of surfacing a transient
            # launch race to the browser dashboard.
...
async def handoff(
    ...
    if target not in ("agent", "user"):
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "target must be 'agent' or 'user'")
...
async def shutdown_browser(user: Any = Depends(current_user)) -> dict[str, Any]:
    """Terminate the current user's Artemis-managed Chrome. Cookies stay on disk."""
    try:
