...
from artemis.api.deps import current_user, get_server, require_admin
...
def _runners_payload(body: ACPRunnersBody) -> dict[str, dict[str, Any]]:
    ...
    for name, runner in body.runners.items():
        key = name.strip()
        if not key:
            raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "runner name cannot be empty")
...
async def get_global_acp_runner(
    ...
    if runner is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, f"ACP runner {runner_name!r} not found")
    return dict(runner)
...
async def delete_global_acp_runner(
    ...
    if runner_name in _BUILTIN_RUNNERS:
        raise ArtemisError(ErrorCode.FORBIDDEN, f"built-in runner {runner_name!r} cannot be deleted")
    registry = _registry(server)
    runners = registry.acp_settings.load_runners(user.id)
