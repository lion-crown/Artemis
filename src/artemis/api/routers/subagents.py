...
from artemis.infra.agents.subagents.catalog import SubagentDefinition
from artemis.infra.errors import ErrorCode, ArtemisError
from artemis.infra.utils.locale import (
    Locale,
    normalize_locale,
...
def _require_catalog(server: Any) -> Any:
    ...
    if catalog is None:
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "subagent catalog not loaded")
    return catalog
...
async def get_subagent_catalog_item(
    ...
    if item is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, f"subagent {slug!r} not found")
