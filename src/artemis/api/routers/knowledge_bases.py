...
from artemis.api.common.upload_limit import read_upload_capped
from artemis.api.deps import current_user, get_server, require_permission
from artemis.config import DEFAULT_MAX_UPLOAD_MB, MAX_MAX_UPLOAD_MB, upload_mb_to_bytes
from artemis.infra.agents.providers.model_flags import (
    is_chat_eligible_model,
    is_embedding_model,
    is_onnx_local_provider,
    is_vision_model,
)
from artemis.infra.agents.providers.onnx_catalog import (
    ONNX_PRESET_MODEL_IDS,
    list_onnx_catalog_models,
)
from artemis.infra.agents.providers.onnx_service import (
    DOWNLOAD_MANAGER,
    OnnxServiceConfig,
...
from artemis.infra.knowledge.files import document_path
from artemis.infra.knowledge.gate import (
    assert_knowledge_usable,
    get_capability,
    set_feature_enabled,
)
from artemis.infra.knowledge.jobs import enqueue_index_document, reindex_all_documents
from artemis.infra.knowledge.ocr import (
    ensure_ocr_deps_async,
    get_ocr_capability,
    set_ocr_settings,
    validate_ocr_settings,
)
from artemis.infra.knowledge.service import (
    MAX_BASES_PER_OWNER,
    MAX_DOCS_PER_KB,
    MAX_DOCUMENT_BYTES,
    KnowledgeService,
...
def _knowledge_service(server: ArtemisServer) -> KnowledgeService:
    if server.services is None:
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "knowledge services are not initialized")
...
def _owner_fields(server: ArtemisServer, owner_user_id: int) -> dict[str, str | None]:
    if server.services is None:
...
def _base_payload(server: ArtemisServer, row: Any) -> dict[str, Any]:
    payload = asdict(row)
...
def _max_upload_mb(server: ArtemisServer) -> int:
...
def _max_upload_bytes(server: ArtemisServer) -> int:
    config = getattr(getattr(server, "services", None), "config", None)
...
def _map_knowledge_error(
    exc: Exception, *, locale: str, server: ArtemisServer | None = None
) -> ArtemisError:
    if isinstance(exc, ArtemisError):
        return exc
    if isinstance(exc, (LookupError, FileNotFoundError)):
        return ArtemisError.localized(ErrorCode.KNOWLEDGE_NOT_FOUND, locale)
    if isinstance(exc, PermissionError):
        return ArtemisError.localized(ErrorCode.KNOWLEDGE_FORBIDDEN, locale)
    text = str(exc).lower()
    if isinstance(exc, RuntimeError):
        ...
        return ArtemisError.localized(code, locale)
    if "at most 100" in text:
        return ArtemisError.localized(ErrorCode.KNOWLEDGE_DOC_LIMIT, locale)
    ...
    if "at most" in text and "knowledge bases" in text:
        return ArtemisError.localized(ErrorCode.KNOWLEDGE_BASE_LIMIT, locale)
    if "unsupported knowledge document content type" in text:
        return ArtemisError.localized(ErrorCode.KNOWLEDGE_UNSUPPORTED_TYPE, locale)
    if "knowledge ocr" in text:
        return ArtemisError.localized(ErrorCode.KNOWLEDGE_PREREQUISITES_FAILED, locale)
    if "unique constraint" in text or "duplicate key" in text or "already exists" in text:
        return ArtemisError.localized(ErrorCode.KNOWLEDGE_NAME_TAKEN, locale)
    if "invalid knowledge document name" in text:
        return ArtemisError.localized(ErrorCode.KNOWLEDGE_NAME_INVALID, locale)
    return ArtemisError.localized(ErrorCode.KNOWLEDGE_PREREQUISITES_FAILED, locale)
...
def _enable_onnx_service(server: ArtemisServer, model: str) -> None:
...
def _require_usable(server: ArtemisServer, request: Request) -> None:
    assert server.services is not None
    try:
...
def _capability_payload(server: ArtemisServer) -> dict[str, Any]:
    assert server.services is not None
