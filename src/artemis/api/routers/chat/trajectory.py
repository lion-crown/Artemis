...
from artemis.api.routers.chat.history import _require_thread
...
from artemis.infra.trajectory.settings import TRAJECTORY_SSE_REPLAY_MAX
from artemis.infra.trajectory.types import TrajectoryEvent
...
def _trajectory_service(server: Any) -> TrajectoryService:
    ...
    if not isinstance(service, TrajectoryService):
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "trajectory service unavailable")
    return service
...
async def get_trajectory_event(
    ...
    if event is None or event.thread_id != thread_id or event.agent_id != agent_id:
        raise ArtemisError(ErrorCode.NOT_FOUND, f"event {event_id!r} not found")
