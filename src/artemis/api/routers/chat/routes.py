...
from artemis.api.common.agent import assert_agent_access
from artemis.api.deps import current_user, get_server
from artemis.api.routers.chat.models import HitlResumeBody, PolishBody
from artemis.api.routers.chat.sse import format_sse
...
from artemis.infra.agents.profile import welcome_from_row
...
from artemis.infra.gateway.hitl.store import HitlPendingRecord
from artemis.infra.utils.llm_text import ainvoke_text
...
async def get_chat_welcome(
    ...
    2. Agent workspace ``.artemis/manifest.json`` (seeded at create; quick cards + fallback copy).
...
async def polish_prompt(
    ...
    if not draft:
        raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "text is required")
    ...
    try:
        ...
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "polish request timed out") from None
    except Exception as exc:
    ...
    if not polished:
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "model returned empty polish result")
