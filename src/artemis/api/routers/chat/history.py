...
from artemis.api.common.agent import require_agent_row
...
from artemis.api.routers.chat.models import ForkThreadBody, RebindSessionBody, RenameThreadBody
from artemis.api.routers.chat.serialize import (
    HISTORY_DEFAULT_LIMIT,
    _backfill_thread_projection,
    _clamp_history_limit,
    _load_projected_thread_messages,
)
from artemis.infra.agents.context_breakdown import SEGMENT_KEYS, compute_context_breakdown
from artemis.infra.agents.middleware.thread_artifacts import artifacts_for_response
from artemis.infra.agents.thread_fork import fork_dashboard_thread
from artemis.infra.agents.workspace_dir import agent_facing_workspace_dir_from_config
from artemis.infra.errors import ErrorCode, ArtemisError
from artemis.infra.gateway.hitl.coordinator import pending_hitl_payload
from artemis.infra.gateway.threads import ThreadRegistry, thread_row_has_messages
...
def _require_thread(
    ...
    if row.user_id != effective_uid:
        raise ArtemisError(ErrorCode.FORBIDDEN, "thread not owned by user")
    return row
...
async def get_thread_history(
    ...
    from artemis.api.routers.chat.serialize import (  # noqa: PLC0415
        _enrich_history_tool_media,
        _serialize_history_message,
    )
    from artemis.infra.history.reader import read_page  # noqa: PLC0415
...
async def export_history(
    ...
    from artemis.infra.history.reader import export_messages  # noqa: PLC0415

    _require_thread(server, agent_id, thread_id, user, as_user)
    archive = getattr(server.app_runtime, "history_archive", None)
    if not isinstance(archive, HistoryArchive):
        raise ArtemisError(ErrorCode.NOT_FOUND, "Versioned history is not enabled")
