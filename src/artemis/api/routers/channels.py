...
from artemis.api.common.agent import require_agent_owner_row
from artemis.api.deps import get_server, require_permission
from artemis.infra.errors import ErrorCode, ArtemisError
from artemis.infra.gateway.bot_creators.feishu_runner import extract_feishu_credentials
from artemis.infra.gateway.channels import dingtalk_registration, qr_bind
from artemis.infra.gateway.gateway import ChannelKind
from artemis.infra.utils.locale import DEFAULT_LOCALE, resolve_request_locale
from artemis.infra.utils.subprocess_io import parse_subprocess_json_lines
...
async def create_channel(
    ...
    _require_agent_access(agent_id, user=user, as_user=as_user, server=server)
    from artemis.infra.gateway.gateway import ChannelCreateSpec  # noqa: PLC0415
    from artemis.infra.utils.ulid import new_ulid as _new_ulid  # noqa: PLC0415
...
_FEISHU_PROFILE_NAMES = {
    "feishu": "artemis-feishu-bot",
    "lark": "artemis-lark-bot",
}
...
def _resolve_profiles_root() -> _FsPath:
    ...
    default = _FsPath(artemis_browser_profiles_dir())
