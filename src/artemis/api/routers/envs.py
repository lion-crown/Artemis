"""Environment variables API — backed by ``~/.artemis/env`` (inherited by all agents)."""
...
from artemis.infra.utils.env_file import (
...
@router.put(
    ...
    description=(
        "Overwrite ~/.artemis/env and align the Artemis process environment "
...
async def batch_save_envs(
    ...
    for key, value in body.items():
        k = key.strip()
        if not k:
            raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, "env key cannot be empty")
        if not _KEY_RE.match(k):
            raise ArtemisError(ErrorCode.SLASH_BAD_ARGS, f"invalid env key: {k!r}")
        cleaned[k] = str(value)
    path = env_file_path(server.paths.root)
...
@router.delete(
    ...
    description="Remove a key from ~/.artemis/env and the process environment.",
)
