...
from artemis.infra.users.permissions import PERMISSIONS
...
async def patch_user(
    ...
    if body.role is not None:
        if user_id == actor.id and Role(body.role) is not Role.ADMIN:
            raise ArtemisError(ErrorCode.FORBIDDEN, "cannot demote yourself")
...
async def delete_user(
    ...
    if user_id == actor.id:
        raise ArtemisError(ErrorCode.FORBIDDEN, "cannot delete yourself")
    row = server.user_manager.get_row(user_id)
    if row is None:
        raise ArtemisError(ErrorCode.NOT_FOUND, "user not found")
