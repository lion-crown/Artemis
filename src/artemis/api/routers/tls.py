...
from artemis.infra.setup.tls.manager import get_tls_manager
from artemis.infra.setup.tls.modes import is_issue_eligible, validate_issue_domain
from artemis.infra.setup.tls.preflight import PreflightResult
...
async def tls_issue(
    ...
    if not is_issue_eligible(cfg):
        raise ArtemisError.localized(ErrorCode.TLS_NOT_ELIGIBLE, locale)

    try:
        validate_issue_domain(body.domain, cfg)
    except ValueError:
        raise ArtemisError.localized(ErrorCode.TLS_DOMAIN_MISMATCH, locale) from None
    ...
    try:
        ...
        if "already in progress" in str(exc):
            raise ArtemisError.localized(ErrorCode.TLS_ISSUE_IN_PROGRESS, locale) from exc
