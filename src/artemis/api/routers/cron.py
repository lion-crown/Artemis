...
from artemis.api.common.agent import assert_agent_access, require_agent_row, user_owns_agent
...
from artemis.infra.cron.task_type import (
...
from artemis.infra.utils.ulid import new_cron_id
...
def _assert_cron_manage(
    *, agent_id: str, agent_row: Any, user: Any, row: Any | None = None
    ...
    if not _user_may_manage_agent_cron(agent_row=agent_row, user=user):
        raise ArtemisError(ErrorCode.FORBIDDEN, "cron job not accessible to user")
...
async def cron_examples(
    ...
    """Return ``task_examples`` from the agent workspace ``.artemis/manifest.json``.
...
async def create_cron(
    ...
    from artemis.api.common.validators import validate_chat_mcp_servers  # noqa: PLC0415
    from artemis.infra.cron.manager import CronCreateSpec  # noqa: PLC0415
...
async def patch_cron(
    ...
    from artemis.api.common.validators import validate_chat_mcp_servers  # noqa: PLC0415
    from artemis.infra.db.repos._base import UNSET  # noqa: PLC0415
