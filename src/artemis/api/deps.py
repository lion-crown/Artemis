...
from artemis.infra.users.permissions import PERMISSIONS, user_has_permission
...
ACCESS_TOKEN_RESPONSE_HEADER = "X-Artemis-Access-Token"
...
def get_server(request: Request) -> ArtemisServer:
    server: ArtemisServer = request.app.state.artemis_server
    if not getattr(server, "_started", False):
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "server not started")
    return server


def require_database(server: ArtemisServer) -> None:
...
def _decode(server: ArtemisServer, token: str) -> dict[str, Any]:
    assert server.services is not None
    secret = server.services.secret_repo.get("jwt")
    if secret is None:
        raise ArtemisError(ErrorCode.INTERNAL_ERROR, "jwt secret missing")
    try:
        return decode_token(secret, token)
    except TokenExpired as exc:
        raise ArtemisError(ErrorCode.TOKEN_EXPIRED, "token expired") from exc
    except InvalidToken as exc:
        raise ArtemisError(ErrorCode.AUTH_FAILED, "invalid token") from exc


def resolve_user_from_token(server: ArtemisServer, token: str) -> User:
    payload = _decode(server, token)
    assert server.user_manager is not None
    user = server.user_manager.get_by_id(int(payload["sub"]))
    if user is None:
        raise ArtemisError(ErrorCode.USER_DISABLED, "user not active")
    return user


def maybe_sliding_renew_token(server: ArtemisServer, token: str, user: User) -> str | None:
...
def authenticate_request(request: Request, server: ArtemisServer) -> User:
    raw = extract_raw_token(
        authorization=request.headers.get("authorization"),
        access_token=request.query_params.get("access_token"),
    )
    if not raw:
        raise ArtemisError(ErrorCode.AUTH_FAILED, "missing credentials")
...
async def current_user(
    ...
    cached = getattr(request.state, "artemis_user", None)
...
def require_admin() -> Callable[..., Awaitable[User]]:
    ...
    async def _dep(user: User = Depends(current_user)) -> User:
        if not user.is_admin:
            raise ArtemisError(ErrorCode.FORBIDDEN, "admin required")
        return user
