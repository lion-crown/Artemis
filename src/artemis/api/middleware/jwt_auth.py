...
Validated users are cached on ``request.state.artemis_user`` so route-level
...
attached as ``X-Artemis-Access-Token`` on the response.
"""
...
_INSTALL_ATTR = "_artemis_jwt_auth_installed"
...
def install(app: Any, server: Any) -> None:
    ...
    async def _jwt_auth(
        ...
        try:
            request.state.artemis_user = authenticate_request(request, server)
        ...
        if raw is not None:
            try:
                renewed = maybe_sliding_renew_token(server, raw, request.state.artemis_user)
            except ArtemisError:
                renewed = None
            if renewed:
