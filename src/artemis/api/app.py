...
from artemis.api.middleware.jwt_auth import install as install_jwt_auth
from artemis.api.middleware.setup_lockdown import install as install_setup_lockdown
from artemis.api.openapi_meta import API_DESCRIPTION, OPENAPI_TAGS, configure_openapi
...
def _install_exception_handlers(app: FastAPI) -> None:
    @app.exception_handler(ArtemisError)
    async def _artemis(request: Request, exc: ArtemisError) -> JSONResponse:
        # Client responses are locale-localized; log the original English message
        # so INTERNAL_ERROR (and other 5xx) remain diagnosable in ~/.artemis/logs.
        if exc.status >= 500:
            logger.error(
                "ArtemisError %s in %s: %s",
                exc.code.value,
                request.url.path,
                exc.message,
                exc_info=exc,
    ...
    async def _unhandled(request: Request, exc: Exception) -> JSONResponse:
        ...
        locale = resolve_request_locale(request)
        err = ArtemisError.localized(ErrorCode.INTERNAL_ERROR, locale)
        return JSONResponse(status_code=err.status, content=err.to_envelope())


def build_app(server: ArtemisServer) -> FastAPI:
    ...
    app.state.artemis_server = server
    _install_exception_handlers(app)

    if cfg and cfg.cors_origins:
        from artemis.api.deps import ACCESS_TOKEN_RESPONSE_HEADER
    ...
    from artemis.api.routers import (
        acp,
        admin,
        agent_files,
        agent_tools,
    ...
    from artemis.api.routers.filesystem import router as filesystem_router
    from artemis.api.routers.observability import router as observability_router
    from artemis.api.routers.providers import admin_router as admin_providers_router
    from artemis.api.routers.security import router as security_router
    from artemis.api.routers.storage_backends import admin_router as admin_storage_router
    from artemis.api.routers.storage_backends import user_router as storage_backends_user_router
    from artemis.api.routers.tls import router as tls_router
    from artemis.api.routers.voice import admin_router as admin_voice_router
    ...
    _mount_routers(
        ...
        [
            ...
            _RouterMount(service_control.router, "/api/admin", ["admin"]),
