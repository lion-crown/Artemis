"""Composition root — wire ArtemisServer, FastAPI, and uvicorn for ``artemis run``."""
...
def _ensure_linux_bubblewrap() -> None:
    """Best-effort startup provisioning; never fail the ``artemis run`` process."""
    try:
...
async def run_foreground(
    *,
    ...
    from artemis.infra.setup.tls.http_companion import build_http_companion_app
