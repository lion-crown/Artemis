...
from artemis.infra.utils.paths import PRIVATE_FILE_MODE, restrict_to_owner
...
_DATABASE_ENV_KEYS = (
    "ARTEMIS_DATABASE_URL",
    "ARTEMIS_DATABASE_DRIVER",
    "ARTEMIS_DATABASE_SQLITE_PATH",
    "ARTEMIS_DATABASE_HOST",
    "ARTEMIS_DATABASE_PORT",
    "ARTEMIS_DATABASE_NAME",
    "ARTEMIS_DATABASE_USER",
    "ARTEMIS_DATABASE_PASSWORD",
)


def database_env_configured() -> bool:
    """True when any ``ARTEMIS_DATABASE_*`` env var is set."""
...
class DatabaseConfig:
    ...
    driver: str = "sqlite"
    sqlite_path: str = "artemis.db"
    host: str = "127.0.0.1"
    port: int = 5432
    database: str = "artemis"
    user: str = "artemis"
    password: str | None = None
    url: str | None = None  # verbatim ARTEMIS_DATABASE_URL when set
    ...
    def resolve_sqlite_path(self, artemis_root: Path) -> Path:
        """Absolute path to the SQLite file (relative paths are under ``artemis_root``)."""
        p = Path(self.sqlite_path)
        return p if p.is_absolute() else artemis_root / p
...
def _coerce_int(name: str, value: str, default: int) -> int:
    try:
        ...
        # Do not log the raw env value — names like ARTEMIS_*_PASSWORD can flow here.
...
def _coerce_bool(name: str, value: str, default: bool) -> bool:
    ...
    if normalized in {"0", "false", "no", "off"}:
        return False
    # Do not log the raw env value — ARTEMIS_REQUIRE_SETUP_PASSWORD is classified as
...
def _parse_database_url(url: str) -> dict[str, Any]:
    ...
    if parsed.scheme not in ("postgresql", "postgres"):
        msg = f"ARTEMIS_DATABASE_URL scheme must be postgresql, got {parsed.scheme!r}"
        raise ValueError(msg)
    if not parsed.hostname:
        raise ValueError("ARTEMIS_DATABASE_URL missing host")
    if not parsed.path or parsed.path == "/":
        raise ValueError("ARTEMIS_DATABASE_URL missing database name in path")
    out: dict[str, Any] = {
        "driver": "postgresql",
...
def parse_database_config(merged: dict[str, Any]) -> DatabaseConfig:
    ...
    if driver == "sqlite":
        sqlite_path = str(merged.get("sqlite_path", "artemis.db")).strip()
        if not sqlite_path:
...
def _apply_database_env(merged_db: dict[str, Any]) -> dict[str, Any]:
    out = dict(merged_db)
    if v := os.environ.get("ARTEMIS_DATABASE_URL"):
        out.update(_parse_database_url(v))
    if v := os.environ.get("ARTEMIS_DATABASE_DRIVER"):
        out["driver"] = v.strip().lower()
    if v := os.environ.get("ARTEMIS_DATABASE_SQLITE_PATH"):
        out["sqlite_path"] = v
    if v := os.environ.get("ARTEMIS_DATABASE_HOST"):
        out["host"] = v
    if v := os.environ.get("ARTEMIS_DATABASE_PORT"):
        out["port"] = _coerce_int("ARTEMIS_DATABASE_PORT", v, int(out.get("port", 5432)))
    if v := os.environ.get("ARTEMIS_DATABASE_NAME"):
        out["database"] = v
    if v := os.environ.get("ARTEMIS_DATABASE_USER"):
        out["user"] = v
    if v := os.environ.get("ARTEMIS_DATABASE_PASSWORD"):
        out["password"] = v
    return out


def env_bind_overrides() -> tuple[str | None, int | None]:
    """``ARTEMIS_BIND_HOST`` / ``ARTEMIS_PORT`` overrides shared by config and CLI.

    An invalid ``ARTEMIS_PORT`` logs a warning and yields no override, so the
    file/default value wins — same fallback semantics as ``load_config``.
    """
    host = os.environ.get("ARTEMIS_BIND_HOST") or None
    port: int | None = None
    if v := os.environ.get("ARTEMIS_PORT"):
        try:
            port = int(v)
        except ValueError:
            # Do not log the raw env value.
            logger.warning("env %s is not int; ignoring override", "ARTEMIS_PORT")
    return host, port


def load_config(path: Path) -> ArtemisConfig:
    ...
    if v := os.environ.get("ARTEMIS_LOG_LEVEL"):
        merged["log_level"] = v
    if v := os.environ.get("ARTEMIS_ACCESS_TOKEN_TTL"):
        merged["access_token_ttl_seconds"] = _coerce_int(
            "ARTEMIS_ACCESS_TOKEN_TTL", v, merged["access_token_ttl_seconds"]
        )
    if v := os.environ.get("ARTEMIS_LOGIN_MAX_ATTEMPTS"):
        merged["login_max_attempts"] = _coerce_int(
            "ARTEMIS_LOGIN_MAX_ATTEMPTS", v, merged.get("login_max_attempts", 5)
        )
    if v := os.environ.get("ARTEMIS_LOGIN_LOCKOUT_SECONDS"):
        merged["login_lockout_seconds"] = _coerce_int(
            "ARTEMIS_LOGIN_LOCKOUT_SECONDS", v, merged.get("login_lockout_seconds", 900)
        )
    if (v := os.environ.get("ARTEMIS_DEFAULT_TIMEZONE")) or (
        v := os.environ.get("ARTEMIS_CRON_TIMEZONE")
    ):
        merged["default_timezone"] = v
    if v := os.environ.get("ARTEMIS_CORS_ORIGINS"):
        merged["cors_origins"] = [s.strip() for s in v.split(",") if s.strip()]
    if v := os.environ.get("ARTEMIS_ENABLE_DASHBOARD"):
        merged["enable_dashboard"] = _coerce_bool(
            "ARTEMIS_ENABLE_DASHBOARD", v, bool(merged["enable_dashboard"])
        )
    if v := os.environ.get("ARTEMIS_ENABLE_API_DOCS"):
        merged["enable_api_docs"] = _coerce_bool(
            "ARTEMIS_ENABLE_API_DOCS", v, bool(merged["enable_api_docs"])
        )
    if v := os.environ.get("ARTEMIS_REQUIRE_SETUP_PASSWORD"):
        merged["require_setup_password"] = _coerce_bool(
            "ARTEMIS_REQUIRE_SETUP_PASSWORD", v, bool(merged["require_setup_password"])
        )
    if v := os.environ.get("ARTEMIS_MAX_UPLOAD_MB"):
    ...
    if v := os.environ.get("ARTEMIS_BROWSER_IDLE_TIMEOUT_MINUTES"):
        merged["browser_idle_timeout_minutes"] = _coerce_int(
            "ARTEMIS_BROWSER_IDLE_TIMEOUT_MINUTES",
            v,
            int(
                merged.get(
    ...
    if v := os.environ.get("ARTEMIS_ENABLE_MOBILE"):
        forced = _coerce_bool("ARTEMIS_ENABLE_MOBILE", v, capabilities.mobile.enabled)
    ...
    if v := os.environ.get("ARTEMIS_BACKUP_AUTO_ENABLED"):
        backup = replace(
            backup,
            auto_enabled=_coerce_bool("ARTEMIS_BACKUP_AUTO_ENABLED", v, backup.auto_enabled),
        )
    if v := os.environ.get("ARTEMIS_BACKUP_SCHEDULE"):
        schedule = v.strip() or backup.schedule
        backup = replace(backup, schedule=schedule)
    if v := os.environ.get("ARTEMIS_BACKUP_RETENTION_COUNT"):
        retention = _coerce_int("ARTEMIS_BACKUP_RETENTION_COUNT", v, backup.retention_count)
        if retention < 1:
            logger.warning(
                "env ARTEMIS_BACKUP_RETENTION_COUNT is < 1; using %s",
                backup.retention_count,
            )
            retention = backup.retention_count
        backup = replace(backup, retention_count=retention)
    if v := os.environ.get("ARTEMIS_BACKUP_INCLUDE_CONFIG"):
        backup = replace(
            backup,
            include_config=_coerce_bool("ARTEMIS_BACKUP_INCLUDE_CONFIG", v, backup.include_config),
        )
    if v := os.environ.get("ARTEMIS_BACKUP_INCLUDE_WORKSPACES"):
        backup = replace(
            backup,
            include_workspaces=_coerce_bool(
                "ARTEMIS_BACKUP_INCLUDE_WORKSPACES", v, backup.include_workspaces
            ),
        )
    if v := os.environ.get("ARTEMIS_BACKUP_INCLUDE_SKILL_PACKAGES"):
        backup = replace(
            backup,
            include_skill_packages=_coerce_bool(
                "ARTEMIS_BACKUP_INCLUDE_SKILL_PACKAGES",
                v,
                backup.include_skill_packages,
            ),
        )
    if v := os.environ.get("ARTEMIS_BACKUP_INCLUDE_PLUGINS"):
        backup = replace(
            backup,
            include_plugins=_coerce_bool(
                "ARTEMIS_BACKUP_INCLUDE_PLUGINS", v, backup.include_plugins
            ),
        )
    if v := os.environ.get("ARTEMIS_BACKUP_INCLUDE_KNOWLEDGE"):
        backup = replace(
            backup,
            include_knowledge=_coerce_bool(
                "ARTEMIS_BACKUP_INCLUDE_KNOWLEDGE", v, backup.include_knowledge
            ),
        )
    if v := os.environ.get("ARTEMIS_BACKUP_INCLUDE_CHATS"):
        backup = replace(
            backup,
            include_chats=_coerce_bool("ARTEMIS_BACKUP_INCLUDE_CHATS", v, backup.include_chats),
        )
    ...
    return ArtemisConfig(
        ...
        history_v2_enabled=_coerce_bool(
            "ARTEMIS_HISTORY_V2_ENABLED",
            os.environ.get(
                "ARTEMIS_HISTORY_V2_ENABLED", str(merged.get("history_v2_enabled", False))
            ),
            False,
        ),
