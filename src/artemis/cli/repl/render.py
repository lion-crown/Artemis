...
class ChunkRenderer:
    ...
    def _handle_tool(self, chunk: dict[str, Any]) -> None:
        ...
        from artemis.i18n import tool_display_name
...
def print_slash_help(*, locale: str = "zh") -> None:
    ...
    from artemis.infra.gateway.slash.catalog import list_specs
...
def print_welcome(*, agent_id: str, model: str, session_key: str, thread_id: str | None) -> None:
    ...
    console = Console()
    try:
        ver = pkg_version("artemis")
    except Exception:
        ver = "unknown"
    console.print(f"  [bold green]Artemis[/] [dim]v{ver}[/] — interactive chat")
