"""``artemis plugin`` — install and manage plugins."""
...
@click.group()
def plugin() -> None:
    """Install and manage Artemis plugins."""
...
def install_plugin(source: str, force: bool) -> None:
    ...
    click.echo(
        "Note: if artemis run is already running, call POST /api/plugins/reload "
...
def reload_plugins() -> None:
    """Reload plugins from disk (offline; restart ``artemis run`` to apply to agents)."""
    loaded = _manager().load_installed(install_deps=False)
    click.echo(f"Reloaded {len(loaded)} plugin(s). Restart artemis run if the server is running.")
