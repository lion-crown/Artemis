"""artemis channel commands (local DB + embedded runtime)."""
...
from artemis.cli.support.ctx import require_agent
...
def list_channels(as_user: str | None, agent_id: str | None) -> None:
    ...
    from artemis.cli.support.offline_ops import list_channels_offline
...
def get_channel(agent_id: str | None, channel_id: str, as_user: str | None) -> None:
    ...
    from artemis.cli.support.qr import mask_secret

    aid = require_agent(agent_id)
    _resolve_user(aid, as_user)
    from artemis.cli.support.offline_ops import get_channel_offline
    from artemis.infra.errors import ArtemisError
...
def create(
    ...
    from artemis.cli.support.offline_ops import create_channel_offline
    from artemis.infra.errors import ArtemisError
...
def patch_channel(
    ...
    from artemis.cli.support.offline_ops import patch_channel_offline
    from artemis.infra.errors import ArtemisError
...
def delete(agent_id: str | None, channel_id: str, as_user: str | None) -> None:
    ...
    aid = require_agent(agent_id)
    _resolve_user(aid, as_user)
    from artemis.cli.support.offline_ops import delete_channel_offline
    from artemis.infra.errors import ArtemisError
...
def test(agent_id: str | None, channel_id: str, as_user: str | None) -> None:
    ...
    from artemis.cli.support.db import resolve_cli_locale
    from artemis.cli.support.embedded_ops import test_channel
    from artemis.infra.errors import ArtemisError
...
def bind_qq(agent_id: str | None, channel_id: str | None, as_user: str | None) -> None:
    ...
    from artemis.cli.support.offline_ops import (
        create_channel_offline,
        get_channel_offline,
        patch_channel_offline,
    )
    from artemis.cli.support.qr import render_qrcode_terminal
    from artemis.infra.gateway.channels import qr_bind
...
def bind_wecom(agent_id: str | None, channel_id: str | None, as_user: str | None) -> None:
    ...
    from artemis.cli.support.offline_ops import create_channel_offline, patch_channel_offline
    from artemis.cli.support.qr import render_qrcode_terminal
    from artemis.infra.gateway.channels import qr_bind
...
def config_channels(agent_id: str | None, as_user: str | None) -> None:
    ...
    kinds = ["feishu", "wecom", "weixin", "qq", "dingtalk", "mqtt"]
...
def feishu_setup(
    ...
    from artemis.cli.support.feishu_creator import dry_run_feishu_setup, run_feishu_bot_creator
    from artemis.cli.support.offline_ops import create_channel_offline, patch_channel_offline
    from artemis.infra.errors import ArtemisError
