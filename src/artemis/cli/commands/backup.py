"""`artemis backup` — export and restore Artemis data."""
...
from artemis.infra.backup.auto import create_and_store_auto_backup
from artemis.infra.backup.store import place_backup_file
...
@click.option(
    "--home", type=click.Path(path_type=Path), default=None, help="Artemis home (default ~/.artemis)."
)
...
@backup.group("auto")
def auto() -> None:
    """Automatic backup status and one-shot run (schedule requires ``artemis run``)."""
...
def auto_status(home: Path | None) -> None:
    ...
    click.echo("note: the scheduler only runs inside an active `artemis run` process")
...
@click.option(
    ...
    type=int,
    default=None,
    help="For LightClaw migration archives: Artemis user id that receives imported ownership "
...
def restore(
    ...
    """Restore from a backup archive. Stop ``artemis run`` first for a clean restore."""
    if not yes:
        click.confirm(
