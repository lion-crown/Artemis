"""`artemis clean` — remove CLI state and (optionally) all Artemis data."""
...
@click.command("clean")
@click.option("--all", "wipe_all", is_flag=True, default=False, help="Wipe ALL of ~/.artemis.")
...
def clean(wipe_all: bool, yes: bool, dry_run: bool) -> None:
    ...
    if wipe_all and not yes:
        click.confirm("This deletes ALL Artemis data (DB, configs, logs). Continue?", abort=True)
    elif not yes:
