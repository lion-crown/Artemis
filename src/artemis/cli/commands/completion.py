"""artemis completion: emit / install shell completion script."""
...
_EVAL_TEMPLATE = {
    "bash": 'eval "$(_ARTEMIS_COMPLETE=bash_source artemis)"\n',
    "zsh": 'autoload -U compinit && compinit\neval "$(_ARTEMIS_COMPLETE=zsh_source artemis)"\n',
}
...
def install(shell: str | None, rc_file: str | None) -> None:
    ...
    if "_ARTEMIS_COMPLETE" in existing:
        click.echo(f"already installed in {rc_path}")
        return
    with rc_path.open("a", encoding="utf-8") as f:
        f.write("\n# artemis shell completion\n")
        f.write(snippet)
