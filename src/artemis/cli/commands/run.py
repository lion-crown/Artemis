"""`artemis run` — start the FastAPI app in the foreground."""
...
from artemis.config import env_bind_overrides
...
def _run_uvicorn(**kwargs: object) -> None:
    ...
    from artemis.launch import run_foreground_blocking
...
def _maybe_generate_self_signed(
    ...
    if not cert.exists() or not key.exists():
        ...
        subject = issuer = x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, "artemis-self-signed")])
        cert_obj = (
...
def run(
    ...
    """Run Artemis in the foreground.

    Host and port are resolved with the following precedence: explicit CLI
    flags > ``ARTEMIS_BIND_HOST``/``ARTEMIS_PORT`` env > ``~/.artemis/config.json``
