...
def create(
    ...
    from artemis.cli.repl.embedded_session import embedded_runtime
...
def from_expert(expert_id: str, name: str | None, as_user: str | None) -> None:
    ...
    from artemis.cli.repl.embedded_session import embedded_runtime
    from artemis.cli.support.acting import resolve_cli_acting_user_id
    from artemis.infra.agents.experts.catalog import build_create_spec_from_expert
...
def list_agents(as_user: str | None) -> None:
    ...
    from artemis.cli.support.db import list_agents_offline
...
def use_agent(agent_id: str) -> None:
    """Pin default agent in CLI state (~/.artemis/cli_state.json)."""
    from artemis.cli.support.state import default_state_path, load, save
...
def _act(action: str, agent_id: str, as_user: str | None) -> None:
    from artemis.cli.support.acting import resolve_cli_acting_user_id
    from artemis.cli.support.embedded_ops import agent_action
    ...
    try:
        ...
        fail_artemis(exc)
    if action == "delete":
        from artemis.cli.support.offline_ops import delete_agent_offline
...
def list_experts() -> None:
    ...
    from artemis.cli.support.ctx import json_output_enabled
    from artemis.cli.support.offline_ops import list_experts_offline
