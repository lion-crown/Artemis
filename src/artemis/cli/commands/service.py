"""`artemis service` — install and manage the Artemis system service."""
...
def _health_failure_hint(runtime: ServiceRuntime) -> str:
    ...
    if runtime.mode == "systemd":
        journal = (
            "journalctl --user -u artemis -n 80 --no-pager"
            if runtime.scope == "user"
            else "journalctl -u artemis -n 80 --no-pager"
        )
...
@click.option(
    ...
    show_default=True,
    help=(
        "Install scope: root → system unit (/etc/systemd/system); "
