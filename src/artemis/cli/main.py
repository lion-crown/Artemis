"""Artemis CLI entry point with lazy command loading."""
...
from artemis.cli.registry import COMMANDS
...
def _ensure_utf8_stdio(
    ...
    (e.g. the success checkmark printed by ``artemis init``) raises
...
def _print_version(ctx: click.Context, _param: click.Parameter, value: bool) -> None:
    if not value or ctx.resilient_parsing:
        return
    click.echo(f"artemis v{_get_version()}")
    ctx.exit()
...
@click.option(
    ...
    help="Show the installed Artemis version.",
)
@click.option(
    "--user",
    "as_user",
