...
from artemis.cli.support.db import (
    open_cli_services,
    resolve_username,
)
from artemis.infra.agents.experts.catalog import ExpertCatalog, default_library_root
from artemis.infra.agents.providers.model_flags import is_local_runtime_provider
from artemis.infra.cron.task_type import normalize_cron_task_type, require_cron_prompt
from artemis.infra.cron.trigger import build_trigger
from artemis.infra.errors import ErrorCode, ArtemisError
from artemis.infra.gateway.threads import ThreadRegistry
from artemis.infra.users.password import hash_password
from artemis.infra.utils.ulid import new_cron_id, new_ulid
...
def _require_username(services: Any, username: str) -> int:
    try:
        ...
        raise ArtemisError(ErrorCode.NOT_FOUND, str(exc)) from exc
...
def create_user_offline(
    *,
    ...
    from artemis.infra.users.email import parse_optional_email

    normalized_email = parse_optional_email(email)
    with open_cli_services(home) as svc:
        if svc.user_repo.get_by_username(username) is not None:
            raise ArtemisError(ErrorCode.USERNAME_TAKEN, f"username {username!r} already exists")
        if (
            normalized_email is not None
            and svc.user_repo.get_by_email(normalized_email) is not None
        ):
            raise ArtemisError(ErrorCode.EMAIL_TAKEN, f"email {normalized_email!r} already exists")
...
def delete_cron_offline(agent_id: str, cron_id: str, *, home: Path | None = None) -> None:
    with open_cli_services(home) as svc:
        ...
        if row is None or row.agent_id != agent_id:
            raise ArtemisError(ErrorCode.NOT_FOUND, "cron job not found")
        svc.cron_repo.delete(cron_id)


def resolve_cron_user_id(agent_id: str, as_user: str | None, *, home: Path | None = None) -> int:
    from artemis.cli.support.acting import resolve_cli_acting_user_id
...
def delete_agent_offline(agent_id: str, *, home: Path | None = None) -> None:
    from artemis.infra.agents.workspace_dir import workspace_dir_from_config_json
    ...
    *,
    ...
    with open_cli_services(home) as svc:
        if svc.provider_repo.get_by_name(name) is not None:
            raise ArtemisError(ErrorCode.PROVIDER_NAME_TAKEN, f"provider {name!r} already exists")
...
def delete_provider_offline(provider_id: int, *, home: Path | None = None) -> None:
    with open_cli_services(home) as svc:
        ...
        if row is None:
            raise ArtemisError(ErrorCode.NOT_FOUND, "provider not found")
...
def list_resolved_models_offline(*, home: Path | None = None) -> list[dict[str, Any]]:
    from artemis.infra.agents.providers.resolved import list_resolved_models
...
def get_channel_offline(
    ...
    with open_cli_services(home) as svc:
        ...
        if row is None or row.agent_id != agent_id:
            raise ArtemisError(ErrorCode.NOT_FOUND, f"channel {channel_id!r} not found")
...
def update_thread_offline(
    ...
    *,
    ...
    with open_cli_services(home) as svc:
        ...
        if row is None or row.agent_id != agent_id:
            raise ArtemisError(ErrorCode.AGENT_NOT_FOUND, f"thread {thread_id!r} not found")
