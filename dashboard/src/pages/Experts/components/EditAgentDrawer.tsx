...
import { artemisAgentsApi } from "../../../api/modules/artemisAgents";
import { useAgent, type ArtemisAgent } from "../../../context/AgentContext";
...
interface EditFormValues {
  ...
  top_p?: number;
  max_tokens?: number;
  execution_profile?: "standard" | "lean_code_qa" | "business_system_qa";
  business_reply_guidance?: string;
...
/**
 * Merge page-config fields into the existing workspace .artemis/manifest.json.
    ...
    const load = async () => {
      try {
        ...
        form.setFieldsValue({
          ...
          execution_profile:
            cfg.execution_profile === "business_system_qa"
              ? "business_system_qa"
              : cfg.execution_profile === "lean_code_qa"
              ? "lean_code_qa"
              : "standard",
          business_reply_guidance:
            typeof cfg.business_reply_guidance === "string"
              ? cfg.business_reply_guidance
              : "",
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const nextConfig = omitAgentRuntimeConfig({
        ...agentConfig,
        backend: backendSpec,
        execution_profile: values.execution_profile ?? "standard",
        enable_trajectory: values.enable_trajectory === true,
      });
      const businessReplyGuidance =
        values.business_reply_guidance?.trim() ?? "";
      if (businessReplyGuidance) {
        nextConfig.business_reply_guidance = businessReplyGuidance;
      } else {
        delete nextConfig.business_reply_guidance;
      }
                ...
                <ExpertAvatarPicker
                  ...
                  onPick={(file) => {
                    void (async () => {
                      setAvatarBusy(true);
                      try {
                        const result = await artemisAgentsApi.uploadAvatar(
                          agent.agent_id,
                          file,
                        );
                  ...
                  onRemove={() => {
                    void (async () => {
                      setAvatarBusy(true);
                      try {
                        await artemisAgentsApi.deleteAvatar(agent.agent_id);
                        setIconUrl(null);
                        await refresh({ silent: true });
                      } catch (err) {
                        message.error(
                  ...
                  children: (
                    <Form form={form} layout="vertical" size="middle">
                      <Form.Item
                        ...
                        label={t("experts.executionProfile")}
                        tooltip={t("experts.executionProfileHint")}
                      >
                        <Select
                          options={[
                            {
                              value: "standard",
                              label: t("experts.executionProfileStandard"),
                            },
                            {
                              value: "lean_code_qa",
                              label: t("experts.executionProfileLeanCodeQa"),
                            },
                            {
                              value: "business_system_qa",
                              label: t(
                                "experts.executionProfileBusinessSystemQa",
                              ),
                            },
                          ]}
                        />
                      ...
                      <Alert
                        type="info"
                        showIcon
                        message={t("experts.businessReplySystemRules")}
                        description={t(
                          "experts.businessReplySystemRulesDescription",
                        )}
                      />
                      <Form.Item
                        name="business_reply_guidance"
                        label={t("experts.businessReplyGuidance")}
                        tooltip={t("experts.businessReplyGuidanceHint")}
                        extra={t("experts.businessReplyGuidanceExtra")}
                        style={{ marginTop: 16 }}
                      >
                        <Input.TextArea
                          ...
                          maxLength={1000}
                          placeholder={t(
                            "experts.businessReplyGuidancePlaceholder",
                          )}
                          showCount
                        />
                      </Form.Item>
