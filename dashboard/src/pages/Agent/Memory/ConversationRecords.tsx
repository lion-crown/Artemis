...
import {
  ...
  artemisThreadsApi,
  type ArtemisThread,
} from "../../../api/modules/artemisThreads";
import {
  extractText,
...
interface ConversationThreadCardProps {
  thread: ArtemisThread;
  selected: boolean;
  deleting: boolean;
  channelLabel: string;
  timeZone: string;
  onView: (thread: ArtemisThread) => void;
  onDelete: (thread: ArtemisThread) => void;
}
...
export default function ConversationRecords({
  ...
  const [threads, setThreads] = useState<ArtemisThread[]>([]);
  const [listLoading, setListLoading] = useState(false);
  const [selectedThread, setSelectedThread] = useState<ArtemisThread | null>(
    null,
  );
  ...
  const loadThreads = useCallback(async () => {
    setListLoading(true);
    try {
      const list = await artemisThreadsApi.list(agentId, 100);
  ...
  const fetchThreadHistory = useCallback(
    async (threadId: string, offset: number) => {
      const history = await artemisThreadsApi.history(agentId, threadId, {
  ...
  const handleDelete = useCallback(
    async (thread: ArtemisThread) => {
      ...
      try {
        await artemisThreadsApi.delete(agentId, thread.thread_id);
  ...
  const columns: ColumnsType<ArtemisThread> = useMemo(
    () => [
      {
