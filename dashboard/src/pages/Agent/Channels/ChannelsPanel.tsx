...
function configFromFormValues(
  ...
  let config: Record<string, unknown> = {
    ...
    ...(values.kind === "wecom"
      ? { delivery_mode: delivery_mode ?? "stream" }
      : {}),
  };
...
export default function ChannelsPanel({ agentId }: ChannelsPanelProps) {
  ...
  const displayChannelKeys = CHANNEL_KEYS;

  const visibleConfiguredCount = useMemo(
    () => Array.from(channelByKind.keys()).length,
    [channelByKind],
  );
    ...
    async (row: ChannelRow) => {
      ...
      if (detail) {
        ...
        const next: ChannelFormValues = {
          ...
          delivery_mode:
            row.kind === "wecom" && cfg.delivery_mode === "final_only"
              ? "final_only"
              : "stream",
          show_thinking:
      ...
      <div className={styles.channelsToolbar}>
        <span className={styles.channelsStats}>
          {t("channels.statsSummary", {
            supported: CHANNEL_SELECTABLE_KEYS.length,
            configured: visibleConfiguredCount,
          })}
        </span>
        <Button
