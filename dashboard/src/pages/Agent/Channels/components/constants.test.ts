...
describe("channel catalog", () => {
  it("does not include removed YuanBao, XiaoYi, or Telegram channels", () => {
    expect(CHANNEL_KEYS).not.toContain("yuanbao");
    expect(CHANNEL_KEYS).not.toContain("xiaoyi");
    expect(CHANNEL_KEYS).not.toContain("telegram");
    expect(CHANNEL_SELECTABLE_KEYS).toEqual(CHANNEL_KEYS);
  });

  it("does not publish an obsolete credential portal for the legacy bot protocol", () => {
    expect(CHANNEL_URLS.artemisbot).toBeUndefined();
  });
});
