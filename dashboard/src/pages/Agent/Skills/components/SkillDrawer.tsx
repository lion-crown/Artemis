...
export interface SkillFormValues {
  ...
  summaryZh?: string;
  summaryEn?: string;
  /** Surfaced as ``metadata.artemis.emoji`` in SKILL.md. */
  emoji: string;
...
export const ARTEMIS_EMOJI_META_KEY = "artemis.emoji";
const ARTEMIS_LABEL_ZH_META_KEY = "artemis.label.zh";
const ARTEMIS_LABEL_EN_META_KEY = "artemis.label.en";
const ARTEMIS_SUMMARY_ZH_META_KEY = "artemis.summary.zh";
const ARTEMIS_SUMMARY_EN_META_KEY = "artemis.summary.en";
const PRESENTATION_META_KEYS = new Set([
  ARTEMIS_LABEL_ZH_META_KEY,
  ARTEMIS_LABEL_EN_META_KEY,
  ARTEMIS_SUMMARY_ZH_META_KEY,
  ARTEMIS_SUMMARY_EN_META_KEY,
]);
...
/** Split ``artemis.emoji`` out of flattened metadata for the dedicated picker. */
export function parseSkillEmojiAndMetadata(
  ...
  for (const row of pairs ?? []) {
    if (row.key.trim() === ARTEMIS_EMOJI_META_KEY) {
...
function withEmojiMetadata(
  ...
  const rest = (pairs ?? []).filter(
    (row) => row.key.trim() !== ARTEMIS_EMOJI_META_KEY,
  );
  const value = emoji.trim();
  if (!value) return rest;
  return [{ key: ARTEMIS_EMOJI_META_KEY, value }, ...rest];
}
...
function withPresentationMetadata(
  ...
  const presentation = [
    [ARTEMIS_LABEL_ZH_META_KEY, values.labelZh],
    [ARTEMIS_LABEL_EN_META_KEY, values.labelEn],
    [ARTEMIS_SUMMARY_ZH_META_KEY, values.summaryZh],
    [ARTEMIS_SUMMARY_EN_META_KEY, values.summaryEn],
  ]
...
function parseSkillFormFromDetail(detail: SkillDetail): SkillFormValues {
  ...
  return {
    name: displayName,
    description,
    labelZh: presentationValue(ARTEMIS_LABEL_ZH_META_KEY),
    labelEn: presentationValue(ARTEMIS_LABEL_EN_META_KEY),
    summaryZh: presentationValue(ARTEMIS_SUMMARY_ZH_META_KEY),
    summaryEn: presentationValue(ARTEMIS_SUMMARY_EN_META_KEY),
    emoji,
...
const FILE_TREE_COLLAPSED_KEY = "artemis:skill-drawer-tree-collapsed";
