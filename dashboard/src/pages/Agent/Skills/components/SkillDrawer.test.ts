...
describe("SkillDrawer emoji metadata", () => {
  it("writes artemis.emoji into frontmatter from the emoji field", () => {
    ...
    expect(md).toContain("artemis:");
  });

  it("writes localized presentation fields into artemis metadata", () => {
    const md = buildSkillMarkdown({
      name: "demo",
  ...
  it("extracts emoji from flattened metadata and keeps other keys", () => {
    const { emoji, metadata } = parseSkillEmojiAndMetadata([
      { key: ARTEMIS_EMOJI_META_KEY, value: "🔧" },
      { key: "artemis.requires.bins", value: "git" },
    ]);
    expect(emoji).toBe("🔧");
    expect(metadata).toEqual([{ key: "artemis.requires.bins", value: "git" }]);
  });
});
