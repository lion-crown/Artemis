...
/**
 * SkillSpec mirrors the JSON shape returned by Artemis's
 * ``GET /api/agents/{aid}/skills`` (see ``src/artemis/api/routers/skills.py``).
 *
 * Field-name reconciliation: the legacy UI treats the identifier as ``slug``
 * and uses ``kind`` (``builtin`` | ``workspace``) to split built-in vs
 * customised skills. Artemis only returns ``name`` and has no kind concept.
 *
 * To reuse the existing components without renaming everything, this
 * hook exposes ``slug`` (= server's ``name``) and synthesises ``kind``
 * (always ``"workspace"`` for now — every Artemis skill is editable).
