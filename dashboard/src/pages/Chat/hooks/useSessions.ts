...
import { artemisThreadsApi } from "../../../api/modules/artemisThreads";
...
async function fetchSessionsPage(
  ...
  const rows = await artemisThreadsApi.list(agentId, limit + 1);
...
export function useSessions(agentId: string | null) {
  ...
  const createSession = useCallback((): {
    ...
    const resolvedId = artemisThreadsApi
      .create(agentId)
      .then((created) => {
  ...
  const deleteSession = useCallback(
    async (id: string) => {
      ...
      try {
        await artemisThreadsApi.delete(agentId, id);
  ...
  const pinSession = useCallback(
    (id: string, pinned: boolean) => {
      ...
      if (agentId) {
        void artemisThreadsApi.patch(agentId, id, { pinned }).catch(() => {});
      }
    },
    [agentId],
  );
  ...
  const renameSession = useCallback(
    (id: string, name: string) => {
      ...
      if (agentId) {
        void artemisThreadsApi.rename(agentId, id, next).catch(() => {});
      }
    },
    [agentId],
  );
