...
const PANEL_MODE_KEY = "artemis:chat-dock:mode";
const PANEL_SIZE_KEY = "artemis:chat-dock:size";
/** Legacy keys — read once for migration. */
const LEGACY_FILE_MODE_KEY = "artemis:file-panel:mode";
const LEGACY_BROWSER_MODE_KEY = "artemis:browser-panel:mode";
const LEGACY_FILE_SIZE_KEY = "artemis:file-panel:size";
const LEGACY_BROWSER_SIZE_KEY = "artemis:browser-panel:size";
...
export function useChatDockPanel(isMobile: boolean, agentId?: string | null) {
  ...
  const openFileAt = useCallback(
    (path?: string | null) => {
      ...
      // Keep host-absolute tool paths. Collapsing ``~/.artemis/agents/<id>/…`` to a
      // relative key breaks virtual ``root_dir`` nests (bytes live under
      // ``{root}/Users/…/.artemis/agents/<id>/…``, not the real agent home).
