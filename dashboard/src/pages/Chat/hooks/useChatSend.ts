...
export function useChatSend({
  ...
  useEffect(() => {
    const consumePending = () => {
      const raw = localStorage.getItem("artemis.pendingChatMessage");
      if (!raw) return;
      localStorage.removeItem("artemis.pendingChatMessage");
    ...
    window.addEventListener("artemis:pending-chat-message", handler);
    return () =>
      window.removeEventListener("artemis:pending-chat-message", handler);
