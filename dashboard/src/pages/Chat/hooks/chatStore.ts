...
const DRAFT_STORAGE_PREFIX = "artemis:chat-draft:";
...
function extractToolResultOutput(messages: unknown[]): string {
  ...
  for (const raw of messages) {
    ...
    // Already-parsed JSON object (e.g. artemis_ui envelope) — must not drop.
 ...
 * (merges into ``artemis_ui`` JSON ``data`` when present). Searches the focused
