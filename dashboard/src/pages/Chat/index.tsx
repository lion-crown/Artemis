...
function ChatPageInner() {
  ...
  const handleForkAssistantMessage = useCallback(
    async (messageId: string) => {
      ...
      setForking(true);
      try {
        const created = await artemisThreadsApi.fork(agent, activeThreadId, {
  ...
  const handleForkSession = useCallback(
    async (threadId: string, agentId?: string | null) => {
      ...
      setForking(true);
      try {
        const created = await artemisThreadsApi.fork(agent, threadId, {
    ...
    <ChatSidebarPanel
      ...
      onSelectSession={(sessionId, agentId) => {
        ...
        if (agentId && agentId !== resolvedAgentId) {
          void artemisThreadsApi.rebind(agentId, sessionId).catch(() => {});
