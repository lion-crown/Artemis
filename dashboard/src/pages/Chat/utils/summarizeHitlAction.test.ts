...
describe("summarizeHitlAction", () => {
  ...
  it("ignores the English HITL template and explains write_file in Chinese", () => {
    const view = summarizeHitlAction(
      "write_file",
      {
        file_path: "/.artemis/workspaces/J7Y3TW/test.txt",
        content: "Hello, this is a test file!\nCreated at: 2025-01-25\n",
      },
      tZh,
      "写入文件",
      "Tool execution requires approval Tool: write_file Args: {'file_path': '/.artemis/workspaces/J7Y3TW/test.txt', 'content': 'Hello, this is a test file!\\nCreated at: 2025-01-25\\n'}",
    );

    expect(view.summary).toBe("写入文件 /.artemis/workspaces/J7Y3TW/test.txt");
    expect(view.summary).not.toContain("Tool execution");
    expect(view.rows).toEqual([
      {
        label: "文件路径",
        value: "/.artemis/workspaces/J7Y3TW/test.txt",
        mono: true,
      },
      {
        label: "内容",
