...
/**
 * Collapse agent-home absolute paths (and truncated ``/.artemis/agents/…``
 * extracts) to a workspace-relative path for stable list / tab identity.
 *
 * ``/home/wally/.artemis/agents/main/generated/a.pptx``,
 * ``C:/Users/wally/.artemis/agents/main/generated/a.pptx``, and
 * ``/.artemis/agents/main/generated/a.pptx`` all become
 * ``generated/a.pptx``.
 *
...
export function canonicalizeDockFilePath(
  ...
  if (agentId) {
    ...
    const markers = [`/.artemis/agents/${idLower}/`, `.artemis/agents/${idLower}/`];
    ...
    const agentRoot = `/.artemis/agents/${idLower}`;
    if (
      lower === agentRoot ||
      lower.endsWith(agentRoot) ||
      lower === agentRoot.slice(1) ||
      lower.endsWith(`.artemis/agents/${idLower}`)
    ) {
      return "";
    }
  }

  const anyAgent = posix.match(/(?:^|\/)\.artemis\/agents\/[^/]+\/(.+)$/i);
...
function preferDisplayPath(current: string, candidate: string): string {
  ...
  const score = (p: string) => {
    ...
    if (
      p.includes("/.artemis/agents/") ||
      p.startsWith("/.artemis/agents/") ||
      /(?:^|\/)\.artemis\/agents\//i.test(p)
    ) {
      s += 500;
    }
    return s;
