...
const FOLDER_COLLAPSED_KEY = "artemis:minimal-agent-folders-collapsed";
...
export default function MinimalAgentSessionNav({
  ...
  const refreshAgentPreview = useCallback(async (agentId: string) => {
    if (!agentId) return;
    try {
      const rows = await artemisThreadsApi.list(
        agentId,
  ...
  const handleDelete = useCallback(
    async (agentId: string, sessionId: string) => {
      ...
      try {
        await artemisThreadsApi.delete(agentId, sessionId);
        patchLocal(agentId, (prev) => prev.filter((s) => s.id !== sessionId));
      } catch {
        /* ignore */
      }
  ...
  const handleRename = useCallback(
    (agentId: string, sessionId: string, name: string) => {
      ...
      if (agentId === activeAgentId) {
        ...
        void artemisThreadsApi.rename(agentId, sessionId, next).catch(() => {});
      }
