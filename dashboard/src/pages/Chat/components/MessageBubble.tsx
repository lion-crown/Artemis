...
import { BuiltinArtemisUiFallback } from "../../../plugins/toolRenderers/builtin/BuiltinArtemisUiFallback";
...
function canDockToolUi(
  ...
  if (parsed.artemisUi) return true;
  return (
    registration != null &&
...
export function ToolDetailsInline({
  ...
  const parsed = useMemo(
    () => parseArtemisToolOutput(toolData.output),
    [toolData.output],
  );
  const pluginId =
  ...
  if (parsed.artemisUi) {
    return (
      <div
        ...
        className={styles.toolUiRendererWrap}
      >
        <BuiltinArtemisUiFallback {...props} />
        {dockActions}
      </div>
    );
  }
  ...
  return (
    <div data-artemis-tool-renderer="">
