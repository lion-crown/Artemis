...
import type { ArtemisAgent } from "../../../context/AgentContext";
...
const VIEW_STORAGE_KEY = "artemis:admin-agents-view";
...
export default function ArtemisAgentsPage() {
  ...
  const [detailAgent, setDetailAgent] = useState<ArtemisAgent | null>(null);

  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const data = await request<ArtemisAgent[]>("/agents?scope=all");
      setAgents(data);
    } catch (err: unknown) {
      message.error(
  ...
  const onAct = useCallback(
    async (row: ArtemisAgent, action: "start" | "stop" | "reload") => {
  ...
  const columns: ColumnsType<ArtemisAgent> = useMemo(
    () => [
      {
        ...
        render: (name: string, row: ArtemisAgent) => (
          <button
            type="button"
      ...
      },
      {
        ...
        width: 120,
        render: (owner: string | undefined, row: ArtemisAgent) =>
          owner ? (
