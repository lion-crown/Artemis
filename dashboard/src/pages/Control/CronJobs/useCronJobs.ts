...
import { artemisCronApi } from "../../../api/modules/cronjob";
import { useAgent } from "../../../context/AgentContext";
import type { CronJobSpecOutput, ArtemisCronRow } from "../../../api/types";
...
/** Artemis-aligned form values for create / edit drawer. */
...
function fromArtemis(row: ArtemisCronRow, timezone: string): CronJob {
  ...
  return {
    ...
    meta: {
      ...
      artemis_fresh_thread: row.fresh_thread,
      artemis_session_key: row.session_key,
      artemis_model: row.model,
      artemis_task_type: row.task_type,
      artemis_mcp_servers: row.mcp_servers ?? [],
      artemis_last_run_at: row.last_run_at,
      artemis_last_status: row.last_status,
      artemis_last_error: row.last_error,
    },
  };
}
...
export function jobToFormValues(
  ...
  return {
    ...
    model: defaultModelToForm(
      job.model ??
        (typeof meta.artemis_model === "string" ? meta.artemis_model : undefined),
    ),
    fresh_thread: Boolean(meta.artemis_fresh_thread),
    session_key:
      typeof meta.artemis_session_key === "string"
        ? meta.artemis_session_key
        : null,
    mcp_servers: Array.isArray(meta.artemis_mcp_servers)
      ? (meta.artemis_mcp_servers as string[])
      : [],
...
function toArtemisCreateBody(values: CronJobFormValues) {
  return {
...
function toArtemisPatchBody(values: CronJobFormValues) {
  return {
    ...toArtemisCreateBody(values),
    enabled: Boolean(values.enabled),
  };
}
...
export function useCronJobs() {
  ...
  const fetchJobs = useCallback(
    async (opts?: { soft?: boolean }) => {
      ...
      try {
        const data = await artemisCronApi.list(agentId);
        if (gen !== fetchGenRef.current) return;
        const mapped = (data || []).map((row) =>
          fromArtemis(row, timezoneRef.current),
        );
  ...
  const createJob = async (values: CronJobFormValues) => {
    ...
    try {
      const created = await artemisCronApi.create(
        activeAgentId,
        toArtemisCreateBody(values),
      );
      const row = fromArtemis(created, cronTimezone);
  ...
  const updateJob = async (jobId: string, values: CronJobFormValues) => {
    ...
    const optimistic = {
      ...
      meta: {
        ...
        artemis_fresh_thread: values.fresh_thread,
        artemis_session_key: values.session_key,
        artemis_model: values.model,
      },
    } as CronJob;
    ...
    try {
      const updated = await artemisCronApi.patch(
        activeAgentId,
        jobId,
        toArtemisPatchBody(values),
      );
      const row = fromArtemis(updated, cronTimezone);
      patchJobs(activeAgentId, (prev) =>
  ...
  const deleteJob = async (jobId: string) => {
    ...
    try {
      await artemisCronApi.delete(activeAgentId, jobId);
  ...
  const toggleEnabled = async (job: CronJob) => {
    ...
    try {
      const returned = await artemisCronApi.patch(activeAgentId, job.id, {
        enabled: nextEnabled,
      });
      const row = fromArtemis(returned, cronTimezone);
      patchJobs(activeAgentId, (prev) =>
  ...
  const executeNow = async (jobId: string) => {
    if (!activeAgentId) return false;
    try {
      await artemisCronApi.runNow(activeAgentId, jobId);
