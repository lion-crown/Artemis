...
const REFRESH_STORAGE_KEY = "artemis:remote-browser:refresh-interval";
const PROFILE_STORAGE_KEY = "artemis:remote-browser:harness-profile";
const LEGACY_SESSION_STORAGE_KEY = "artemis:remote-browser:session-id";
/** Whether the user left the remote-browser stream open last time. */
const STREAM_ACTIVE_KEY = "artemis:remote-browser:stream-active";
const DEFAULT_REFRESH_INTERVAL = 500;
const DEFAULT_START_URL = "https://www.baidu.com";
const BROWSER_AI_PANEL_KEY = "artemis:remote-browser:ai-panel-open";
const BROWSER_AI_PANEL_WIDTH_KEY = "artemis:remote-browser:ai-panel-width";
const BROWSER_AI_PANEL_HEIGHT_KEY = "artemis:remote-browser:ai-panel-height";
...
export default function RemoteBrowserPage({
  ...
  } = useViewportMode("artemis:remote-browser:viewport", "auto");
  ...
  const handleUninstall = useCallback(() => {
    ...
    modal.confirm({
      ...
      content: t(
        ...
        "将关闭当前浏览器窗口，并卸载 Artemis 自动安装的浏览器。你电脑上已有的 Chrome 等浏览器不受影响。",
      ),
        ...
        <div>
          ...
          {installLogs.length > 0 && (
            <div>
              ...
              >
                {t(
                  "common.askArtemisHint",
                  "If the install keeps failing, copy the log and ask Artemis to help you troubleshoot.",
                )}
              </div>
            </div>
          )}
        ...
        return (
          <Alert
            ...
            description={t(
              ...
              "harness-browser (CDP) 已就绪，浏览器功能可用。如需 Playwright 备用模式，可安装 artemis[browser] extras。",
            )}
          />
        );
      }
    ...
    return (
      <Result
        ...
        subTitle={t(
          ...
          "Artemis 需要浏览器才能帮你自动打开网页、填写表单和截图。点击下方按钮即可自动安装，无需手动配置。",
        )}
        style={{ padding: "8px 0" }}
      />
    );
