...
const RESOLUTION_STORAGE_KEY = "artemis:remote-desktop:resolution";
const FPS_STORAGE_KEY = "artemis:remote-desktop:max-fps";
...
export default function DesktopPanel({
      ...
      if (needsMacPermissions) {
        message.warning(
          t(
            ...
            "请先在系统设置中开启屏幕录制与辅助功能权限，然后重启 Artemis",
          ),
        );
        return;
      }
  ...
  const pageSubtitle = t(
    ...
    "查看并操控 Artemis 主机操作系统桌面",
  );
  const setupMascot = (
    <ArtemisEmptyMascot size={120} className={styles.setupMascot} />
  );
                    ...
                    ? [
                        ...
                        },
                        {
                          label: t(
                            ...
                            "重启 Artemis（artemis run），再回到本页点击「重新检测」",
                          ),
                        },
                      ]
                    : [
