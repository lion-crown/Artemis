...
import { authApi, type ArtemisUser } from "../../api/modules/auth";
...
import { EmptyState, ArtemisEmptyMascot } from "../../components/EmptyState";
...
export default function SkillPackagesPage() {
  ...
  const [user, setUser] = useState<ArtemisUser | null>(null);
  ...
  const {
    ...
    max: 480,
    defaultSize: 280,
    storageKey: "artemis:skill-packages:sidebar-width",
  });
  const { collapsed: listPanelCollapsed, toggle: toggleListPanel } =
    useListPanelCollapsed("artemis:skill-packages:list-collapsed");
