...
interface UserCardGridProps {
  rows: UserRow[];
  loading: boolean;
  agentsByUserId: Map<number, ArtemisAgent[]>;
...
export default function UsersListPanel() {
  ...
  const [editAgent, setEditAgent] = useState<ArtemisAgent | null>(null);
  ...
  const agentsByUserId = useMemo(() => {
    const map = new Map<number, ArtemisAgent[]>();
  ...
  const patchAgent = useCallback(
    (agentId: string, patch: Partial<ArtemisAgent>) => {
      setAgents((prev) =>
