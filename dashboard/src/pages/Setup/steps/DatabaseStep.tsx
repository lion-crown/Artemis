...
export default function DatabaseStep({ onContinue, onBack }: Props) {
  ...
  const buildSqliteBody = (values: DatabaseSetupBody): DatabaseSetupBody => ({
    driver: "sqlite",
    sqlite_path: values.sqlite_path?.trim() || "artemis.db",
  });
  ...
  const buildPostgresBody = (values: DatabaseSetupBody): DatabaseSetupBody => ({
    ...
    database: values.database?.trim() || "artemis",
    user: values.user?.trim() || "artemis",
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Input prefix={<Database size={14} />} placeholder="artemis.db" />
        </Form.Item>
