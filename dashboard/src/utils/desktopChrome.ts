...
export const CHROME_END_PAD_ATTR = "data-artemis-chrome-end-pad";
export const WINDOW_CONTROLS_SPACER_ATTR = "data-artemis-window-controls-spacer";
...
export const DESKTOP_DRAG_REGION_CLASS = "artemis-desktop-drag";
export const DESKTOP_NO_DRAG_CLASS = "artemis-desktop-no-drag";
export const DESKTOP_TITLEBAR_DRAG_HEIGHT = 32;

const NO_DRAG_SELECTOR =
  'button, a, input, textarea, select, [role="button"], [role="menuitem"], [data-artemis-no-drag], .artemis-desktop-no-drag';
...
export function applyDesktopChrome(
  ...
  if (!style) {
    delete root.dataset.artemisDesktopChrome;
    root.style.removeProperty(INSET_VAR);
    return;
  }
  root.dataset.artemisDesktopChrome = style;
...
export function installDesktopWindowDrag(
  ...
  if (root.dataset.artemisDragReady === "1") return false;
  root.dataset.artemisDragReady = "1";
