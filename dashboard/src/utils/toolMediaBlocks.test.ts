...
describe("toMediaPreviewSource", () => {
  it("does not wrap workspace tree keys as host file:// paths", () => {
    expect(
      toMediaPreviewSource("/artemis-logo.png", {
        agentId: "ED7N8B",
        fromWorkspace: true,
      }),
    ).toBe("artemis-logo.png");
    expect(
  ...
  it("keeps agent-home host abs as file:// for chat/tools (virtual failback)", () => {
    expect(
      toMediaPreviewSource("/Users/me/.artemis/agents/ED7N8B/artemis-logo.png", {
        agentId: "ED7N8B",
      }),
    ).toBe("file:///Users/me/.artemis/agents/ED7N8B/artemis-logo.png");
    expect(
  ...
  it("feeds agentMediaPreviewUrl file:// for agent-home pngs from chat", () => {
    const url = agentMediaPreviewUrl(
      "ED7N8B",
      "/Users/me/.artemis/agents/ED7N8B/artemis-logo.png",
      "image/png",
    );
    expect(url).toContain("source=file");
    expect(url).toContain("artemis-logo.png");
  });
});
