 ...
 *
 * Workspace tree entries use leading-slash keys (``/artemis-logo.png``). Wrapping
 * those as ``file:///artemis-logo.png`` makes the backend look on the host root
...
export function workspacePathFromAccessUrl(url: string): string | undefined {
  try {
    const parsed = url.startsWith("http")
      ? new URL(url)
      : new URL(url, "http://artemis.local");
