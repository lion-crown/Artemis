...
describe("desktopChrome", () => {
  afterEach(() => {
    ...
    delete document.documentElement.dataset.artemisDragReady;
    vi.unstubAllGlobals();
  });
  ...
  it("reserves a trailing inset for the title row and right-edge overlays", () => {
    ...
    expect(CHROME_END_PAD_ATTR).toBe("data-artemis-chrome-end-pad");
    expect(WINDOW_CONTROLS_SPACER_ATTR).toBe(
      "data-artemis-window-controls-spacer",
    );
  ...
  it("pads right-edge overlays away from frameless window controls", () => {
    ...
    expect(css).toContain("[data-artemis-chrome-end-pad]");
    expect(css).not.toContain("[data-dock-panel]");
    expect(css).toContain(".artemis-drawer-right");
    expect(css).toMatch(/padding-inline-end:\s*max\(/);
  });

  it("exports a stable class for Wails title-bar dragging", () => {
    expect(DESKTOP_DRAG_REGION_CLASS).toBe("artemis-desktop-drag");
  });

  it("writes the inset token on <html> and can clear it", () => {
    applyDesktopChrome("mac");
    expect(document.documentElement.dataset.artemisDesktopChrome).toBe("mac");
    expect(
    ...
    applyDesktopChrome(null);
    expect(document.documentElement.dataset.artemisDesktopChrome).toBeUndefined();
    expect(
      document.documentElement.style.getPropertyValue(
        "--window-controls-inset-end",
      ),
