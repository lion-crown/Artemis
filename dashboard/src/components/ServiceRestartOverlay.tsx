...
export default function ServiceRestartOverlay({
  ...
  return createPortal(
    <div
      ...
      aria-label={t("serviceRestart.restarting")}
    >
      <div className={styles.panel} style={PANEL_STYLE}>
        {phase === "confirm" && (
          <>
            <Power size={28} className={styles.confirmIcon} />
            <p style={TITLE_STYLE}>{t("serviceRestart.confirmTitle")}</p>
            <p style={SUBTITLE_STYLE}>{t("serviceRestart.confirmContent")}</p>
            <div className={styles.actions}>
              {onCancel && (
                <Button onClick={onCancel}>{t("serviceRestart.cancel")}</Button>
              )}
              {onConfirm && (
                <Button type="primary" onClick={onConfirm}>
                  {t("serviceRestart.button")}
                </Button>
              )}
            </div>
          </>
        ...
        {phase === "restarting" && (
          <>
            <Spin size="large" tip={t("serviceRestart.restartingHint")} />
            <p style={TITLE_STYLE}>{t("serviceRestart.restarting")}</p>
          </>
        )}
        {phase === "success" && (
          <>
            <CheckCircle size={28} className={styles.successIcon} />
            <p style={TITLE_STYLE}>{t("serviceRestart.success")}</p>
            <p style={SUBTITLE_STYLE}>{t("serviceRestart.readyHint")}</p>
          </>
        )}
        {phase === "timeout" && (
          <>
            <AlertTriangle size={28} className={styles.warnIcon} />
            <p style={TITLE_STYLE}>{t("serviceRestart.timeout")}</p>
            <div className={styles.actions}>
              {onRetry && (
