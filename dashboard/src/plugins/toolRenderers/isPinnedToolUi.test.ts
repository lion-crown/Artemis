...
import { parseArtemisToolOutput } from "./parseToolOutput";
  ...
  it("does not pin empty or failed hot-list cards", () => {
    const msg = {
      ...
      toolData: {
        name: "get_hot_topics",
        output: JSON.stringify({
          artemis_ui: { renderer: "hot_topics_list" },
  ...
  it("keeps pinned tools in full-split counts after they leave the fold", () => {
    const toolMsg = {
      ...
      toolData: {
        ...
        output: JSON.stringify({
          artemis_ui: { renderer: "server_status_card" },
          data: { ok: true },
        }),
      },
    } as ChatMessage;
