...
/** Parsed ``artemis_ui`` envelope from a tool JSON string output. */
export interface ArtemisUiHint {
  renderer: string;
  version?: number;
}
...
export interface ToolRendererRegistration {
  /** Plugin-local renderer id (matches ``artemis_ui.renderer``). */
  id: string;
  /** Owning plugin id (``builtin`` for first-party renderers). */
  pluginId: string;
  /** Tool names this renderer handles when ``artemis_ui`` is absent. */
  tools?: string[];
...
export interface ArtemisPluginUIHost {
  registerRenderer(
  ...
  /** Authenticated Artemis API request (path starts with ``/`` under ``/api``). */
...
export interface PluginUiModule {
  setup?: (host: ArtemisPluginUIHost) => void;
  default?: { setup?: (host: ArtemisPluginUIHost) => void };
}
