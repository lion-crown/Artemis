...
/**
 * Fallback when tool output has ``artemis_ui`` but the plugin ESM has not
 * registered (or failed to load). Still shows structured ``data`` visibly.
 */
export function BuiltinArtemisUiFallback({
  toolName,
  displayName,
  data,
  textFallback,
  ...
  return (
    <div
      className="artemis-builtin-ui-fallback"
      data-artemis-plugin-ui="builtin-fallback"
      style={cardStyle}
    >
