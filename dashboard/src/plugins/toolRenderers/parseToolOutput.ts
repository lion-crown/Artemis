import type { ArtemisUiHint, ParsedToolOutput } from "./types";

function asArtemisUi(value: unknown): ArtemisUiHint | undefined {
...
/**
 * Parse tool ``output`` for the ``artemis_ui`` envelope.
 * Accepts a JSON string or an already-parsed object.
 */
export function parseArtemisToolOutput(output: unknown): ParsedToolOutput {
  ...
  if (typeof output === "object") {
    ...
    return {
      isJson: true,
      raw: output,
      artemisUi: asArtemisUi(obj.artemis_ui),
...
export function mergePatchedToolOutput(
  ...
  const parsed = parseArtemisToolOutput(previousOutput);
