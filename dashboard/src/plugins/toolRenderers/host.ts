...
function createHost(defaultPluginId: string): ArtemisPluginUIHost {
  return {
...
export const builtinPluginHost: ArtemisPluginUIHost = createHost("builtin");

/** Build a host scoped to an installed plugin id (for dynamic UI modules). */
export function createPluginUiHost(pluginId: string): ArtemisPluginUIHost {
  return createHost(pluginId);
}

export { parseArtemisToolOutput, mergePatchedToolOutput };
