 ...
 * in ``localStorage`` (``artemis:active-agent``), and pipes the selected id
 * into ``api/request.ts`` so every agent-scoped HTTP call gets an
 * ``X-Artemis-Agent-Id`` header.
 */

export interface ArtemisAgent {
  /** Surrogate integer primary key from the database. */
  id: number;
  /** Public agent id used in API paths and ``X-Artemis-Agent-Id``. */
  agent_id: string;
...
const STORAGE_KEY = "artemis:active-agent";
 ...
 */
export function projectChatAgentOption(agent: ArtemisAgent): {
  agent_id: string;
  name: string;
  icon_name: string | null;
  icon_url: string | null;
...
interface ListAgentsResponse {
  // Server returns ArtemisAgent[]; typed loosely so the legacy agent.ts module
  // (which has a different ``agentApi`` shape) stays
  // untouched.
  list: () => Promise<ArtemisAgent[]>;
}
 ...
 * regenerated yet.
 */
async function fetchAgents(): Promise<ArtemisAgent[]> {
  ...
  return request<ArtemisAgent[]>("/agents");
}

export function AgentProvider({ children }: { children: ReactNode }) {
  const [agents, setAgents] = useState<ArtemisAgent[]>([]);
