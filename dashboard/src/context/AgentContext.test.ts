...
import type { ArtemisAgent } from "./AgentContext";
import { projectChatAgentOption, selectEnabledExperts } from "./AgentContext";

function agent(
  agent_id: string,
  state: ArtemisAgent["state"],
  overrides: Partial<ArtemisAgent> = {},
): ArtemisAgent {
  return {
    id: 0,
    agent_id,
...
describe("projectChatAgentOption", () => {
  it("projects the full ArtemisAgent down to ChatAgentOption shape", () => {
