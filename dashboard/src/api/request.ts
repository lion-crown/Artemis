 ...
 */
export const UNAUTHORIZED_EVENT = "artemis:unauthorized";
/** Fired when an agent-scoped API request is forbidden. */
export const FORBIDDEN_EVENT = "artemis:forbidden";

/** Response header used by the server for JWT sliding renewal. */
export const ACCESS_TOKEN_RESPONSE_HEADER = "X-Artemis-Access-Token";
...
export function clearAuthToken() {
  ...
  localStorage.removeItem("artemis:active-agent");
 ...
 *
 * The value is ALSO mirrored to ``localStorage["artemis:active-agent"]`` by
 ...
 * ``X-Artemis-Agent-Id`` header. Health, admin, auth, setup, providers, and
...
function buildHeaders(path: string, extra?: HeadersInit): HeadersInit {
  ...
  if (
    ...
    !headers["X-Artemis-Agent-Id"]
  ) {
    headers["X-Artemis-Agent-Id"] = activeAgentId;
  }
