...
export interface ArtemisThread {
  thread_id: string;
  title: string | null;
  channel_type: string;
  session_key: string;
...
export interface ArtemisThreadHistory {
  thread_id: string;
  messages: Array<{
    role: string;
    content: unknown;
...
export interface ArtemisThreadPatch {
  title?: string;
  pinned?: boolean;
  model_ref?: string | null;
  reasoning_mode?: "auto" | "enabled" | "disabled" | null;
...
export const artemisThreadsApi = {
  list: (agentId: string, limit = 50) =>
    request<ArtemisThread[]>(
  ...
  history: (
    ...
    return request<ArtemisThreadHistory>(
  ...
  rename: (agentId: string, threadId: string, title: string) =>
    artemisThreadsApi.patch(agentId, threadId, { title }),

  patch: (agentId: string, threadId: string, body: ArtemisThreadPatch) =>
    request<{
      thread_id: string;
      title: string | null;
      pinned?: boolean;
