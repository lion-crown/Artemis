"""Live integration tests for the agent manager.
...
async def test_create_agent_seeds_workspace_and_starts(
    ...
    env = backend["env"]
    assert env["ARTEMIS_AGENT_ID"] == row.agent_id
    assert "ARTEMIS_AUTH_DIR" in env and "ARTEMIS_HOME" in env and "ARTEMIS_SKILLS_DIR" in env
