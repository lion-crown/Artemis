...
from tests.support.app import artemis_client
...
async def app_client(
    tmp_artemis_home: Path,
) -> AsyncIterator[tuple[httpx.AsyncClient, ArtemisServer, Path]]:
    """HTTP client + server before setup wizard completes."""
    async with artemis_client(tmp_artemis_home, patch_llm=False) as (client, srv):
        yield client, srv, tmp_artemis_home
...
async def patched_app_client(
    ...
    async with artemis_client(tmp_artemis_home) as (client, srv):
        yield client, srv, tmp_artemis_home
...
async def env(
    tmp_artemis_home: Path,
) -> AsyncIterator[tuple[httpx.AsyncClient, ArtemisServer, dict[str, str]]]:
...
async def env_admin_client(
    env: tuple[httpx.AsyncClient, ArtemisServer, dict[str, str]],
...
async def env_with_agent(
    env_with_provider: tuple[httpx.AsyncClient, ArtemisServer, dict[str, str]],
) -> AsyncIterator[tuple[httpx.AsyncClient, ArtemisServer, dict[str, str], str]]:
...
async def env_with_provider_record(
    env: tuple[httpx.AsyncClient, ArtemisServer, dict[str, str]],
) -> AsyncIterator[tuple[httpx.AsyncClient, ArtemisServer, dict[str, str], int]]:
...
async def env_alice_bob_agent(
    env_with_provider: tuple[httpx.AsyncClient, ArtemisServer, dict[str, str]],
) -> AsyncIterator[tuple[httpx.AsyncClient, ArtemisServer, dict[str, str], dict[str, str], str]]:
...
async def env_admin_alice(
    env_with_provider: tuple[httpx.AsyncClient, ArtemisServer, dict[str, str]],
) -> AsyncIterator[tuple[httpx.AsyncClient, ArtemisServer, dict[str, str], dict[str, str]]]:
    client, srv, admin_auth = env_with_provider
...
@pytest.fixture
async def env_usage(
    env_admin_alice: tuple[httpx.AsyncClient, ArtemisServer, dict[str, str], dict[str, str]],
) -> AsyncIterator[
    tuple[httpx.AsyncClient, ArtemisServer, dict[str, str], dict[str, str], dict[str, int]]
]:
...
async def env_with_channel(
    env_with_agent: tuple[httpx.AsyncClient, ArtemisServer, dict[str, str], str],
) -> AsyncIterator[tuple[httpx.AsyncClient, ArtemisServer, dict[str, str], str, int]]:
...
async def env_skills_offline(
    env_with_main_agent: tuple[httpx.AsyncClient, ArtemisServer, dict[str, str], str],
) -> AsyncIterator[tuple[httpx.AsyncClient, ArtemisServer, dict[str, str], str]]:
    ...
    def _agent_not_running(_agent_id: str) -> None:
        raise ArtemisError(ErrorCode.AGENT_NOT_RUNNING, f"agent {_agent_id!r} not running")
...
async def env_terminal(
    env_with_main_agent: tuple[httpx.AsyncClient, ArtemisServer, dict[str, str], str],
) -> AsyncIterator[tuple[ArtemisServer, Any, str, str]]:
...
async def env_fake_harness(
    tmp_artemis_home: Path,
) -> AsyncIterator[tuple[httpx.AsyncClient, ArtemisServer, FakeHarnessAgent, Path]]:
    ...
    async with artemis_client(tmp_artemis_home, fake_agent=fake) as (client, srv):
        yield client, srv, fake, tmp_artemis_home
