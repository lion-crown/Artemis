...
async def test_install_published_expert_does_not_copy_source_composer_defaults(
    ...
    owner_connector = await client.put(
        ...
        json={
            "servers": {
                "publisher-docs": {
                    ...
                    "url": "https://mcp.example.com/publisher",
                }
            },
        },
    )
    assert owner_connector.status_code == 200, owner_connector.text
    patched = await client.patch(
        f"/api/agents/{source_agent_id}",
        headers=owner_auth,
        json={
            "knowledge_base_ids": [owner_kb.id],
            "mcp_servers": ["publisher-docs"],
        },
    )
    ...
    peer_connector = await client.put(
        "/api/connectors/custom-mcp",
        headers=peer_auth,
        json={
            "servers": {
                "installer-docs": {
                    "transport": "streamable_http",
                    "url": "https://mcp.example.com/installer",
                }
            },
        },
    )
    assert peer_connector.status_code == 200, peer_connector.text
    mcp_name = "installer-docs"
    ...
    assert "publisher-docs" not in chosen["mcp_servers"]
