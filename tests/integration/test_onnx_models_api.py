...
def _seed_cache(home: Path, model: str) -> None:
    ...
    from artemis.infra.agents.providers.onnx_service import model_cache_dir
...
async def test_enable_succeeds_once_weights_are_present(
    ...
    client, _server, auth = env
    from artemis.api.routers import onnx_models
    from artemis.infra.agents.providers import onnx_service

    monkeypatch.setattr(onnx_models, "ensure_local_embedding_deps_async", _ready)
    monkeypatch.setattr(onnx_service, "local_embedding_deps_available", lambda: True)
    _seed_cache(tmp_artemis_home, MODEL)
...
async def test_activate_reports_downloaded_nested_weights(
    ...
    client, _server, auth = env
    from artemis.api.routers import knowledge_bases, onnx_models
    from artemis.infra.agents.providers import onnx_service
...
async def _missing_weights(_model: str) -> dict[str, Any]:
    return {"ok": False, "error": "ONNX embedding model is not downloaded yet"}
...
async def test_knowledge_onnx_test_reports_missing_weights(
    ...
    client, _server, auth = env
    from artemis.api.routers import knowledge_bases

    monkeypatch.setattr(knowledge_bases, "ensure_local_embedding_deps_async", _ready)
    monkeypatch.setattr(knowledge_bases, "probe_local_model", _missing_weights)
