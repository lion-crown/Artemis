...
Set ``ARTEMIS_TEST_DATABASE_URL`` to a **dedicated** database (tests DROP SCHEMA public).
Example::
...
def _conninfo() -> str:
    return os.environ["ARTEMIS_TEST_DATABASE_URL"]
...
def _pg_payload_from_url(url: str) -> dict[str, object]:
    ...
    return {
        ...
        "database": (parsed.path or "/artemis").lstrip("/") or "artemis",
...
def test_pg_migrate_and_user_roundtrip() -> None:
    ...
    from artemis.infra.db.pool import PostgresPool
...
@pytest.mark.postgresql
@pytest.mark.asyncio
async def test_setup_database_postgresql_bind(tmp_artemis_home: Path) -> None:
    """Wizard: probe + bind live PostgreSQL, then create admin."""
    from artemis.infra.db.pool import PostgresPool
    from artemis.infra.setup.password_file import read_password
    ...
    payload = _pg_payload_from_url(_conninfo())
    async with artemis_client(tmp_artemis_home, bind_database=False) as (client, srv):
        ...
        pw = read_password(tmp_artemis_home.parent)
