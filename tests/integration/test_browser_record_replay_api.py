...
async def test_record_replay_status_returns_daemon_status(env: Any) -> None:
    ...
    with (
        patch(
            "artemis.api.routers.browser.record_replay.send_record_request",
            new=AsyncMock(return_value={"ok": True, "active": None}),
        ) as mock_send,
        patch(
            "artemis.api.routers.browser.record_replay._latest_recording_id",
...
async def test_record_replay_start_ensures_daemon_and_starts_recording(env: Any) -> None:
    ...
    with (
        patch(
            "artemis.api.routers.browser.record_replay.ensure_record_daemon",
...
async def test_record_replay_replay_runs_runner(env: Any) -> None:
    ...
    with (
        ...
        patch(
            "artemis.api.routers.browser.record_replay.run_replay_recording",
            new=runner,
        ),
    ):
        r = await client.post(
