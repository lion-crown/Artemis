...
def test_probe_env_when_playwright_missing() -> None:
    ...
    from artemis.api.routers.browser import env as br
...
def test_probe_env_accepts_system_chrome() -> None:
    ...
    with (
        ...
        patch(
            "artemis.infra.browser.setup.playwright_chromium_installed",
            return_value=False,
        ),
        patch(
            "artemis.infra.browser.setup.chrome_source_for_path",
            return_value="system",
        ),
    ):
        out = br._probe_env()
...
async def test_harness_list_tabs_uses_sticky_target_id(monkeypatch) -> None:
    ...
    from artemis.api.routers.browser import harness as harness_router
