...
from artemis.infra.auth.sso.service import RedirectResult, SsoService
...
async def test_oidc_public_routes_and_exchange_return_login_shape(
    ...
    assert start.cookies["artemis_oidc_state"] == "browser-state"
    ...
    assert "artemis_oidc_state=" in callback_response.headers["set-cookie"]
