...
_DASHBOARD = Path(__file__).resolve().parents[2] / "src" / "artemis" / "dashboard"


async def test_root_serves_dashboard_index(tmp_artemis_home: Path) -> None:
    async with artemis_client(tmp_artemis_home) as (_client, srv):
        app = build_app(srv)
...
async def test_dashboard_shell_is_not_cached(tmp_artemis_home: Path) -> None:
    async with artemis_client(tmp_artemis_home) as (_client, srv):
        app = build_app(srv)
        with TestClient(app) as c:
...
async def test_hashed_dashboard_assets_are_immutable(tmp_artemis_home: Path) -> None:
...
async def test_missing_hashed_asset_returns_404(tmp_artemis_home: Path) -> None:
...
async def test_path_traversal_is_blocked(tmp_artemis_home: Path) -> None:
    async with artemis_client(tmp_artemis_home) as (_client, srv):
        app = build_app(srv)
        with TestClient(app) as c:
