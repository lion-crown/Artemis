...
from tests.support.app import write_artemis_config
...
async def test_start_writes_wizard_password_when_no_users(tmp_artemis_home: Path) -> None:
    srv = ArtemisServer(home=tmp_artemis_home)
    await srv.start()
    try:
        assert (tmp_artemis_home.parent / "artemis-login.txt").exists()
    finally:
        await srv.stop()


async def test_start_skips_wizard_password_when_disabled(tmp_artemis_home: Path) -> None:
    write_artemis_config(tmp_artemis_home, require_setup_password=False)
    srv = ArtemisServer(home=tmp_artemis_home)
    await srv.start()
    try:
        assert not (tmp_artemis_home / "artemis-login.txt").exists()
    finally:
        await srv.stop()
...
async def test_main_agent_seeds_general_assistant_workspace(patched_app_client: Any) -> None:
    ...
    assert (ws / ".artemis" / "skills" / "artemis-assistant" / "SKILL.md").is_file()
...
async def test_main_agent_uses_home_scoped_backend(patched_app_client: Any) -> None:
    ...
    assert cfg["workspace_dir"] == "/.artemis/workspaces/main"
