...
async def test_provider_test_ok(env_with_provider_record: Any) -> None:
    ...
    with patch(
        "artemis.infra.agents.providers.probe.build_probe_chat_model", return_value=fake_model
    ):
...
async def test_provider_test_failure(env_with_provider_record: Any) -> None:
    ...
    with patch("artemis.infra.agents.providers.probe.build_probe_chat_model", side_effect=boom):
        r = await c.post(f"/api/admin/providers/{pid}/test", headers=auth)
    assert r.status_code == 200, r.text
    body = r.json()
...
async def test_provider_test_embedding_skips_chat_probe(
    ...
    with patch(
        "artemis.api.routers.providers.probe_provider_row",
