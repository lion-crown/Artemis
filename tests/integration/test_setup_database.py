...
from artemis.config import DatabaseConfig, ArtemisConfig, load_config
...
@pytest.mark.asyncio
async def test_deferred_verify_password_then_bind(tmp_artemis_home: Path) -> None:
    ...
    async with artemis_client(tmp_artemis_home, bind_database=False) as (client, srv):
        ...
        applied = await client.post(
            "/api/setup/database",
            json={"driver": "sqlite", "sqlite_path": "artemis.db"},
        )
        assert applied.status_code == 200, applied.text
        assert srv.database_bound is True
        assert (tmp_artemis_home / "artemis.db").is_file()
