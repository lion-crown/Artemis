...
def _chat_ws(c: httpx.AsyncClient, aid: str, auth: dict[str, str]) -> Any:
    return ws_connect(c._artemis_app, chat_ws_path(aid, auth))  # type: ignore[attr-defined]


@pytest.fixture
async def env(tmp_artemis_home: Path) -> AsyncIterator[Any]:
    ...
    async with artemis_client(tmp_artemis_home, fake_agent=fake) as (c, srv):
        await bootstrap_admin(c, tmp_artemis_home)
