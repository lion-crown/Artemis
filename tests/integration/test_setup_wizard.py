...
from artemis.infra.setup.password_file import WIZARD_FILE_NAME, read_password
...
async def test_begin_issues_token_when_password_not_required(tmp_artemis_home: Path) -> None:
    write_artemis_config(tmp_artemis_home, require_setup_password=False)

    async with artemis_client(tmp_artemis_home) as (c, _srv):
        r = await c.get("/api/setup/status")
        body = r.json()
