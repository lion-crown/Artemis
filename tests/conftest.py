...
# Modules whose tests boot the server, real harness, browser tooling, or bwrap.
...
# teardown, so any test that boots the server without going through
# ``artemis_client`` would hang the suite. Scheduler unit tests opt out.
...
def _isolated_user_home(
    ...
    Required for xdist so workers never share ``~/.artemis`` / CLI state.
    """
...
@pytest.fixture
def tmp_artemis_home(_isolated_user_home: Path) -> Iterator[Path]:
    """Redirect ``~/.artemis`` under the autouse isolated HOME."""
    runtime_home = _isolated_user_home / ".artemis"
