...
from artemis.infra.knowledge import service as service_module
from artemis.infra.knowledge.service import MAX_DOCS_PER_KB, KnowledgeService
...
def service(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> KnowledgeService:
    monkeypatch.setenv("ARTEMIS_HOME", str(tmp_path / "home"))
...
def test_create_base_enforces_owner_limit(service: KnowledgeService) -> None:
    from artemis.infra.knowledge.service import MAX_BASES_PER_OWNER
...
def test_delete_document_works_when_feature_disabled(service: KnowledgeService) -> None:
    ...
    import artemis.infra.knowledge.service as mod
