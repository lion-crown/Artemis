...
These run without the full ``ArtemisServer`` / ``harness_gateway`` stack:
...
from artemis.api.routers import terminal
...
# ---------------------------------------------------------------------------
# WS handler driven with fakes (no real PTY, no ArtemisServer)
# ---------------------------------------------------------------------------
...
class _FakeWS:
    ...
    def __init__(self, server: object, received: tuple[str, ...] = ()) -> None:
        self.app = SimpleNamespace(state=SimpleNamespace(artemis_server=server))
...
async def test_ws_bad_token_closes_4001(monkeypatch) -> None:
    ...
    def _bad(_server, _token):
        raise ArtemisError(ErrorCode.AUTH_FAILED, "nope")
