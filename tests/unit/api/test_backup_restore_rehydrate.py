...
from artemis.api.routers import backup as backup_router
from artemis.infra.backup.auto import BACKUP_LOCK
from artemis.infra.backup.store import BackupFileInfo
...
async def test_restore_backup_file_rehydrates_providers_channels_and_cron(
    ...
    result = await backup_router.restore_backup_file(
        filename="artemis-backup.tar.gz",
        restore_config=True,
    ...
    assert result["name"] == "artemis-backup.tar.gz"
...
async def test_create_backup_offloads_to_thread(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    entry = BackupFileInfo(
        name="artemis-backup-20260101T000000Z.tar.gz",
        size=12,
...
async def test_backup_status_reports_idle_and_busy() -> None:
    from artemis.infra.backup.auto import hold_backup_lock
