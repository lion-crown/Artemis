...
from artemis.infra.utils import runtime_packages as mod


def test_find_uv_binary_from_path(monkeypatch) -> None:
    monkeypatch.setenv("ARTEMIS_UV_BIN", "")
...
def test_find_uv_binary_from_env_override(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setenv("ARTEMIS_UV_BIN", str(uv))
...
def test_install_packages_extra_fallback(monkeypatch) -> None:
    ...
    assert any("artemis[local-embedding]" in cmd for cmd in calls)
