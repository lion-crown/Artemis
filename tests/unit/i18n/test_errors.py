...
from artemis.i18n import error_message
...
def test_artemis_error_localized_factory():
    err = ArtemisError.localized(ErrorCode.FORBIDDEN, "zh")
    assert err.code is ErrorCode.FORBIDDEN
    assert err.message == "没有权限。"


def test_artemis_error_to_envelope_with_locale():
    err = ArtemisError(ErrorCode.AGENT_NAME_TAKEN, "agent name 'x' already in use")
...
def test_localized_message_falls_back_when_key_missing(monkeypatch: pytest.MonkeyPatch):
    err = ArtemisError(ErrorCode.AUTH_FAILED, "custom detail")
    monkeypatch.setattr(
        "artemis.infra.errors.i18n_error_message",
