...
def test_classify_rate_limit() -> None:
    assert (
        ...
        == "artemis:stream_errors.rate_limit"
    )
...
def test_classify_insufficient_balance() -> None:
    ...
    assert classify_stream_error_message(msg) == "artemis:stream_errors.insufficient_balance"
    assert (
        classify_stream_error_message("HTTP 402 POST https://api.example.com/v1/embeddings: quota")
        == "artemis:stream_errors.insufficient_balance"
    )
    assert (
...
def test_classify_provider_unavailable_http_status() -> None:
    assert (
        ...
        == "artemis:stream_errors.provider_unavailable"
    )
    assert (
...
def test_classify_context_length() -> None:
    assert (
        ...
        == "artemis:stream_errors.context_length"
    )
...
def test_stream_error_message_artemis_key() -> None:
