...
from artemis.infra.connectors.oauth.discovery import (
...
def test_oauth_target_helpers_only_accept_custom_mcp():
    ...
    with pytest.raises(ValueError, match="custom_mcp"):
        oauth_state_kind_for_target({"type": "catalog", "kind": "notion"})
...
async def test_discover_oauth_from_mcp_url_uses_cache(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "artemis.infra.connectors.oauth.discovery._discover_oauth_from_mcp_url_uncached",
        fake_uncached,
    )
