"""Tests for application-owned built-in Skills and the Skill Manager helper."""
...
from artemis.infra.agents.builtin_skills import sync_artemis_builtin_skills
...
async def test_sync_seeds_manager_and_retires_old_installer(tmp_path: Path) -> None:
    ...
    synced = await sync_artemis_builtin_skills(agent.workspace)
    ...
    assert "{{ARTEMIS_WORKSPACE}}" not in manager
...
async def test_sync_uses_system_files_path(tmp_path: Path) -> None:
    ...
    assert "{{ARTEMIS_SKILLS}}" not in manager
    assert str(tmp_path / ".artemis" / "skills") in manager.replace("\\", "/") or (
        tmp_path / ".artemis" / "skills"
    ).as_posix() in manager.replace("\\", "/")
    assert (tmp_path / ".artemis" / "_builtin_skills" / "skill-manager" / "SKILL.md").is_file()
    assert not (tmp_path / "_builtin_skills").exists()
    assert (tmp_path / "AGENTS.md").exists() is False

    deployed = tmp_path / ".artemis/_builtin_skills/skill-manager/scripts/manage_skills.py"
    inferred = subprocess.run(
        [sys.executable, str(deployed), "list"],
        check=False,
        capture_output=True,
...
def test_manager_refuses_to_replace_itself(tmp_path: Path) -> None:
    ...
    assert result.returncode == 1
    assert "Artemis-owned built-in" in result.stdout
