...
from artemis.infra.agents.providers.onnx_catalog import get_onnx_model_meta
from artemis.infra.agents.providers.onnx_download import (
...
def test_bge_small_zh_infers_hf_repo_without_fastembed(monkeypatch) -> None:
    from artemis.infra.agents.providers import onnx_catalog as catalog
...
def test_download_uses_winner_then_falls_back(monkeypatch, tmp_path: Path) -> None:
    from artemis.infra.agents.providers import onnx_download as mod
...
def test_hf_snapshot_emits_tqdm_byte_progress(monkeypatch, tmp_path: Path) -> None:
    ...
    from artemis.infra.agents.providers.onnx_download import _download_hf_snapshot
...
def test_cos_base_env_override(monkeypatch) -> None:
    monkeypatch.setenv("ARTEMIS_ONNX_COS_BASE", "https://example-cos.example.com/")
...
def test_cos_download_writes_hf_cache(monkeypatch, tmp_path: Path) -> None:
    from artemis.infra.agents.providers.onnx_download import _download_cos_snapshot
