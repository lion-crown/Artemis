...
from artemis.infra.agents.middleware.thread_artifacts import (
...
def _ws(tmp_path: Path) -> Path:
    return tmp_path / ".artemis" / "agents" / "ABC123"
...
def test_normalize_artifact_path_absolute_passthrough_relative_joins_workspace(
    ...
    assert (
        normalize_artifact_path("/.artemis/agents/main/skills/env-reader/SKILL.md", ws)
        == "/.artemis/agents/main/skills/env-reader/SKILL.md"
    )
...
def test_extract_screenshot_from_tool_result_text(tmp_path: Path) -> None:
    ws = _ws(tmp_path)
    abs_path = "/Users/me/.artemis/agents/A1/outbound/screenshots/harness.png"
    paths = extract_artifact_paths(
...
def test_middleware_records_successful_write(tmp_path: Path) -> None:
    ...
    with patch(
        "artemis.infra.agents.middleware.thread_artifacts.current_thread_id",
        return_value="thr_1",
    ):
...
def test_middleware_records_absolute_write_as_is(tmp_path: Path) -> None:
    ...
    store = _FakeThreads()
    mw = ThreadArtifactsMiddleware(thread_repo=store, workspace_dir=ws)
    abs_path = "/.artemis/agents/main/skills/env-reader/SKILL.md"
