...
from artemis.infra.agents.builtin_skills import ARTEMIS_BUILTIN_SKILLS_ROOT
from artemis.infra.agents.experts.catalog import (
    MANIFEST_FILENAME,
    WORKSPACE_MANIFEST_PATH,
    seed_expert_directory,
)
from artemis.infra.agents.experts.publish import (
    PublishedExpertSnapshotMeta,
    assert_can_mutate_published,
    export_agent_workspace_to_dir,
    resolve_published_expert_slug,
)
from artemis.infra.db.repos.published_experts import PublishedExpertRow
...
def test_only_creator_or_admin_can_mutate_published_expert() -> None:
    ...
    with pytest.raises(ArtemisError) as exc:
...
async def test_export_snapshot_writes_manifest_and_seed_files(tmp_path: Path) -> None:
    ...
    assert copied == 4
    assert (installed_dir / ".artemis" / "manifest.json").read_text(encoding="utf-8") == json.dumps(
        manifest,
        ensure_ascii=False,
    )
    assert (installed_dir / "skills" / "research" / "SKILL.md").read_bytes() == b"# Research"
    assert (installed_dir / ".artemis" / "avatar.png").read_bytes().startswith(b"\x89PNG")
...
async def test_export_snapshot_excludes_runtime_workspace_dirs(tmp_path: Path) -> None:
    ...
    await source.aupload_many(
        [
            ...
            ("SOUL.md", b"# Source soul"),
            (f"{ARTEMIS_BUILTIN_SKILLS_ROOT}/skill-manager/SKILL.md", b"# Builtin"),
    ...
    assert not (destination / ARTEMIS_BUILTIN_SKILLS_ROOT).exists()
