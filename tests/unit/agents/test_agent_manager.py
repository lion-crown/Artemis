"""Unit tests for :mod:`artemis.infra.agents.manager` internals."""
...
from artemis.infra.agents.experts.catalog import default_library_root
from artemis.infra.agents.manager import AgentManager, _memory_extract_settings
from artemis.infra.backend.resolver import default_agent_backend_spec
...
def _expected_default_backend(manager: AgentManager, agent_id: str) -> dict[str, Any]:
    from artemis.infra.agents.execute_env import inject_agent_execute_env
...
def manager(tmp_path: Path) -> AgentManager:
    ...
    run_migrations(db)
    services = build_shared_services(db=db, paths=paths, config=ArtemisConfig())
...
def test_build_harness_config_business_system_qa_uses_business_root(
    manager: AgentManager, tmp_path: Path
) -> None:
    agent_id = "AGT_BUSINESS"
    system_workspace = manager._paths.ensure_agent_workspace(agent_id)
    (system_workspace / ".bootstrapped").write_text("", encoding="utf-8")
    ...
    row = _row(
        ...
        config_json=json.dumps(
            {
                "workspace_dir": str(system_workspace),
                "system_files_path": ".artemis",
                "execution_profile": "business_system_qa",
                "backend": {
                    "type": "local_shell",
                    "root_dir": str(business_root),
                    "virtual_mode": True,
                },
            }
        ),
    ...
    assert "你是业务问答助手" in cfg.system_prompt
    assert cfg.backend["type"] == "composite"
    assert cfg.backend["default"]["root_dir"] == str(business_root)
    assert cfg.backend["routes"]["/.artemis/"]["root_dir"] == str(system_workspace / ".artemis")
    assert not any(
        type(item).__name__ == "BusinessSearchLimitMiddleware" for item in (cfg.middleware or [])
    )
...
def test_build_harness_config_appends_custom_business_reply_guidance(
    manager: AgentManager,
) -> None:
    from dataclasses import replace

    agent_id = "AGT_BUSINESS_REPLY"
    ws = manager._paths.ensure_agent_workspace(agent_id)
    (ws / ".bootstrapped").write_text("", encoding="utf-8")
    row = replace(
        _row(agent_id=agent_id),
        ...
        config_json=json.dumps(
            {
                "backend": _fs_backend(ws),
                "execution_profile": "lean_code_qa",
                "business_reply_guidance": "面向销售同事，先说业务影响。",
            }
        ),
    )
    ...
    assert cfg.system_prompt is not None
    assert "Code-answer policy" in cfg.system_prompt
    assert "面向销售同事" in cfg.system_prompt
    assert "does not conflict with system rules" in cfg.system_prompt
...
async def test_delete_thread_checkpoint_returns_false_when_agent_not_running(
    ...
    # ArtemisError, which must be swallowed (checkpoint cleanup is best-effort).
...
def test_build_harness_config_keeps_fs_permissions_for_local_shell_guard(
    ...
    Artemis must not re-mount the guard (or ModelSettings) via cfg.middleware.
    """
...
def test_get_business_openai_model_uses_the_selected_agent_model(
    manager: AgentManager,
) -> None:
    _seed_test_provider(manager)
    manager._repos.agent_repo.create(
        ...
        user_id=None,
        name="business",
        default_model="test-openai/gpt-4o-mini",
        config_json=json.dumps(_MEMORY_OFF),
    )
    result = manager.get_business_openai_model("business-agent", None)

    assert isinstance(result, BusinessOpenAIModel)
    assert result.model == "gpt-4o-mini"
    assert result.base_url == "https://api.example.com/v1"


def test_cancel_stream_signals_active_business_investigation(manager: AgentManager) -> None:
    event = manager.begin_business_investigation("agent", "thread")

    manager.cancel_stream("agent", "thread")

    assert event.is_set()
    manager.end_business_investigation("agent", "thread")
...
def test_build_harness_config_passes_default_model_without_embedded_providers(
    ...
    from artemis.infra.agents import manager as mgr_mod
...
async def test_stop_and_start_round_trip(manager: AgentManager) -> None:
    ...
    with pytest.raises(ArtemisError, match="not running") as stopped_exc:
...
async def test_reload_skips_stopped_agent(manager: AgentManager) -> None:
    ...
    with pytest.raises(ArtemisError, match="not running") as skipped_exc:
...
async def test_create_seeds_bootstrap_files(manager: AgentManager) -> None:
    ...
    from artemis.infra.agents.manager import AgentCreateSpec
    ...
    assert cfg.get("system_files_path") == ".artemis"
    ws = manager.resolve_workspace_dir(row.agent_id)
    assert cfg.get("workspace_dir") == str(ws)
    assert (ws / "AGENTS.md").is_file()
    assert (ws / ".artemis" / "_builtin_skills").is_dir() or agent.workspace.exists(
...
async def test_create_persists_rootfs_workspace_under_scoped_root(
    ...
    try:
        # manager fixture uses ``{tmp_path}/.artemis`` as ARTEMIS_HOME, so scoping
        ...
        cfg = manager.get_config(row.agent_id)
        assert cfg["workspace_dir"] == f"/.artemis/workspaces/{row.agent_id}"
        host = manager.resolve_workspace_dir(row.agent_id)
        assert host == (tmp_path / ".artemis" / "workspaces" / row.agent_id).resolve()
        assert host.is_dir()
        ...
        expected_harness_ws = (
            Path(f"/.artemis/workspaces/{row.agent_id}") if os.name == "posix" else host
        )
        assert Path(harness_cfg.workspace_dir) == expected_harness_ws
        assert not (tmp_path / ".artemis" / "agents" / row.agent_id).exists()
    finally:
        manager._harness_manager.close()
...
async def test_seed_expert_template_writes_workspace_files(
    ...
    from artemis.infra.agents.experts.catalog import Expert, ExpertCatalog, ExpertSummary
    ...
    manifest = json.loads((ws / ".artemis" / "manifest.json").read_text(encoding="utf-8"))
...
async def test_update_config_json_cannot_change_system_files_path(
    ...
    assert manager.get_config(row.agent_id).get("system_files_path") == ".artemis"
...
async def test_reload_agent_does_not_block_event_loop(
    ...
    from artemis.infra.agents.manager import AgentCreateSpec, AgentManager
...
def test_peer_manifest_metadata_bad_json(manager: AgentManager) -> None:
    manager._repos.agent_repo.create(agent_id="AGT1", user_id=None, name="demo")
    ws = manager._paths.ensure_agent_workspace("AGT1")
    (ws / ".artemis").mkdir(parents=True, exist_ok=True)
    (ws / ".artemis" / "manifest.json").write_text("{", encoding="utf-8")
...
def test_peer_manifest_metadata_reads_cards(manager: AgentManager) -> None:
    ...
    ws = manager._paths.ensure_agent_workspace("AGT1")
    (ws / ".artemis").mkdir(parents=True, exist_ok=True)
    (ws / ".artemis" / "manifest.json").write_text(
        json.dumps(
            {
