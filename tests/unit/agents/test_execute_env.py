...
from artemis.infra.utils.env_file import save_env_file
...
def test_inject_local_shell_uses_artemis_auth_for_new_layout(tmp_path: Path, monkeypatch) -> None:
    ...
    spec = inject_agent_execute_env(
        ...
        row=_row("agent-1"),
        workspace_dir=ws,
        cfg={"system_files_path": ".artemis"},
    )
    env = spec["env"]
    assert env["ARTEMIS_AGENT_ID"] == "agent-1"
    assert (ws / ".artemis" / "auth").is_dir()
    assert env["ARTEMIS_AUTH_DIR"] == str(ws / ".artemis" / "auth")
    assert env["ARTEMIS_HOME"] == str(paths.root)
    assert env["ARTEMIS_SKILLS_DIR"] == str(ws / ".artemis" / "skills")
    assert "GLOBAL_KEY" not in env
    assert "AGENT_KEY" not in env
    assert spec["inherit_env"] is True


def test_inject_legacy_layout_keeps_root_artemis_auth(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("ARTEMIS_HOME", str(tmp_path))
    paths = PathLayout(tmp_path)
    paths.ensure_root()
    ...
    assert (ws / ".artemis-auth").is_dir()
    assert spec["env"]["ARTEMIS_AUTH_DIR"] == str(ws / ".artemis-auth")
...
def test_inject_keeps_legacy_auth_when_tokens_already_there(tmp_path: Path, monkeypatch) -> None:
    ...
    assert spec["env"]["ARTEMIS_AUTH_DIR"] == str(legacy)
...
def test_inject_composite_default_local_shell(tmp_path: Path, monkeypatch) -> None:
    ...
    assert spec["default"]["env"]["ARTEMIS_AGENT_ID"] == "agent-c"
    assert spec["default"]["env"]["ARTEMIS_AUTH_DIR"] == str(ws / ".artemis" / "auth")
...
def test_inject_scoped_root_uses_agent_facing_auth_paths(tmp_path: Path, monkeypatch) -> None:
    ...
    root.mkdir()
    ws = root / ".artemis" / "workspaces" / "agent-s"
    ws.mkdir(parents=True)
    cfg = {
        "system_files_path": ".artemis",
        "workspace_dir": "/.artemis/workspaces/agent-s",
        "backend": {
            "type": "local_shell",
            "root_dir": str(root),
            "virtual_mode": True,
    ...
    env = spec["env"]
    assert (ws / ".artemis" / "auth").is_dir()
    assert env["ARTEMIS_AUTH_DIR"] == "/.artemis/workspaces/agent-s/.artemis/auth"
    assert env["ARTEMIS_SKILLS_DIR"] == "/.artemis/workspaces/agent-s/.artemis/skills"
...
def test_inject_docker_only_sets_platform_defaults(tmp_path: Path, monkeypatch) -> None:
    ...
    env = spec["env"]
    assert env["ARTEMIS_AGENT_ID"] == "agent-2"
    assert env["ARTEMIS_AUTH_DIR"] == str(ws / ".artemis" / "auth")
    assert "GLOBAL_KEY" not in env
