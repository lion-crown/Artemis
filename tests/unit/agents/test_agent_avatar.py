...
def test_sniff_and_reject() -> None:
    ...
    with pytest.raises(ArtemisError) as raised:
...
async def test_write_read_delete_roundtrip() -> None:
    ...
    assert await write_workspace_avatar(workspace, JPEG) == ".artemis/avatar.jpg"
    found = await read_workspace_avatar(workspace)
    assert found is not None
    data, media_type = found
    assert media_type == "image/jpeg"
    assert data == JPEG
    assert workspace.files == {".artemis/avatar.jpg": JPEG}
    assert agent_avatar_api_path("agent01") == "/api/agents/agent01/avatar"
    await write_workspace_avatar(workspace, PNG)
    assert set(workspace.files) == {".artemis/avatar.png"}
...
async def test_write_skips_unsupported_mkdir() -> None:
    ...
    workspace = _NoMkdir()
    assert await write_workspace_avatar(workspace, PNG) == ".artemis/avatar.png"
    assert workspace.files[".artemis/avatar.png"] == PNG
...
def test_read_snapshot_avatar_and_display_url(tmp_path) -> None:
    ...
    avatar = tmp_path / ".artemis" / "avatar.png"
