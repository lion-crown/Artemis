...
def test_scoped_default_persists_agent_facing_path(tmp_path: Path) -> None:
    root = tmp_path / "home"
    root.mkdir()
    paths = PathLayout(tmp_path / "artemis-home")
    cfg = _scoped_cfg(root)
    host = seed_workspace_dir_on_create(cfg, paths=paths, agent_id="J1BT2X")
    assert cfg["workspace_dir"] == "/.artemis/workspaces/J1BT2X"
    # On-disk tree still lands under the jail root_dir.
    assert host == (root / ".artemis" / "workspaces" / "J1BT2X").resolve()
    assert host.is_dir()
    ...
    assert agent_facing_workspace_dir_from_config(cfg) == "/.artemis/workspaces/J1BT2X"
    assert (
        agent_facing_workspace_root(
            host,
            root_dir=root,
        ...
        == "/.artemis/workspaces/J1BT2X"
    )
    assert join_agent_facing("/.artemis/workspaces/J1BT2X", "inbound/a.zip") == (
        "/.artemis/workspaces/J1BT2X/inbound/a.zip"
    )


def test_host_ops_map_artemis_workspaces_under_root(tmp_path: Path) -> None:
    root = tmp_path / "home"
    expected = root / ".artemis" / "workspaces" / "J1BT2X"
    expected.mkdir(parents=True)
    cfg = _scoped_cfg(root)
    assert resolve_workspace_host_path("/.artemis/workspaces/J1BT2X", cfg) == expected.resolve()
...
def test_host_rooted_default_uses_artemis_home(tmp_path: Path) -> None:
    ...
    assert host == (tmp_path / ".artemis" / "agents" / "A1").resolve()
...
def test_harness_workspace_maps_non_absolute_persisted_value(tmp_path: Path) -> None:
    """Windows cannot express ``/.artemis/workspaces/<id>`` as absolute — host-map it."""
    root = tmp_path / "home"
    cfg = _scoped_cfg(root, workspace_dir=".artemis/workspaces/W1N")
    assert harness_workspace_path(cfg["workspace_dir"], cfg) == (
        root / ".artemis" / "workspaces" / "W1N"
    )
