...
def _patch_harness(monkeypatch) -> MagicMock:
    ...
    monkeypatch.setattr(
        "artemis.infra.agents.manager.HarnessAgentManager",
        _constructor,
    )
    return fake_hm
...
async def test_update_raises_for_unknown_id(tmp_path: Path) -> None:
    """update() raises ArtemisError when the agent_id does not exist."""
    from artemis.infra.errors import ArtemisError  # noqa: PLC0415
...
async def test_list_subagent_summaries_fills_color_from_frontmatter(tmp_path: Path) -> None:
    """When harness omits color, Artemis copies it from workspace frontmatter."""
    services = _make_services(tmp_path)
    fake_agent = MagicMock(name="HarnessAgent")
    fake_agent.list_subagent_summaries = AsyncMock(
        return_value=[
...
async def test_on_provider_changed_bootstraps_factory_when_none(
    ...
    monkeypatch.setattr(
        "artemis.infra.agents.providers.store.ProviderStore.build_harness_configs",
...
async def test_on_provider_changed_adds_new_provider(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "artemis.infra.agents.manager.HarnessAgentConfig",
...
async def test_create_with_template_writes_files(tmp_path: Path) -> None:
    ...
    # New agents use system_files_path=.artemis — BackendWorkspace remaps skills/.
    skill_md = ws / ".artemis" / "skills" / "my-skill" / "SKILL.md"
...
async def test_create_with_unknown_template_does_not_crash(tmp_path: Path) -> None:
    ...
    from artemis.infra.agents.experts.catalog import ExpertCatalog  # noqa: PLC0415
