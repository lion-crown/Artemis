...
from artemis.infra.skills import skills_hub
from artemis.infra.skills.skills_hub import (
...
def test_resolve_skills_sh_url_uses_raw_github_without_token(
    ...
    with patch(
        "artemis.infra.skills.skills_hub._http_text_get",
...
def test_resolve_github_url_accepts_none_version(
    ...
    with (
        patch("artemis.infra.skills.skills_hub._http_text_get", side_effect=_fake_http_text_get),
        patch("artemis.infra.skills.skills_hub._http_bytes_get", return_value=archive.getvalue()),
    ):
        resolved = resolve_bundle_from_url(
...
def test_resolve_github_url_with_slash_branch_uses_encoded_archive_ref(
    ...
    with (
        patch("artemis.infra.skills.skills_hub._http_text_get", side_effect=_fake_http_text_get),
        patch("artemis.infra.skills.skills_hub._http_bytes_get", side_effect=_fake_http_bytes_get),
    ):
        resolved = resolve_bundle_from_url(
...
def test_custom_import_source_can_be_enabled_without_frontend_changes(
    ...
    monkeypatch.setenv(
        "ARTEMIS_SKILLS_IMPORT_URL_PREFIXES",
...
def test_custom_json_source_resolves_through_generic_adapter(
    ...
    with patch(
        "artemis.infra.skills.skills_hub._http_json_get",
