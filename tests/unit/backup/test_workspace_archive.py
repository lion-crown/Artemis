...
async def test_export_ignores_same_named_file_at_backend_root(tmp_path: Path) -> None:
    ...
    """
    home = tmp_path / "home"
    workspace_dir = home / ".artemis" / "workspaces" / "AGT1"
    ...
    workspace = BackendWorkspace(backend, workspace_dir, system_files_path=".artemis")
...
async def test_replace_import_keeps_system_state(tmp_path: Path) -> None:
    ...
    Hidden paths are outside the export glob, so ``.artemis`` (sessions / skills /
    auth) is absent from every archive; clearing it on replace destroyed data that
    the import could not restore.
    """
    ws = tmp_path / "ws"
    (ws / ".artemis" / "sessions").mkdir(parents=True)
    (ws / ".artemis" / "sessions" / "state.db").write_bytes(b"SESSION DB")
    (ws / ".artemis" / "auth").mkdir(parents=True)
    (ws / ".artemis" / "auth" / "token.json").write_bytes(b"TOKEN")
    (ws / "old.txt").write_text("old", encoding="utf-8")
    ...
    assert (ws / "new.txt").read_bytes() == b"new"
    assert (ws / ".artemis" / "sessions" / "state.db").read_bytes() == b"SESSION DB"
    assert (ws / ".artemis" / "auth" / "token.json").read_bytes() == b"TOKEN"
...
async def test_export_reads_legacy_root_system_dir_from_workspace(tmp_path: Path) -> None:
    ...
    (``.artemis``), so probing it for a workspace that still keeps legacy root
