...
def test_backups_dir_paths(tmp_path: Path) -> None:
    layout = PathLayout(tmp_path / ".artemis")
    assert layout.backups_dir == tmp_path / ".artemis" / "backups"
...
def test_store_roundtrip(tmp_path: Path) -> None:
    layout = PathLayout(tmp_path / ".artemis")
    write_backup_file(layout, "artemis-backup-test.tar.gz", b"payload")
    items = list_backup_files(layout)
    assert len(items) == 1
    assert items[0].name == "artemis-backup-test.tar.gz"
    assert read_backup_file(layout, "artemis-backup-test.tar.gz") == b"payload"
    delete_backup_file(layout, "artemis-backup-test.tar.gz")
...
def test_created_at_from_canonical_filename(tmp_path: Path) -> None:
    layout = PathLayout(tmp_path / ".artemis")
    name = "artemis-backup-20260721T013000Z.tar.gz"
...
def test_created_at_falls_back_on_invalid_filename_stamp(tmp_path: Path) -> None:
    ...
    write_backup_file(layout, "artemis-backup-20261399T999999Z.tar.gz", b"x")
    items = list_backup_files(layout)
    assert len(items) == 1
    datetime.fromisoformat(items[0].created_at)
...
def test_created_at_from_auto_filename(tmp_path: Path) -> None:
    layout = PathLayout(tmp_path / ".artemis")
    name = "artemis-auto-backup-20260721T020000Z.tar.gz"
    write_backup_file(layout, name, b"x")
    items = list_backup_files(layout)
    assert len(items) == 1
    assert items[0].created_at == "2026-07-21T02:00:00+00:00"
    assert is_auto_backup_filename(name)
    assert not is_auto_backup_filename("artemis-backup-20260721T020000Z.tar.gz")


def test_prune_auto_backups_keeps_newest_and_spares_manual(tmp_path: Path) -> None:
    layout = PathLayout(tmp_path / ".artemis")
    write_backup_file(layout, "artemis-backup-20260101T000000Z.tar.gz", b"manual")
    write_backup_file(layout, "artemis-auto-backup-20260101T010000Z.tar.gz", b"a1")
    write_backup_file(layout, "artemis-auto-backup-20260102T010000Z.tar.gz", b"a2")
    write_backup_file(layout, "artemis-auto-backup-20260103T010000Z.tar.gz", b"a3")

    deleted = prune_auto_backups(layout, keep=2)
    assert set(deleted) == {"artemis-auto-backup-20260101T010000Z.tar.gz"}
    names = {f.name for f in list_backup_files(layout)}
    assert names == {
        "artemis-backup-20260101T000000Z.tar.gz",
        "artemis-auto-backup-20260102T010000Z.tar.gz",
        "artemis-auto-backup-20260103T010000Z.tar.gz",
    }
...
def test_prune_auto_backups_keep_zero_deletes_all_auto(tmp_path: Path) -> None:
    ...
    assert deleted == ["artemis-auto-backup-20260101T010000Z.tar.gz"]
    names = {f.name for f in list_backup_files(layout)}
    assert names == {"artemis-backup-20260101T000000Z.tar.gz"}


def test_peek_backup_contents_reads_manifest(tmp_path: Path) -> None:
    from artemis.config import DatabaseConfig
    from artemis.infra.backup.system_archive import create_system_backup
    ...
    write_backup_file(layout, "artemis-backup-peek.tar.gz", archive.read_bytes())
...
def _minimal_manifest_json(**flags: bool) -> bytes:
    ...
    payload = {
        "manifest_version": 1,
        "artemis_version": "0.0.0-test",
        "schema_version": 1,
        "created_at": "2026-01-01T00:00:00+00:00",
        "home": "/tmp",
        "db_file": "db/artemis.db",
        "agents": [],
