...
from artemis.config import DatabaseConfig
from artemis.infra.backup.manifest import MANIFEST_VERSION, BackupManifest
...
def test_manifest_roundtrip_includes_driver_fields() -> None:
    ...
    assert loaded.db_file == "db/artemis.dump"
...
def test_backup_skips_unwritable_workspace_dir(layout: PathLayout, tmp_path: Path) -> None:
    """Stale host paths (e.g. former /root/.artemis/agents/…) must not abort backup."""
...
def test_default_backup_omits_chats_and_restore_preserves_current_chats(
    ...
    source_sessions = source_ws / ".artemis" / "sessions"
    source_sessions.mkdir(parents=True)
    (source_sessions / "large.jsonl").write_text("chat", encoding="utf-8")
    (source_ws / ".artemis" / "checkpoints.sqlite").write_bytes(b"chat-db")
    ...
    class Row:
        agent_id = "agent01"
        name = "Agent"
        config_json = '{"system_files_path": ".artemis"}'
    ...
    assert "workspaces/agent01/.artemis/sessions/large.jsonl" not in names
    assert "workspaces/agent01/.artemis/checkpoints.sqlite" not in names
    ...
    target_sessions = target_layout.ensure_agent_workspace("agent01") / ".artemis" / "sessions"
...
def test_restore_skips_chat_workspace_files_when_manifest_omits_chats(
    ...
    members["workspaces/agent01/.artemis/sessions/injected.jsonl"] = b"overwrite-me"
    ...
    run_migrations(target_pool)
    keep = target.ensure_agent_workspace("agent01") / ".artemis" / "sessions" / "current.jsonl"
    ...
    assert not (
        target.ensure_agent_workspace("agent01") / ".artemis" / "sessions" / "injected.jsonl"
    ).exists()


def test_sqlite_strip_chats_reclaims_pages(tmp_path: Path) -> None:
    from artemis.infra.backup.chats import strip_chat_tables_from_sqlite_file
    from artemis.infra.backup.snapshot import snapshot_sqlite_file
...
def _make_migration_backup(
    ...
    *,
    ...
    # Rewrite artemis_version to signal a LightClaw migration backup.
    members: dict[str, bytes] = {}
    ...
    manifest_obj["artemis_version"] = manifest_obj["artemis_version"] + "-migrated-from-lightclaw"
...
def test_migration_restore_preserves_current_users_and_imported_agents(
    ...
    - users table contains only the *current instance* users (``artemis_admin``).
    ...
    # --- target: a fresh Artemis instance with its own admin user + JWT ---
    tgt_layout = PathLayout(tmp_path / "tgt")
    tgt_layout.root.mkdir()
    tgt_pool = SqlitePool(tgt_layout.db)
    run_migrations(tgt_pool)
    local_jwt = b"local-artemis-jwt-secret-must-survive-restore!!"
    with tgt_pool.connect() as conn:
        conn.execute(
            "INSERT INTO users(username, password_hash, role, created_at) VALUES (?, ?, ?, ?)",
            ("artemis_admin", "admin_hash", "admin", 2),
        )
        conn.execute(
            "INSERT INTO secrets(k, v, created_at) VALUES (?, ?, ?)",
            ("jwt", local_jwt, 2),
    ...
    with tgt_pool.connect() as conn:
        ...
        row = conn.execute(
            "SELECT password_hash FROM users WHERE username = ?", ("artemis_admin",)
        ).fetchone()
        assert row is not None, "artemis_admin was deleted — users not preserved"
        ...
        admin_id = conn.execute(
            "SELECT id FROM users WHERE username = ?", ("artemis_admin",)
        ).fetchone()[0]
...
def test_migration_restore_remaps_ownership_to_admin_user_id_2(tmp_path: Path) -> None:
    ...
    with tgt_pool.connect() as conn:
        ...
        assert [(r[0], r[1]) for r in users] == [(2, "artemis_admin")]
...
def test_migration_restore_via_none_autodetect(tmp_path: Path) -> None:
    """preserve_users=None auto-detects the migration flag from artemis_version."""
    src_layout = PathLayout(tmp_path / "src")
    src_layout.root.mkdir()
    ...
    with source_pool.connect() as conn:
        conn.executescript(
            (
                ...
                / "src/artemis/infra/db/migrations/001_initial.sql"
            ).read_text()
        )
...
def test_refuse_newer_schema_backup_before_database_replace(
    ...
    with pytest.raises(ArtemisError) as excinfo:
        restore_system_backup(
            archive,
            paths=layout,
            pool=pool,
