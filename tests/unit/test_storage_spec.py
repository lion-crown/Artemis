...
from artemis.infra.backend.adapter import row_to_backend_spec, storage_backend_kind_agent_resolvable
...
from artemis.infra.db.repos.backends import BackendRepo, BackendRow
...
def test_row_to_backend_spec_opensandbox() -> None:
    from artemis.infra.backend.adapter import storage_spec_previewable
...
def test_row_to_backend_spec_docker() -> None:
    row = BackendRow(
        ...
        region=None,
        config_json='{"allow_network": false, "memory": "512m", "sandbox_scope": "user", "sandbox_prefix": "artemis_sandbox"}',
        note=None,
        enabled=1,
        created_at=0,
        updated_at=0,
...
def test_enrich_docker_backend_spec_defaults() -> None:
    ...
    assert agent["sandbox_prefix"] == "artemis_sandbox"
...
def test_inject_docker_global_environment_sets_file() -> None:
    from artemis.infra.backend.docker_spec import inject_docker_global_environment

    spec = inject_docker_global_environment(
        {"type": "docker", "environment": {"FOO": "from-spec"}},
        "/data/artemis/env",
    )
    assert spec["environment_file"] == "/data/artemis/env"
    assert spec["environment"] == {"FOO": "from-spec"}
    local = inject_docker_global_environment({"type": "local_shell"}, "/data/artemis/env")
