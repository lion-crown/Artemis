...
def test_channel_list_uses_root_agent_fallback(monkeypatch) -> None:
    ...
    (or ARTEMIS_AGENT env) supplies a fallback when not given inline."""
    ...
    import artemis.cli.commands.channel as channel_cmd
    from artemis.cli.support import offline_ops as offline_mod
...
def test_root_help_does_not_import_subcommand_modules() -> None:
    """`artemis --help` must not import individual command modules eagerly."""
    import importlib
    import sys

    for mod in list(sys.modules):
        if mod.startswith("artemis.cli.commands."):
            del sys.modules[mod]
    importlib.invalidate_caches()
    # Force re-import of main itself so the lazy group is fresh.
    if "artemis.cli.main" in sys.modules:
        del sys.modules["artemis.cli.main"]
    from artemis.cli.main import cli as fresh_cli

    runner = CliRunner()
    result = runner.invoke(fresh_cli, ["--help"])
    assert result.exit_code == 0

    leaked = [m for m in sys.modules if m.startswith("artemis.cli.commands.")]
