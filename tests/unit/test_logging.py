...
Covers the daily-rotation + retention logging strategy in ``artemis.infra.server``:
the log path lives under ``~/.artemis/logs`` and stale rotated files are purged.
"""
...
from artemis.infra.server import (
...
def test_build_log_handler_uses_daily_rotation_and_retention(tmp_path: Path):
    handler = _build_log_handler(tmp_path / "artemis.log", retention_days=7, max_bytes=4096)
    try:
...
def test_purge_stale_logs_removes_only_old_files(tmp_path: Path):
    ...
    log_dir.mkdir()
    old = _stale_path(log_dir, "artemis.log.2020-01-01", age_days=100)
    new = _stale_path(log_dir, "artemis.log.2026-07-16", age_days=1)
...
def test_attach_log_handler_is_idempotent(tmp_path: Path):
    root = logging.getLogger()
    handler = _build_log_handler(tmp_path / "artemis.log", retention_days=7)
    try:
...
def test_setup_logging_creates_logs_dir_and_file(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("ARTEMIS_LOG_LEVEL", raising=False)
    monkeypatch.delenv("ARTEMIS_LOG_RETENTION_DAYS", raising=False)
    server = ArtemisServer(home=tmp_path / ".artemis")
...
def test_setup_logging_restricts_runtime_log_paths(tmp_path: Path) -> None:
    server = ArtemisServer(home=tmp_path / ".artemis")

    server._setup_logging()

    assert server.paths.root.stat().st_mode & 0o777 == 0o700
    assert server.paths.logs_dir.stat().st_mode & 0o777 == 0o700
    assert server.paths.log.stat().st_mode & 0o777 == 0o600


@pytest.mark.skipif(os.name != "posix", reason="POSIX permission bits")
def test_setup_logging_prepares_owner_only_harness_log(tmp_path: Path) -> None:
    server = ArtemisServer(home=tmp_path / ".artemis")

    server._setup_logging()

    harness_log = server.paths.logs_dir / f"{datetime.now().astimezone():%Y-%m-%d}.log"
    assert harness_log.is_file()
    assert harness_log.stat().st_mode & 0o777 == 0o600


def test_setup_logging_migrates_legacy_log(tmp_path: Path):
    home = tmp_path / ".artemis"
    legacy = home / "artemis.log"
    legacy.parent.mkdir(parents=True, exist_ok=True)
    legacy.write_text("legacy content", encoding="utf-8")
    server = ArtemisServer(home=home)
...
def test_setup_logging_respects_retention_env(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("ARTEMIS_LOG_RETENTION_DAYS", "3")
    server = ArtemisServer(home=tmp_path / ".artemis")
...
def test_setup_logging_respects_level_env(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("ARTEMIS_LOG_LEVEL", "debug")
    server = ArtemisServer(home=tmp_path / ".artemis")
...
def test_setup_logging_falls_back_on_invalid_level_env(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("ARTEMIS_LOG_LEVEL", "not-a-level")
    server = ArtemisServer(home=tmp_path / ".artemis")
...
def test_setup_logging_falls_back_on_invalid_retention_env(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("ARTEMIS_LOG_RETENTION_DAYS", "oops")
    server = ArtemisServer(home=tmp_path / ".artemis")
...
def test_parse_log_max_bytes_defaults_and_env(monkeypatch):
    monkeypatch.delenv("ARTEMIS_LOG_MAX_BYTES", raising=False)
    assert _parse_log_max_bytes() == DEFAULT_LOG_MAX_BYTES
    monkeypatch.setenv("ARTEMIS_LOG_MAX_BYTES", "2048")
    assert _parse_log_max_bytes() == 2048
    monkeypatch.setenv("ARTEMIS_LOG_MAX_BYTES", "bad")
    assert _parse_log_max_bytes() == DEFAULT_LOG_MAX_BYTES


def test_log_handler_rotates_when_max_bytes_exceeded(tmp_path: Path):
    log_path = tmp_path / "artemis.log"
    handler = _build_log_handler(log_path, retention_days=3, max_bytes=256)
    test_logger = logging.getLogger("test.artemis.log.size_roll")
    ...
    try:
        ...
        handler.flush()
        backups = [p for p in tmp_path.glob("artemis.log*") if p != log_path]
...
def test_delaycompress_keeps_newest_plain_on_startup(tmp_path: Path):
    log_dir = tmp_path / "logs"
    log_dir.mkdir()
    older = log_dir / "artemis.log.2026-08-28"
    newer = log_dir / "artemis.log.2026-08-29"
    ...
    assert (log_dir / "artemis.log.2026-08-28.gz").exists()
    assert newer.exists()
    assert not (log_dir / "artemis.log.2026-08-29.gz").exists()


def test_delaycompress_single_plain_stays_uncompressed(tmp_path: Path):
    log_dir = tmp_path / "logs"
    log_dir.mkdir()
    plain = log_dir / "artemis.log.2026-08-29"
...
def test_parse_log_compress_env(monkeypatch):
    monkeypatch.delenv("ARTEMIS_LOG_COMPRESS", raising=False)
    assert _parse_log_compress() is True
    monkeypatch.setenv("ARTEMIS_LOG_COMPRESS", "0")
...
def test_purge_stale_logs_removes_old_gz_files(tmp_path: Path):
    log_dir = tmp_path / "logs"
    log_dir.mkdir()
    old = _stale_path(log_dir, "artemis.log.2020-01-01.gz", age_days=100)
    new = _stale_path(log_dir, "artemis.log.2026-07-16.gz", age_days=1)
    _purge_stale_logs(log_dir, retention_days=14)
    assert not old.exists()
    assert new.exists()
...
def test_setup_logging_respects_max_bytes_env(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("ARTEMIS_LOG_MAX_BYTES", "8192")
    server = ArtemisServer(home=tmp_path / ".artemis")
    server._setup_logging()
    handler = next(
        h
