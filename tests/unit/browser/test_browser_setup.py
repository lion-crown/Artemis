...
from artemis.infra.browser.setup import (
    _probe_dir_writable,
...
def test_configure_browser_idle_timeout_updates_harness(
    ...
    from artemis.infra.utils.browser_media import configure_browser_idle_timeout
...
def test_ensure_loopback_no_proxy_preserves_existing_entries(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    values = set(os.environ["NO_PROXY"].split(","))
    assert {"example.com", "localhost", "127.0.0.1", "::1"} <= values
...
def test_chrome_source_classifies_playwright_and_system(
    ...
    from artemis.infra.browser import setup as browser_setup
...
def test_temp_scope_token_uses_username_when_getuid_missing(
    ...
    monkeypatch.setenv("USERNAME", "ArtemisUser")
    monkeypatch.delenv("USER", raising=False)

    assert _temp_scope_token() == "ArtemisUser"
...
def test_resolve_browser_display_uses_x_socket(
    ...
    monkeypatch.setattr(
        "artemis.infra.desktop.setup._display_from_env_file",
        lambda: ":99",
    )
...
def test_artemis_browser_profiles_dir_shared(tmp_path: Path) -> None:
    from artemis.infra.utils.browser_media import (
        BROWSER_PROFILES_REL,
        artemis_browser_profiles_dir,
    )
    from artemis.infra.utils.paths import PathLayout

    paths = PathLayout(root=tmp_path / "artemis")
    dest = artemis_browser_profiles_dir(paths)
...
def test_configure_browser_profiles_dir_sets_env(
    ...
    from artemis.infra.utils.browser_media import configure_browser_profiles_dir
...
def test_legacy_profiles_migrated_once(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from artemis.infra.utils import browser_media as media
    from artemis.infra.utils.paths import PathLayout
    ...
    paths = PathLayout(root=tmp_path / "artemis")
    dest = media.artemis_browser_profiles_dir(paths)
    assert (dest / "default" / "Preferences").read_text(encoding="utf-8") == "{}"
    assert not legacy.exists()

    # Second call must not fail when legacy is empty/gone.
    media.artemis_browser_profiles_dir(paths)
    assert (dest / "default" / "Preferences").exists()


@posix_only
def test_under_root_home_exempts_artemis_home(monkeypatch: pytest.MonkeyPatch) -> None:
    from artemis.infra.browser import setup as browser_setup
    from artemis.infra.utils.paths import PathLayout
    ...
    def _fake_from_env() -> PathLayout:
        return PathLayout(root=Path("/root/.artemis"))

    monkeypatch.setattr(
        "artemis.infra.utils.paths.PathLayout.from_env",
        staticmethod(_fake_from_env),
    )

    assert browser_setup._under_root_home(Path("/root/.artemis/browser-profiles/default")) is False
