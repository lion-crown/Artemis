...
def test_default_agent_backend_spec_posix_uses_host_root(tmp_path: Path) -> None:
    ...
    with patch("artemis.infra.backend.resolver.os", SimpleNamespace(name="posix")):
...
def test_default_agent_backend_spec_windows_scopes_to_workspace(tmp_path: Path) -> None:
    ws = tmp_path / "agents" / "AGT001"
    ws.mkdir(parents=True)
    with patch("artemis.infra.backend.resolver.os", SimpleNamespace(name="nt")):
        spec = default_agent_backend_spec(ws)
    assert spec == {
        "type": "local_shell",
...
def test_resolve_composite_backend_normalizes_route_prefix() -> None:
    frontend = {"type": "filesystem", "root_dir": "/workspace/frontend", "virtual_mode": True}
    ...
    resolved = resolve_agent_backend_spec(
        {
            ...
            "routes": {"/workspace/frontend": frontend},
        },
        repo=None,
    )

    assert resolved["routes"] == {"/workspace/frontend/": frontend}
