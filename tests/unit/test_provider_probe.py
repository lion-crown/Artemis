...
from artemis.infra.agents.providers.probe import build_probe_chat_model as _build_chat_model
...
async def test_embedding_probe_posts_embeddings_endpoint() -> None:
    ...
    with (
        patch(
            "artemis.infra.agents.providers.probe.httpx.AsyncClient",
            return_value=_mock_async_client(post=post),
        ),
        patch("artemis.infra.agents.providers.probe.build_probe_chat_model") as chat,
    ):
...
async def test_chat_probe_maps_insufficient_balance() -> None:
    ...
    with patch(
        "artemis.infra.agents.providers.probe.build_probe_chat_model",
        return_value=fake,
    ):
...
async def test_chat_probe_skips_embeddings_endpoint() -> None:
    ...
    with (
        ...
        patch("artemis.infra.agents.providers.probe.httpx.AsyncClient") as client,
    ):
