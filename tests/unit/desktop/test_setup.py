...
from artemis.infra.desktop.setup import (
...
def test_friendly_install_log_line_maps_apt_lock() -> None:
    from artemis.i18n import tr
    from artemis.infra.desktop.setup import _friendly_install_log_line
...
def test_enterprise_linux_major_version(release: dict[str, str], expected: int | None) -> None:
    from artemis.infra.desktop.setup import _enterprise_linux_major_version
...
def test_desktop_status_rejects_fresh_el10_before_python_deps() -> None:
    ...
    with (
        patch("artemis.infra.desktop.setup.platform.system", return_value="Linux"),
        patch("artemis.infra.desktop.setup._python_deps_available", return_value=False),
        patch("artemis.infra.desktop.setup._unsupported_linux_desktop_reason", return_value=reason),
        patch(
            "artemis.infra.desktop.setup._resolve_linux_setup",
...
async def test_iter_subprocess_lines_handles_long_line_without_newline() -> None:
    from artemis.infra.desktop.setup import _iter_subprocess_lines
...
def test_sanitize_subprocess_log_filters_script_paths() -> None:
    assert (
        _sanitize_subprocess_log(
            "Running /workspace/src/artemis/infra/desktop/scripts/linux/v1.0/install.sh ..."
        )
        is None
    )
...
def test_desktop_uninstall_succeeded_when_deps_gone() -> None:
    with patch("artemis.infra.desktop.setup._python_deps_available", return_value=False):
...
def test_desktop_uninstall_succeeded_linux_stack_removed_with_host_ready() -> None:
    with (
        patch("artemis.infra.desktop.setup.platform.system", return_value="Linux"),
        patch("artemis.infra.desktop.setup._python_deps_available", return_value=True),
        patch("artemis.infra.desktop.setup._linux_virtual_desktop_present", return_value=False),
        patch(
            "artemis.infra.desktop.setup.desktop_status",
...
def test_desktop_uninstall_failed_linux_stack_remains() -> None:
    with (
        patch("artemis.infra.desktop.setup.platform.system", return_value="Linux"),
        patch("artemis.infra.desktop.setup._python_deps_available", return_value=True),
        patch("artemis.infra.desktop.setup._linux_virtual_desktop_present", return_value=True),
    ):
...
def test_allowed_script_path_rejects_outside_bundle(tmp_path, monkeypatch) -> None:
    from artemis.infra.desktop.setup import _allowed_script_path, bundled_scripts_dir
...
def test_script_path_from_env_rejects_outside_bundle(tmp_path, monkeypatch) -> None:
    from artemis.infra.desktop.setup import _script_path_from_env

    outside = tmp_path / "evil.sh"
    outside.write_text("#!/bin/bash\n", encoding="utf-8")
    monkeypatch.setenv("ARTEMIS_DESKTOP_INSTALL_SCRIPT", str(outside))
    assert _script_path_from_env("ARTEMIS_DESKTOP_INSTALL_SCRIPT") is None
...
def test_python_deps_install_cmd_prefers_uv(monkeypatch) -> None:
    monkeypatch.setattr(
        "artemis.infra.utils.runtime_packages.find_uv_binary",
        lambda: "/usr/bin/uv",
    )
    monkeypatch.setattr("artemis.infra.desktop.setup.sys.executable", "/venv/bin/python3")
...
def test_python_deps_install_cmd_falls_back_to_pip(monkeypatch) -> None:
    monkeypatch.setattr("artemis.infra.utils.runtime_packages.find_uv_binary", lambda: None)
    monkeypatch.setattr("artemis.infra.desktop.setup.sys.executable", "/venv/bin/python3")
    cmd = python_deps_install_cmd()
    assert cmd == [
        "/venv/bin/python3",
...
def test_desktop_status_deps_missing_darwin_omits_linux_cmds() -> None:
    with (
        patch("artemis.infra.desktop.setup._python_deps_available", return_value=False),
        patch("artemis.infra.desktop.setup.platform.system", return_value="Darwin"),
    ):
        status = desktop_status()
    assert status.setup_state == "deps_missing"
...
def test_desktop_status_darwin_missing_perms_not_ok() -> None:
    with (
        patch("artemis.infra.desktop.setup._python_deps_available", return_value=True),
        patch("artemis.infra.desktop.setup.platform.system", return_value="Darwin"),
        patch("artemis.infra.desktop.setup._mac_screen_recording_granted", return_value=False),
        patch("artemis.infra.desktop.setup._mac_accessibility_granted", return_value=True),
    ):
        status = desktop_status()
...
def test_desktop_status_ready_darwin_omits_linux_cmds() -> None:
    with (
        patch("artemis.infra.desktop.setup._python_deps_available", return_value=True),
        patch("artemis.infra.desktop.setup.platform.system", return_value="Darwin"),
        patch("artemis.infra.desktop.setup._mac_screen_recording_granted", return_value=True),
        patch("artemis.infra.desktop.setup._mac_accessibility_granted", return_value=True),
    ):
        status = desktop_status()
    assert status.setup_state == "ready"
...
def test_virtual_desktop_installed_requires_complete_stack() -> None:
    from artemis.infra.desktop.setup import _virtual_desktop_installed

    with (
        patch("artemis.infra.desktop.setup._runtime_scripts_present", return_value=True),
        patch("artemis.infra.desktop.setup.system_conf_dir") as conf,
        patch("artemis.infra.desktop.setup.desktop_env_file") as env,
        patch("artemis.infra.desktop.setup._systemd_available", return_value=True),
        patch("artemis.infra.desktop.setup._systemd_unit_files_present", return_value=False),
    ):
...
def test_virtual_desktop_installed_ok_with_units() -> None:
    ...
    with (
        ...
        patch("artemis.infra.desktop.setup.desktop_env_file") as env,
        patch("artemis.infra.desktop.setup._systemd_available", return_value=True),
        patch("artemis.infra.desktop.setup._systemd_unit_files_present", return_value=True),
    ):
        conf.return_value.is_dir.return_value = True
        env.return_value.is_file.return_value = False
...
def test_resolve_linux_partial_install_needs_reinstall() -> None:
    from artemis.infra.desktop.setup import _resolve_linux_setup

    with (
        patch("artemis.infra.desktop.setup._check_vnc_localhost", return_value=(None, "")),
        patch.dict("os.environ", {"DISPLAY": "", "WAYLAND_DISPLAY": ""}, clear=False),
        patch("artemis.infra.desktop.setup._display_from_env_file", return_value=None),
        patch("artemis.infra.desktop.setup._virtual_desktop_installed", return_value=False),
    ):
...
def test_bundled_install_script_exists() -> None:
    from artemis.infra.desktop.setup import bundled_scripts_dir, resolve_install_script_path
...
def test_python_deps_uninstall_cmd_prefers_uv(monkeypatch) -> None:
    monkeypatch.setattr(
        "artemis.infra.desktop.setup.shutil.which",
        lambda name: "/usr/bin/uv" if name == "uv" else None,
    )
    monkeypatch.setattr("artemis.infra.desktop.setup.sys.executable", "/venv/bin/python3")
    from artemis.infra.desktop.setup import python_deps_uninstall_cmd
