...
from artemis.infra.desktop.input import InputInjector
...
def test_virtual_display_uses_xdotool_for_click(monkeypatch) -> None:
    ...
    monkeypatch.setattr("artemis.infra.desktop.input.shutil.which", lambda name: "/usr/bin/xdotool")
    monkeypatch.setattr("artemis.infra.desktop.input._run_xdotool", fake_run)
...
def test_virtual_display_does_not_init_pynput_on_click(monkeypatch) -> None:
    _force_linux(monkeypatch)
    monkeypatch.setattr("artemis.infra.desktop.input.shutil.which", lambda name: "/usr/bin/xdotool")
    monkeypatch.setattr("artemis.infra.desktop.input._run_xdotool", lambda *_a, **_k: True)
