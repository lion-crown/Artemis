...
from artemis.infra.desktop.session import (
...
async def test_acquire_replaces_same_user_session(monkeypatch) -> None:
    monkeypatch.setattr("artemis.infra.desktop.session._MAX_DESKTOP_SESSIONS", 1)
...
async def test_supersede_calls_previous_disconnect() -> None:
    from artemis.infra.desktop.session import clear_user_stream, supersede_user_stream
...
async def test_disconnect_all_streams_closes_sessions(monkeypatch) -> None:
    from artemis.infra.desktop.session import disconnect_all_streams, supersede_user_stream

    monkeypatch.setattr("artemis.infra.desktop.session._MAX_DESKTOP_SESSIONS", 3)
...
async def test_release_ignores_stale_session_after_replace(monkeypatch) -> None:
    monkeypatch.setattr("artemis.infra.desktop.session._MAX_DESKTOP_SESSIONS", 2)
