...
from artemis.infra.desktop.capture import CaptureFrame, ScreenCapture


def _force_linux(monkeypatch) -> None:
    monkeypatch.setattr("artemis.infra.desktop.capture.sys.platform", "linux")
...
def test_prefer_import_on_linux_virtual_display(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "artemis.infra.desktop.capture.shutil.which",
...
def test_prefer_import_false_on_non_linux(monkeypatch) -> None:
    cap = ScreenCapture(display=":99")
    monkeypatch.setattr("artemis.infra.desktop.capture.sys.platform", "darwin")
...
def test_import_jpeg_passthrough_without_reencode(monkeypatch) -> None:
    # Avoid requiring artemis[desktop] / Pillow in default CI installs.
    ...
    monkeypatch.setattr("artemis.infra.desktop.capture.shutil.which", lambda name: "/usr/bin/import")
    monkeypatch.setattr("artemis.infra.desktop.capture.subprocess.Popen", lambda *a, **k: _Proc())
