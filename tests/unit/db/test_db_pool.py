...
def test_migration_005_preserves_populated_users_and_constraints(tmp_path: Path) -> None:
    ...
    pool = SqlitePool(db_path)
    initial_migration = (
        Path(__file__).resolve().parents[3] / "src/artemis/infra/db/migrations/001_initial.sql"
    )
    with pool.connect() as conn:
    ...
    with pool.connect() as conn:
        ...
        conn.execute(
            ...
            """,
            (1, 1, "SSO", "https://issuer.example", "artemis-client", "openid", 30, 30),
        )
        conn.execute(
            """
...
def test_v7_sql_files_declare_identity_rebuild() -> None:
    ...
    root = Path(__file__).resolve().parents[3] / "src/artemis/infra/db/migrations"
...
def test_v7_sqlite_sql_upgrades_legacy_text_pks(tmp_path: Path) -> None:
    ...
    db_path = tmp_path / "artemis.db"
    pool = SqlitePool(db_path)
    migrations = Path(__file__).resolve().parents[3] / "src/artemis/infra/db/migrations"
    with pool.connect() as conn:
