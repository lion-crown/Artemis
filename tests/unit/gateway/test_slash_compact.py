...
from artemis.infra.gateway.slash import BufferSink, SlashCommand, build_default_dispatcher
from artemis.infra.gateway.slash.ctx import SlashCtx
from artemis.infra.gateway.slash.handlers.composite import _short_offload_path, cmd_compact
...
def _agent_manager(tmp_path: Path, db: SqlitePool) -> AgentManager:
    services = build_shared_services(db=db, paths=PathLayout(tmp_path), config=ArtemisConfig())
...
@pytest.mark.asyncio
async def test_cmd_compact_shows_artemis_relative_offload_path(ctx, dispatcher) -> None:
    await ctx.thread_registry.get_or_create_by_key(
        session_key=ctx.session_key,
        agent_id=ctx.agent_id,
        user_id=ctx.user_id,
    ...
    harness.acompact_conversation = AsyncMock(
        return_value=CompactResult(
            ...
            summarized_count=3,
            preserved_count=4,
            file_path=f"/Users/wally/.artemis/workspaces/X/.artemis/conversation_history/{tid}.md",
            reason="ok",
        )
    )
    ctx.agent_manager.get_agent = MagicMock(return_value=harness)
    ...
    text = "\n".join(sink.lines)
    assert ".artemis/conversation_history/" in text
...
def test_short_offload_path_hides_windows_macos_and_linux_hosts() -> None:
    tid = "thr_win"
    cases = (
        rf"C:\Users\wally\.artemis\workspaces\X\.artemis\conversation_history\{tid}.md",
        rf"\\?\C:\Users\wally\.artemis\workspaces\X\.artemis\conversation_history\{tid}.md",
        f"/home/wally/.artemis/workspaces/X/.artemis/conversation_history/{tid}.md",
        f"/Users/wally/.artemis/workspaces/X/.artemis/conversation_history/{tid}.md",
        f"C:/Users/wally/.artemis/conversation_history/{tid}.md",
    )
    for raw in cases:
        got = _short_offload_path(raw)
        assert got == f".artemis/conversation_history/{tid}.md"
        assert "\\" not in got
        assert "C:" not in got
...
async def test_cmd_compact_shows_windows_offload_path(ctx, dispatcher) -> None:
    ...
    harness.acompact_conversation = AsyncMock(
        return_value=CompactResult(
            ...
            summarized_count=3,
            preserved_count=4,
            file_path=rf"C:\Users\wally\.artemis\workspaces\X\.artemis\conversation_history\{tid}.md",
            reason="ok",
        )
    )
    ctx.agent_manager.get_agent = MagicMock(return_value=harness)
...
async def test_cmd_compact_prefers_display_path_over_host_file_path(ctx, dispatcher) -> None:
    ...
    harness.acompact_conversation = AsyncMock(
        return_value=CompactResult(
            ...
            preserved_count=4,
            file_path=rf"C:\Users\wally\.artemis\workspaces\X\.artemis\conversation_history\{tid}.md",
            display_path=f".artemis/conversation_history/{tid}.md",
            reason="ok",
        )
    )
    ctx.agent_manager.get_agent = MagicMock(return_value=harness)
    ...
    text = "\n".join(sink.lines)
    assert f".artemis/conversation_history/{tid}.md" in text
    assert "C:" not in text
