...
@pytest.mark.asyncio
async def test_register_wecom_final_only_sends_the_primary_answer_directly(tmp_path: Path) -> None:
    gw = _make_gateway(tmp_path)
    gw._channel_manager = MagicMock()
    gw._channel_manager.add_channel = AsyncMock()
    gw._channel_manager.get_channel = MagicMock(return_value=MagicMock())
    ...
    row = _fake_row()
    row.kind = "wecom"
    row.config_json = '{"bot_id":"bot","secret":"secret","delivery_mode":"final_only"}'

    await gw._register_channel(row)

    channel = gw._channel_manager.add_channel.await_args.args[0]
    assert not hasattr(channel, "_reply_rewriter")


@pytest.mark.asyncio
async def test_register_business_system_qa_wecom_sends_primary_answer_directly(
    tmp_path: Path,
) -> None:
    gw = _make_gateway(tmp_path)
    gw._channel_manager = MagicMock()
    ...
    gw._channel_manager.get_channel = MagicMock(return_value=MagicMock())
    gw._processor = MagicMock()
    gw._agent_manager.get_config = MagicMock(
        return_value={"execution_profile": "business_system_qa"}
    )
    row = _fake_row()
    row.kind = "wecom"
    row.config_json = '{"bot_id":"bot","secret":"secret","delivery_mode":"final_only"}'
...
async def test_reload_channels_from_db_unregisters_old_and_registers_enabled(
    ...
    from artemis.infra.gateway.cli import CLI_CHANNEL_ID
