...
from artemis.infra.db.repos.thread_messages import ThreadMessageRepo
from artemis.infra.db.repos.threads import ThreadRepo
from artemis.infra.db.repos.trajectory_events import TrajectoryEventRepo
from artemis.infra.db.repos.users import UserRepo
from artemis.infra.gateway.process.history_projection import message_inputs
from artemis.infra.history.recorder import RecordingTracker
from artemis.infra.history.service import HistoryArchive
from artemis.infra.history.store import HistoryStore
from artemis.infra.history.trajectory import ArchiveTrajectoryStore
from artemis.infra.trajectory.live import TrajectoryLiveBus
...
async def test_streamed_archive_messages_include_checkpoint_ts(archive):
    from artemis.infra.gateway.process.message_keys import CHECKPOINT_TS_KEY
...
def test_backup_includes_archive_and_restore_refuses_partial_overwrite(archive, tmp_path):
    ...
    with pytest.raises(ArtemisError, match="offline restore"):
...
async def test_unsegmented_legacy_export_does_not_backfill(archive):
    ...
    from artemis.infra.history.reader import export_messages
...
async def test_dashboard_processor_writes_only_new_archive(archive):
    ...
    from artemis.infra.gateway.ws import WS_CHANNEL_ID
...
def test_rebind_is_rejected_before_closing_old_database(archive, monkeypatch):
    ...
    from artemis.infra.db.rebind import rebind_control_plane
    from artemis.infra.errors import ArtemisError
    ...
    monkeypatch.setattr("artemis.infra.db.rebind.open_database", open_database)
    with pytest.raises(ArtemisError, match="Restart"):
...
async def test_cursor_issued_before_first_switch_survives_new_turns(archive):
    ...
    from artemis.infra.history.reader import read_page
...
async def test_inline_thinking_is_not_duplicated_after_final_state(archive):
    from artemis.api.routers.chat.serialize import _serialize_history_message
...
async def test_dashboard_error_keeps_partial_tokens_and_error(archive):
    ...
    error = next(
        message for message in recovered if message.additional_kwargs.get("artemis_stream_error")
    )
...
async def test_im_projection_commits_back_to_back_fragments_before_delivery(archive):
    ...
    from artemis.infra.gateway.process.stream_project import project_stream
...
async def test_old_body_encoding_remains_readable_without_rewriting_on_resume(archive):
    ...
    from artemis.infra.history.store import dumps
