...
from artemis.infra.gateway.gateway import ChannelCreateSpec, Gateway
...
def _make_services(tmp_path: Path):
    ...
    run_migrations(db)
    return build_shared_services(db=db, paths=PathLayout(tmp_path), config=ArtemisConfig())
...
async def test_gateway_boot_and_shutdown(tmp_path: Path) -> None:
    ...
    with patch("artemis.infra.agents.manager.HarnessAgentManager") as mock_hm_cls:
...
@pytest.mark.asyncio
async def test_create_channel_rejects_removed_channel_kind(tmp_path: Path) -> None:
    services = _make_services(tmp_path)
    agent_id, user_id = _seed_agent(services, tmp_path)
    registry = AgentManager(
        repos=services.repos,
    ...
    assert exc.value.code is ErrorCode.CHANNEL_KIND_UNSUPPORTED
