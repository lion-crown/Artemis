...
deepagents filesystem tools under Artemis's default backend (root_dir='/').
"""
...
from artemis.api.common.attachments import save_attachment
from artemis.infra.agents.middleware.binary_read_guard import read_file_block_reason
from artemis.infra.backend.resolver import default_agent_backend_spec
from artemis.infra.gateway.media.attachment_hints import format_attachment_path_hint
from artemis.infra.gateway.media.inbound_store import resolve_inbound_attachment_path, write_inbound
from artemis.infra.gateway.media.ingress import AgentBackedMediaBackend
from artemis.infra.gateway.process.harness_request import build_content_from_message
from artemis.infra.gateway.threads import ThreadRegistry
from artemis.infra.gateway.ws import WS_CHANNEL_ID


def _artemis_default_agent_workspace(ws_dir: str) -> BackendWorkspace:
...
async def test_hint_remaps_scoped_root_to_agent_facing() -> None:
    with tempfile.TemporaryDirectory() as root_dir:
        host_ws = Path(root_dir) / ".artemis" / "workspaces" / "ABC123"
...
async def test_artemis_default_backend_tool_path_misses_uploaded_file() -> None:
    ...
    with tempfile.TemporaryDirectory() as ws_dir:
        ...
        # Artemis ingress layer can still read the same attachment
...
async def test_image_materialize_keeps_workspace_path_for_history() -> None:
    with tempfile.TemporaryDirectory() as ws_dir:
        ...
        from artemis.infra.gateway.media.attachment_hints import expand_workspace_image_ref
