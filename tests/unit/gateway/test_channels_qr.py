...
def _make_app(server, user):
    ...
    from artemis.api.routers import channels as ch_module
...
def test_wecom_qrcode_generate_returns_scode(mock_server_and_user):
    ...
    with patch(
        "artemis.infra.gateway.channels.qr_bind.httpx.AsyncClient", return_value=mock_async_client
    ):
...
def test_qq_qrcode_generate_returns_task(mock_server_and_user):
    ...
    server, user = mock_server_and_user
    with patch(
        "artemis.api.routers.channels.qr_bind.qq_qr_generate",
        new=AsyncMock(
...
def test_qq_qrcode_poll_returns_credentials(mock_server_and_user):
    ...
    with patch(
        "artemis.api.routers.channels.qr_bind.qq_qr_poll",
...
def test_dingtalk_qrcode_generate_keeps_device_code_server_side(mock_server_and_user):
    """Only an opaque Artemis registration ID and public QR metadata reach the browser."""
    server, user = mock_server_and_user
    from artemis.api.routers import channels as ch_module
    ...
    with patch(
        "artemis.api.routers.channels.dingtalk_registration.generate",
        new=AsyncMock(return_value=result),
    ):
...
def test_dingtalk_qrcode_poll_creates_channel_without_returning_secret(
    ...
    with (
        ...
        patch(
            "artemis.api.routers.channels.dingtalk_registration.poll",
...
def test_dingtalk_registration_client_uses_official_device_flow_endpoints():
    from artemis.infra.gateway.channels import dingtalk_registration
...
def test_pkill_chrome_profile_accepts_resolved_path(monkeypatch: pytest.MonkeyPatch, tmp_path):
    ...
    from artemis.api.routers.channels import _pkill_chrome_profile, _safe_profile_directory
    ...
    assert profile_dir == root / "artemis-feishu-bot"

    with patch("artemis.api.routers.channels.asyncio.to_thread", new_callable=AsyncMock):
        import asyncio
...
def test_feishu_bot_creator_start_without_pkill_mock(mock_server_and_user):
    ...
    with (
        patch("artemis.api.routers.channels.subprocess.Popen", return_value=mock_proc),
        patch(
            "artemis.api.routers.channels._bot_creator_script",
            return_value="/fake/feishu_bot_creator.py",
        ),
        patch("artemis.api.routers.channels.asyncio.to_thread", new_callable=AsyncMock),
        patch("artemis.api.routers.channels.subprocess.run") as mock_run,
    ):
