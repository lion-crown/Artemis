...
from artemis.infra.gateway.process.history_projection import _wire_text
from artemis.infra.gateway.process.processor import GlobalProcessor, _stream_error
...
async def test_iter_turn_chunks_persists_partial_output_and_error() -> None:
    ...
    assert any(
        json.loads(item.message_json)["data"]["additional_kwargs"].get("artemis_stream_error")
