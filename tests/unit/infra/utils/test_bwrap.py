...
from artemis.infra.utils import bwrap as bwrap_mod
from artemis.infra.utils.bwrap import ensure_bubblewrap


def test_ensure_skips_on_non_linux(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("artemis.infra.utils.bwrap.sys.platform", "darwin")
    monkeypatch.setattr("artemis.infra.utils.bwrap.shutil.which", lambda _n: None)
...
def test_ensure_skips_on_windows(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("artemis.infra.utils.bwrap.sys.platform", "win32")
    monkeypatch.setattr("artemis.infra.utils.bwrap.shutil.which", lambda _n: None)
    result = ensure_bubblewrap()
    assert result["status"] == "skipped"
    assert result["reason"] == "not_linux"
...
def test_ensure_ready_when_bwrap_present(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("artemis.infra.utils.bwrap.sys.platform", "linux")
    monkeypatch.setattr("artemis.infra.utils.bwrap.shutil.which", lambda _n: "/usr/bin/bwrap")
    result = ensure_bubblewrap()
...
def test_ensure_degraded_when_no_package_manager(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("artemis.infra.utils.bwrap.sys.platform", "linux")
    monkeypatch.setattr("artemis.infra.utils.bwrap.shutil.which", lambda _n: None)
    monkeypatch.setattr("artemis.infra.utils.bwrap._detect_package_manager", lambda: None)
    result = ensure_bubblewrap()
...
def test_ensure_degraded_when_no_privilege(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("artemis.infra.utils.bwrap.sys.platform", "linux")
    monkeypatch.setattr("artemis.infra.utils.bwrap.shutil.which", lambda _n: None)
    monkeypatch.setattr("artemis.infra.utils.bwrap._detect_package_manager", lambda: "apt")
    monkeypatch.setattr("artemis.infra.utils.bwrap._can_install_without_password", lambda: False)
    result = ensure_bubblewrap()
    assert result["status"] == "degraded"
...
def test_ensure_installed_after_successful_install(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("artemis.infra.utils.bwrap.shutil.which", which)
    monkeypatch.setattr("artemis.infra.utils.bwrap._detect_package_manager", lambda: "apt")
    monkeypatch.setattr("artemis.infra.utils.bwrap._can_install_without_password", lambda: True)
    install = MagicMock(return_value=True)
    monkeypatch.setattr("artemis.infra.utils.bwrap._install_bubblewrap", install)
    result = ensure_bubblewrap()
...
def test_ensure_degraded_when_install_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("artemis.infra.utils.bwrap.sys.platform", "linux")
    monkeypatch.setattr("artemis.infra.utils.bwrap.shutil.which", lambda _n: None)
    monkeypatch.setattr("artemis.infra.utils.bwrap._detect_package_manager", lambda: "dnf")
    monkeypatch.setattr("artemis.infra.utils.bwrap._can_install_without_password", lambda: True)
    monkeypatch.setattr("artemis.infra.utils.bwrap._install_bubblewrap", lambda _mgr: False)
    result = ensure_bubblewrap()
    assert result["status"] == "degraded"
...
def test_install_argv_shapes(
    ...
    monkeypatch.setattr(
        "artemis.infra.utils.bwrap.shutil.which",
