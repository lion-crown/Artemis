...
from artemis.infra.utils.host_dirs import (
...
def test_assert_safe_host_path_rejects_root(
    ...
    # Deny /root only when it is not the process home (non-root Artemis).
    home = tmp_path / "os_home"
    home.mkdir()
    monkeypatch.setattr("artemis.infra.utils.host_dirs.Path.home", lambda: home)
    with pytest.raises(ValueError, match="not allowed"):
...
def test_assert_safe_host_path_allows_root_when_home(
    ...
    """Artemis running as uid 0 uses /root as home; default root_dir must probe OK."""
    root_home = Path("/root")
    if not root_home.is_dir():
        pytest.skip("/root not available")
    monkeypatch.setattr("artemis.infra.utils.host_dirs.Path.home", lambda: root_home)
    assert assert_safe_host_path("/root") == root_home.resolve()
    nested = root_home / ".artemis"
...
def test_host_fs_tree_root_admin_posix(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    home = Path("/tmp/artemis-home-probe")
    monkeypatch.setattr("artemis.infra.utils.host_dirs.Path.home", lambda: home)
    from artemis.infra.utils.host_dirs import host_fs_tree_root
