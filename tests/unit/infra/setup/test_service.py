"""Tests for artemis.infra.setup.service."""
...
from artemis.infra.setup import service as service_mod
...
from artemis.infra.utils import posix_compat as pwd

pytestmark = pytest.mark.skipif(os.name != "posix", reason="POSIX service management")


def _runtime(tmp_path: Path) -> ServiceRuntime:
    artemis_bin = tmp_path / "bin" / "artemis"
    artemis_bin.parent.mkdir(parents=True)
    artemis_bin.write_text("#!/bin/sh\n", encoding="utf-8")
...
def test_resolve_artemis_executable_falls_back_to_path(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    fake_artemis = tmp_path / "fake-artemis"
    fake_artemis.write_text("#!/bin/sh\n", encoding="utf-8")
    python_dir = tmp_path / "python-only"
    python_dir.mkdir()
    python = python_dir / "python"
    python.write_text("#!/bin/sh\n", encoding="utf-8")

    monkeypatch.setenv("ARTEMIS_HOME", str(tmp_path / "empty-home"))
    monkeypatch.setattr(sys, "executable", str(python))
    monkeypatch.setattr(shutil, "which", lambda _name: str(fake_artemis))

    assert service_mod.resolve_artemis_executable() == fake_artemis


def test_render_systemd_unit_user_scope(tmp_path: Path) -> None:
    runtime = _runtime(tmp_path)
    unit = render_systemd_unit(runtime, user_unit=True)
    assert "ARTEMIS_SERVICE_MODE=systemd" in unit
    assert "ARTEMIS_SYSTEMD_USER=1" in unit
    assert "WantedBy=default.target" in unit
    assert f"User={getpass.getuser()}" not in unit
    assert f"ExecStart={runtime.artemis_bin} run\n" in unit
...
def test_render_systemd_unit_system_scope(tmp_path: Path) -> None:
    ...
    assert "ARTEMIS_SYSTEMD_USER=1" not in unit
...
def test_render_systemd_unit_system_scope_uses_runtime_scope_by_default(
    ...
    assert f"Environment=ARTEMIS_HOME={tmp_path}" in unit
...
def test_install_service_systemd_user_uses_systemctl_user(
    ...
    assert calls[1] == ["enable", "artemis"]


def test_install_service_does_not_rewrite_custom_unit_without_force(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """Hand-edited ExecStart/User must survive `artemis service start/restart`."""
    runtime = replace(_runtime(tmp_path), scope="user")
    writes: list[object] = []
    calls: list[list[str]] = []
...
def test_ensure_nofile_dropin_is_idempotent(
    ...
    runtime = replace(_runtime(tmp_path), scope="user")
    dropin = tmp_path / "artemis.service.d" / service_mod.NOFILE_DROPIN_NAME
...
def test_start_service_restarts_systemd_when_apply_unit(
    ...
    assert calls == [["restart", "artemis"]]

    calls.clear()
    service_mod.start_service(runtime, apply_unit=False)
    assert calls == [["start", "artemis"]]
...
def test_restart_service_refreshes_unit(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert ["restart", "artemis"] in calls
    assert calls.index(["restart", "artemis"]) > calls.index(["daemon-reload"])
...
def test_render_launchd_plist_includes_service_mode(tmp_path: Path) -> None:
    ...
    assert "<key>ARTEMIS_SERVICE_MODE</key>" in plist
...
def test_install_service_launchd_force_reload(
    ...
    assert ["launchctl", "bootout", "system/artemis"] in calls
...
def test_install_service_launchd_force_reload_tolerates_unloaded(
    ...
    assert any(cmd[:3] == ["launchctl", "bootout", "system/artemis"] for cmd in calls)
...
def test_resolve_service_scope_honours_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ARTEMIS_SERVICE_SCOPE", "user")
    assert resolve_service_scope() == "user"
    monkeypatch.setenv("ARTEMIS_SERVICE_SCOPE", "system")
    assert resolve_service_scope() == "system"
    monkeypatch.setenv("ARTEMIS_SERVICE_SCOPE", "auto")
...
def test_resolve_service_scope_rejects_garbage(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("ARTEMIS_SERVICE_SCOPE", "nonsense")
...
def test_launchd_domain_user_vs_system() -> None:
    ...
    assert user.endswith("/artemis")
    assert launchd_domain("system") == "system/artemis"
...
def test_unit_path_launchd_user_is_under_home(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert path == owner_home / "Library" / "LaunchAgents" / "artemis.plist"
...
def test_unit_path_launchd_system_is_under_library(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("ARTEMIS_SERVICE_SCOPE", "system")
    path = unit_path("launchd")
    assert path == Path("/Library/LaunchDaemons/artemis.plist")
...
def test_resolve_service_home_uses_passwd_not_effective_uid(
    ...
    """sudo install as root must still target the invoking user's ~/.artemis."""
    monkeypatch.delenv("ARTEMIS_HOME", raising=False)
    monkeypatch.setattr(service_mod, "is_root", lambda: True)
    monkeypatch.setenv("SUDO_USER", "ubuntu")
    assert resolve_service_home(run_as_user="ubuntu") == Path("/home/ubuntu") / ".artemis"


def test_resolve_service_home_root_session(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("ARTEMIS_HOME", raising=False)
    assert resolve_service_home(run_as_user="root") == Path(pwd.getpwnam("root").pw_dir) / ".artemis"
...
def test_build_runtime_root_always_system_scope(
    ...
    root_home.mkdir()
    artemis_bin = root_home / ".artemis" / "bin" / "artemis"
    artemis_bin.parent.mkdir(parents=True)
    artemis_bin.write_text("#!/bin/sh\n", encoding="utf-8")
...
def test_systemctl_user_wraps_xdg_runtime_dir_for_root_install(
    ...
    cmd = service_mod._systemctl("user", "enable", "artemis", run_as_user="ubuntu")
    assert cmd == [
        "sudo",
        "-u",
        "ubuntu",
...
def test_build_runtime_picks_user_scope_for_non_root(
    ...
    monkeypatch.setenv("ARTEMIS_SERVICE_SCOPE", "auto")
    (tmp_path / "bin").mkdir()
    (tmp_path / "bin" / "artemis").write_text("#!/bin/sh\n", encoding="utf-8")
...
def test_needs_sudo_false_when_an_ancestor_is_writable(tmp_path: Path) -> None:
    ...
    nested = tmp_path / "systemd" / "user" / "artemis.service.d" / "10-nofile.conf"
