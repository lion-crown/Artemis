...
def test_saved_env_file_is_owner_only(tmp_path: Path) -> None:
    path = tmp_path / "env"
...
def test_overlay_stdio_spec_env_skips_protected() -> None:
    from artemis.infra.utils.env_file import overlay_stdio_spec_env

    spec = overlay_stdio_spec_env(
        {"transport": "stdio", "command": "npx", "env": {"SPEC": "1"}},
        {"TAVILY_API_KEY": "tvly", "ARTEMIS_HOME": "/nope", "HOME": "/hack"},
    )
    assert spec["env"]["TAVILY_API_KEY"] == "tvly"
    assert spec["env"]["SPEC"] == "1"
    assert "ARTEMIS_HOME" not in spec["env"]
    assert "HOME" not in spec["env"]


def test_overlay_stdio_mcp_configs_applies_to_stdio_only() -> None:
    from artemis.infra.utils.env_file import overlay_stdio_mcp_configs
