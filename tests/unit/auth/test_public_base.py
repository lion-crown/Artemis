...
def test_resolve_public_base_uses_forwarded_proxy_headers() -> None:
    request = _request(
        headers=[
            ...
            (b"x-forwarded-host", b"artemis.example"),
        ]
    )

    assert resolve_public_base(request) == "https://artemis.example"


def test_resolve_public_base_uses_request_url_without_proxy_headers() -> None:
    request = _request(scheme="https", headers=[(b"host", b"artemis.example")])
...
def test_build_redirect_uri_uses_oidc_callback_path() -> None:
    assert (
        build_redirect_uri("https://artemis.example/")
        == "https://artemis.example/api/auth/oidc/callback"
    )
