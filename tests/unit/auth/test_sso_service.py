...
from artemis.infra.auth.sso.crypto import decrypt_secret
...
from artemis.infra.users.identity import Role, User
...
def discovery_client(handler):
    ...
    return patch("artemis.infra.auth.sso.service.httpx.Client", side_effect=factory)
...
def test_put_config_encrypts_with_supplied_secret_repo_and_redacts_for_admin(
    ...
    with patch(
        "artemis.infra.auth.sso.service.encrypt_secret", return_value=b"ciphertext"
    ) as encrypt:
    ...
    assert service.get_config_for_admin(public_base="https://artemis.example") == {
        "enabled": True,
        "display_name": "Company Login",
        "issuer": "https://issuer.example/",
        "client_id": "client-id",
        ...
        "redirect_uri": "https://artemis.example/api/auth/oidc/callback",
    }
...
def test_start_login_when_disabled_raises(service: SsoService) -> None:
    with pytest.raises(ValueError, match="disabled"):
        service.start_login(redirect_after="/chat", public_base="https://artemis.example")
...
def test_start_login_persists_pkce_state_and_uses_s256(service: SsoService) -> None:
    ...
    with discovery_client(handler):
        started = service.start_login(
            redirect_after="https://attacker.example", public_base="https://artemis.example"
        )
    ...
    assert query["redirect_uri"] == "https://artemis.example/api/auth/oidc/callback"
...
async def test_callback_maps_denied_bad_state_and_invalid_token(service: SsoService) -> None:
    denied = await service.handle_callback(
        code=None, state=None, error="access_denied", public_base="https://artemis.example"
    )
    bad_state = await service.handle_callback(
        code="code", state="unknown", error=None, public_base="https://artemis.example"
    )
    assert denied.url == "https://artemis.example/login?oidc_error=denied"
    assert bad_state.url == "https://artemis.example/login?oidc_error=state"
    ...
    with patch.object(service, "_exchange_claims", side_effect=jwt.InvalidTokenError):
        invalid = await service.handle_callback(
            code="code", state="valid-state", error=None, public_base="https://artemis.example"
        )
    assert invalid.url == "https://artemis.example/login?oidc_error=invalid_token"
...
async def test_callback_exchanges_code_and_attaches_login_code(service: SsoService) -> None:
    ...
    with (
        ...
        patch(
            "artemis.infra.auth.sso.service.verify_id_token",
            return_value={
                "sub": "subject-123",
        ...
        started = service.start_login(
            redirect_after="/settings", public_base="https://artemis.example"
        )
        ...
        result = await service.handle_callback(
            ...
            state=state,
            error=None,
            public_base="https://artemis.example",
        )

    assert result.url.startswith("https://artemis.example/login/oidc/complete#code=")
...
async def test_callback_verifies_id_token_against_discovery_issuer(
    ...
    with (
        ...
        started = service.start_login(redirect_after="/chat", public_base="https://artemis.example")
        state = httpx.QueryParams(started["authorization_url"].split("?", 1)[1])["state"]
        result = await service.handle_callback(
            code="authorization-code",
            state=state,
