...
from artemis.config import load_config, parse_database_config
...
def test_defaults_when_missing(tmp_path: Path):
    ...
    assert cfg.database.sqlite_path == "artemis.db"
...
def test_backup_env_overrides(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("ARTEMIS_BACKUP_AUTO_ENABLED", "true")
    monkeypatch.setenv("ARTEMIS_BACKUP_SCHEDULE", "cron:30 5 * * *")
    monkeypatch.setenv("ARTEMIS_BACKUP_RETENTION_COUNT", "5")
    monkeypatch.setenv("ARTEMIS_BACKUP_INCLUDE_CHATS", "true")
    monkeypatch.setenv("ARTEMIS_BACKUP_INCLUDE_CONFIG", "false")
    monkeypatch.setenv("ARTEMIS_BACKUP_INCLUDE_WORKSPACES", "false")
    monkeypatch.setenv("ARTEMIS_BACKUP_INCLUDE_SKILL_PACKAGES", "false")
    monkeypatch.setenv("ARTEMIS_BACKUP_INCLUDE_PLUGINS", "false")
    monkeypatch.setenv("ARTEMIS_BACKUP_INCLUDE_KNOWLEDGE", "false")
...
def test_max_upload_mb_env_override(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("ARTEMIS_MAX_UPLOAD_MB", "25")
    cfg_path = tmp_path / "config.json"
    cfg_path.write_text(json.dumps({"max_upload_mb": 50}))
    cfg = load_config(cfg_path)
...
def test_env_overrides(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("ARTEMIS_PORT", "7000")
    monkeypatch.setenv("ARTEMIS_LOG_LEVEL", "warning")
    monkeypatch.setenv("ARTEMIS_BROWSER_IDLE_TIMEOUT_MINUTES", "12")
    cfg_path = tmp_path / "config.json"
...
def test_database_env_sqlite_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("ARTEMIS_DATABASE_SQLITE_PATH", "/tmp/artemis-test.db")
    cfg = load_config(tmp_path / "config.json")
    assert cfg.database.sqlite_path == "/tmp/artemis-test.db"
    resolved = cfg.database.resolve_sqlite_path(tmp_path)
    assert resolved.is_absolute()
    assert resolved.as_posix().endswith("/tmp/artemis-test.db")


def test_database_env_url_postgresql(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("ARTEMIS_DATABASE_URL", "postgresql://alice:secret@db.example.com:5433/mydb")
    cfg = load_config(tmp_path / "config.json")
...
def test_feature_flags_env_overrides(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("ARTEMIS_ENABLE_DASHBOARD", "false")
    monkeypatch.setenv("ARTEMIS_ENABLE_API_DOCS", "true")
    monkeypatch.setenv("ARTEMIS_REQUIRE_SETUP_PASSWORD", "0")
    cfg = load_config(tmp_path / "config.json")
...
def test_default_timezone_env_override(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("ARTEMIS_DEFAULT_TIMEZONE", "Pacific/Auckland")
    cfg_path = tmp_path / "config.json"
...
def test_legacy_cron_timezone_env_still_works(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.delenv("ARTEMIS_DEFAULT_TIMEZONE", raising=False)
    monkeypatch.setenv("ARTEMIS_CRON_TIMEZONE", "Asia/Tokyo")
    cfg_path = tmp_path / "config.json"
    cfg_path.write_text(json.dumps({"default_timezone": "UTC"}))
    cfg = load_config(cfg_path)
...
def test_new_timezone_env_wins_over_legacy_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("ARTEMIS_DEFAULT_TIMEZONE", "UTC")
    monkeypatch.setenv("ARTEMIS_CRON_TIMEZONE", "Asia/Tokyo")
    cfg = load_config(tmp_path / "config.json")
    assert cfg.default_timezone == "UTC"
