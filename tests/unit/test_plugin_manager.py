"""Unit tests for Artemis plugin manager."""
...
from artemis.infra.agents.plugins.manager import (
    PluginManager,
...
def test_normalize_github_blob_url() -> None:
    blob = "https://github.com/veenyi/artemis-plugins/blob/main/artemis-toolkit.zip"
    assert (
        normalize_plugin_download_url(blob)
        == "https://raw.githubusercontent.com/veenyi/artemis-plugins/main/artemis-toolkit.zip"
    )
    raw_page = "https://github.com/veenyi/artemis-plugins/raw/main/artemis-toolkit.zip"
    assert (
        normalize_plugin_download_url(raw_page)
        == "https://raw.githubusercontent.com/veenyi/artemis-plugins/main/artemis-toolkit.zip"
    )
    already_raw = "https://raw.githubusercontent.com/veenyi/artemis-plugins/main/artemis-toolkit.zip"
...
def test_install_url_rejects_non_zip(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "artemis.infra.agents.plugins.manager.urllib.request.urlretrieve",
        fake_retrieve,
    )
