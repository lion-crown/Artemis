...
from artemis.cli.support.stub import EXIT_NOT_APPLICABLE, not_applicable


def test_not_applicable_exits_with_code_2(capsys: pytest.CaptureFixture[str]) -> None:
    with pytest.raises(SystemExit) as exc:
        not_applicable("Artemis has no embedding subsystem")
    assert exc.value.code == EXIT_NOT_APPLICABLE
    err = capsys.readouterr().err
    assert "Not applicable for Artemis" in err
    assert "Artemis has no embedding subsystem" in err
