...
def test_set_skill_enabled_persists_disabled_list(tmp_artemis_home: Path) -> None:
    from artemis.cli.support.db import open_cli_services
    from artemis.cli.support.skills import set_skill_enabled

    _bootstrap()
    with open_cli_services(home=tmp_artemis_home) as svc:
...
def test_list_skills_offline_delegates_to_embedded_server() -> None:
    from artemis.cli.support import skills as skills_mod
...
def test_skills_list_cli_no_login(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "artemis.cli.support.skills.list_skills_offline",
