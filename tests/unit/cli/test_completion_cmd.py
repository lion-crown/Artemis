...
def test_completion_show_emits_script(shell: str) -> None:
    ...
    assert "_ARTEMIS_COMPLETE" in result.output
...
def test_completion_install_appends_eval_line_idempotent(
    ...
    assert "_ARTEMIS_COMPLETE" in contents
    line_count_first = contents.count("_ARTEMIS_COMPLETE")

    r2 = runner.invoke(cli, ["completion", "install", "--shell", "bash", "--rc-file", str(rc)])
    assert r2.exit_code == 0
    assert rc.read_text().count("_ARTEMIS_COMPLETE") == line_count_first
