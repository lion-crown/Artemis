...
Reported on Chinese Windows (GBK / cp936): ``artemis init`` bootstraps the
...
from artemis.cli.main import _ensure_utf8_stdio
...
def test_ensure_utf8_stdio_reconfigures_gbk_streams() -> None:
    ...
    out.write("\u2705 Artemis bootstrapped")
    out.flush()
...
def test_init_completes_with_gbk_stdout(tmp_path: Path) -> None:
    """``artemis init`` still bootstraps when stdout is forced to GBK. The crash
    ...
    assert (home / "artemis.db").is_file()
