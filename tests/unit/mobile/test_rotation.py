...
from artemis.infra.mobile.adb import (
...
def test_get_user_rotation_parses_value() -> None:
    with patch(
        "artemis.infra.mobile.adb.shell",
        return_value=(0, "1"),
    ):
...
def test_get_display_rotation_parses_window_manager() -> None:
    ...
    with patch("artemis.infra.mobile.adb.shell", return_value=(0, dumpsys)):
...
def test_set_user_rotation_success_when_display_matches() -> None:
    ...
    with patch("artemis.infra.mobile.adb.shell", side_effect=fake_shell):
...
def test_set_user_rotation_emulator_reports_unsupported() -> None:
    ...
    with (
        patch("artemis.infra.mobile.adb.shell", side_effect=fake_shell),
        patch("artemis.infra.mobile.adb._try_emulator_rotate"),
    ):
...
def test_toggle_portrait_landscape_from_portrait() -> None:
    with (
        patch("artemis.infra.mobile.adb.get_display_rotation", return_value=0),
        patch("artemis.infra.mobile.adb.set_user_rotation") as set_rot,
    ):
        from artemis.infra.mobile.adb import RotationResult
...
def test_toggle_portrait_landscape_from_landscape() -> None:
    with (
        patch("artemis.infra.mobile.adb.get_display_rotation", return_value=1),
        patch("artemis.infra.mobile.adb.set_user_rotation") as set_rot,
    ):
        from artemis.infra.mobile.adb import RotationResult
