...
from artemis.infra.mobile.adb import find_adb


def test_find_adb_prefers_path() -> None:
    with (
        patch("artemis.infra.mobile.adb.shutil.which", return_value="/usr/bin/adb"),
...
def test_find_adb_from_android_home(tmp_path: Path) -> None:
    ...
    with (
        patch("artemis.infra.mobile.adb.shutil.which", return_value=None),
        patch.dict(
            "os.environ",
