...
from artemis.infra.mobile.tools import build_mobile_tools
...
def _enabled_config() -> ArtemisConfig:
...
def test_build_mobile_tools_registers_when_adb_present() -> None:
    user_repo = MagicMock()
    with patch("artemis.infra.mobile.tools.find_adb", return_value="/adb"):
...
async def test_mobile_tap_requires_permission() -> None:
    ...
    with (
        ...
        patch("artemis.infra.mobile.tools.mobile_status") as status,
    ):
...
async def test_mobile_tap_success() -> None:
    from artemis.infra.mobile.agent_control import set_mobile_agent_control
    ...
    try:
        with (
            ...
            patch("artemis.infra.mobile.tools.mobile_status") as status,
            patch("artemis.infra.mobile.tools.list_devices", return_value=["emulator-5554"]),
            patch("artemis.infra.mobile.tools.tap", return_value=True) as tap_fn,
        ):
            status.return_value = MagicMock(setup_state="ready", ok=True)
