...
from artemis.infra.mobile.probe import probe_host_capability
...
def test_probe_host_matrix(
    ...
    with (
        patch("artemis.infra.mobile.probe.platform.system", return_value=system),
        patch("artemis.infra.mobile.probe.linux_binder_available", return_value=binder),
        patch("artemis.infra.mobile.probe.kvm_available", return_value=kvm),
    ):
