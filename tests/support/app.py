"""Application server + ASGI client lifecycle for integration tests."""
...
from artemis.infra.db.rebind import persist_database_config
from artemis.infra.server import ArtemisServer
from tests.support.harness import patch_harness


def write_artemis_config(home: Path, **overrides: object) -> None:
...
async def ensure_control_plane_bound(srv: ArtemisServer) -> None:
...
@asynccontextmanager
async def artemis_client(
    home: Path,
    *,
    fake_agent: Any | None = None,
    patch_llm: bool = True,
    bind_database: bool = True,
) -> AsyncIterator[tuple[httpx.AsyncClient, ArtemisServer]]:
    """Start the application server, yield ``(httpx client, server)``, then stop.
    ...
    with ctx:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                client._artemis_app = app  # type: ignore[attr-defined]
                yield client, srv
        finally:
            await srv.stop()
