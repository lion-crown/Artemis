"""Reusable application test doubles."""
...
class FakeHarnessAgent:
    ...
    def __init__(
        ...
        *,
        ...
        if workspace_dir is not None:
            ...
            self._workspace_dir = Path(tempfile.mkdtemp(prefix="artemis-fake-ws-"))
    ...
    def use_workspace_dir(
        ...
        *,
        ...
        ``virtual_mode=False`` mirrors production's host-rooted default. Tests
        may opt into ``virtual_mode=True`` to mirror a named filesystem storage
        backend, where ``/`` is the configured workspace root.
        """
...
"""
    ...
    async def list_subagent_summaries(self) -> list[dict[str, Any]]:
        ...
        paths: set[str] = set()
        for root in ("agents", ".artemis/agents"):
    ...
    async def list_skill_summaries(self) -> list[dict[str, Any]]:
        ...
        for root in ("skills", "_builtin_skills", ".artemis/skills", ".artemis/_builtin_skills"):
...
class _FakeBackend:
    """In-memory backend supporting the harness BackendProtocol surface used by API routers."""
