...
Bypasses ArtemisServer / DB / auth — we override the FastAPI deps used
...
from artemis.api.routers import memory


class _StubPaths:
    """Stub of ``ArtemisServer.services.paths`` with ensure_agent_workspace."""
...
def _build_app() -> FastAPI:
    artemis_home = Path.home() / ".artemis"
    server = _StubServer(_StubServices(_StubPaths(artemis_home / "agents")))

    app = FastAPI()
    app.include_router(memory.router, prefix="/api")

    # ArtemisError → JSON envelope (production app does this in app.py).
    ...
    async def _artemis_error_handler(_req: Request, exc: ArtemisError):
    ...
    import artemis.api.common.memory_client as memory_client
