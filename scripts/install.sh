#!/usr/bin/env bash
# Artemis 安装脚本 (macOS / Linux)
# 用法: bash scripts/install.sh              # 从 PyPI 安装（默认）
#   或: bash scripts/install.sh --from-source  # 从本地源码安装
#   或: curl -fsSL <url>/install.sh | bash   # 远程安装
#
# 将 Artemis 安装到 ~/.artemis，使用 uv 管理 Python 环境。
# 用户无需预先安装 Python — uv 会处理一切。
# 安装后会尽量把 artemis 链接到已在 PATH 中的目录（如 /usr/local/bin），
# 当前终端无需 source / 重开即可直接使用。
set -euo pipefail

# ── 默认配置 ──────────────────────────────────────────────────────────────────
ARTEMIS_HOME="${ARTEMIS_HOME:-$HOME/.artemis}"
ARTEMIS_VENV="$ARTEMIS_HOME/venv"
ARTEMIS_BIN="$ARTEMIS_HOME/bin"
PYTHON_VERSION="3.12"
ARTEMIS_REPO="${ARTEMIS_REPO:-https://github.com/TencentCloud/Artemis.git}"
_ARTEMIS_REPO_BASE="${ARTEMIS_REPO%/*}"
HARNESS_AGENT_REPO="${HARNESS_AGENT_REPO:-${_ARTEMIS_REPO_BASE}/harness-agent.git}"
HARNESS_GATEWAY_REPO="${HARNESS_GATEWAY_REPO:-${_ARTEMIS_REPO_BASE}/harness-gateway.git}"
HARNESS_BROWSER_REPO="${HARNESS_BROWSER_REPO:-${_ARTEMIS_REPO_BASE}/harness-browser.git}"
...
info()  { printf "${GREEN}[Artemis]${RESET} %s\n" "$*"; }
warn()  { printf "${YELLOW}[Artemis]${RESET} %s\n" "$*"; }
error() { printf "${RED}[Artemis]${RESET} %s\n" "$*" >&2; }
die()   { error "$@"; exit 1; }
...
Environment variables:
  ARTEMIS_HOME              Install directory (default: ~/.artemis)
  ARTEMIS_PYPI_MIRROR       PyPI mirror URL (same as --mirror)
  ARTEMIS_REPO              Git URL to clone (used by --from-source with no local dir)
  HARNESS_AGENT_REPO      harness-agent repo (derived from ARTEMIS_REPO by default)
  HARNESS_GATEWAY_REPO    harness-gateway repo (derived from ARTEMIS_REPO by default)
...
Details:
  Everything is installed into an isolated virtualenv (~/.artemis/venv), so the
...
printf "${GREEN}[Artemis]${RESET} Installing Artemis into ${BOLD}%s${RESET}\n" "$ARTEMIS_HOME"
...
_uv_pip_install_with_mirror_fallback() {
    ...
    for mirror in "${candidates[@]}"; do
        ...
        local -a args=(
            --python "$ARTEMIS_VENV/bin/python"
            --quiet
...
_python_dev_headers_ok() {
    local py=""
    if [ -x "${ARTEMIS_VENV:-}/bin/python" ]; then
        py="$ARTEMIS_VENV/bin/python"
...
if [ -x "$ARTEMIS_VENV/bin/python" ]; then
    ...
    uv venv "$ARTEMIS_VENV" --python "$PYTHON_VERSION" --quiet --seed
fi
[ -x "$ARTEMIS_VENV/bin/python" ] || die "Failed to create the virtualenv"
info "Python environment ready ($("$ARTEMIS_VENV/bin/python" --version))"
...
_EXTRA_MIRROR="${PYPI_MIRROR:-${ARTEMIS_PYPI_MIRROR:-}}"
...
prepare_console() {
    local repo_dir="$1"
    local console_dest="$repo_dir/src/artemis/dashboard"
...
# 旧版 harness / 镜像滞后的安装路径。待 Artemis Python 分发包更新后可删除。
_pin_mcp_compat() {
...
_verify_install() {
    ...
    if ! "$ARTEMIS_VENV/bin/python" -c "from artemis.infra.agents.manager import AgentManager" 2>/dev/null; then
...
_clone_source_workspace() {
    ...
    mkdir -p "$workdir"
    info "Cloning harness-agent / harness-gateway / Artemis sources..."
    ...
    git clone --depth 1 "$ARTEMIS_REPO" "$workdir/orca"
}
...
if [ "$FROM_SOURCE" = true ]; then
    if [ -n "$SOURCE_DIR" ]; then
        ...
        uv pip install "${SOURCE_DIR}${EXTRAS_SUFFIX}" --python "$ARTEMIS_VENV/bin/python"
    else
        ...
        uv pip install "${REPO_DIR}${EXTRAS_SUFFIX}" --python "$ARTEMIS_VENV/bin/python"
    fi
else
    # PEP 508: extras 必须在包名与版本说明符之间（artemis[browser]==x.y.z），
    # 不能拼在版本号后面（artemis==x.y.z[browser] 是非法需求串，uv/pip 解析报错）。
    PACKAGE="artemis${EXTRAS_SUFFIX}"
    [ -n "$VERSION" ] && PACKAGE="artemis${EXTRAS_SUFFIX}==$VERSION"
...
[ -x "$ARTEMIS_VENV/bin/artemis" ] || die "Install failed: artemis CLI not found in the virtualenv"
info "Artemis installed successfully"

if [ "$_CONSOLE_AVAILABLE" = 0 ]; then
    CONSOLE_CHECK="$("$ARTEMIS_VENV/bin/python" -c "import importlib.resources, artemis; p=importlib.resources.files('artemis')/'dashboard'/'index.html'; print('yes' if p.is_file() else 'no')" 2>/dev/null || echo 'no')"
...
_install_playwright_system_deps() {
    ...
    if command -v apt-get &>/dev/null || command -v apt &>/dev/null; then
        ...
        if "$ARTEMIS_VENV/bin/python" -m playwright install-deps chromium --with-deps 2>/dev/null; then
            return
        fi
...
_install_playwright_browsers() {
    ...
    _run_playwright_chromium_install() {
        if command -v timeout &>/dev/null; then
            timeout "$_pw_timeout" "$ARTEMIS_VENV/bin/python" -m playwright install chromium
        else
            "$ARTEMIS_VENV/bin/python" -m playwright install chromium
        fi
    }
    ...
    warn "    $ARTEMIS_VENV/bin/python -m playwright install chromium"
    return 1
}
...
_detect_system_chrome() {
    ...
    local chrome
    chrome="$("$ARTEMIS_VENV/bin/python" -c '
import sys
try:
...
if [ "$_WANT_PW_CHROMIUM" != 1 ]; then
    ...
    info "  $ARTEMIS_VENV/bin/python -m playwright install chromium"
elif "$ARTEMIS_VENV/bin/python" -c "import playwright" 2>/dev/null; then
    ...
    warn "playwright is not in the virtualenv, skipping Chromium; you can later run: uv pip install playwright --python $ARTEMIS_VENV/bin/python"
fi

# ── 步骤 4: 创建包装脚本 ─────────────────────────────────────────────────────
mkdir -p "$ARTEMIS_BIN"

cat > "$ARTEMIS_BIN/artemis" << 'WRAPPER'
#!/usr/bin/env bash
# Artemis CLI 包装脚本 — 委托给 uv 管理的环境。
set -euo pipefail

ARTEMIS_HOME="${ARTEMIS_HOME:-$HOME/.artemis}"
REAL_BIN="$ARTEMIS_HOME/venv/bin/artemis"

if [ ! -x "$REAL_BIN" ]; then
    echo "Error: Artemis environment not found in $ARTEMIS_HOME/venv" >&2
...
chmod +x "$ARTEMIS_BIN/artemis"
info "Wrapper script created: $ARTEMIS_BIN/artemis"

# ── 步骤 5: 让 artemis 立即可用（无需 source）────────────────────────────────
...
_try_symlink() {
    # 在 target_dir 创建指向包装脚本的 artemis 符号链接。成功返回 0。
    local target_dir="$1"
    local link_path="$target_dir/artemis"
    local src="$ARTEMIS_BIN/artemis"
...
_link_into_existing_path() {
    ...
    if _try_symlink "$HOME/.local/bin"; then
        LINKED_PATH="$HOME/.local/bin/artemis"
...
if _link_into_existing_path; then
    IMMEDIATE_OK=true
    info "Linked to ${LINKED_PATH} (artemis works in the current shell)"
elif [ -n "$LINKED_PATH" ]; then
    info "Linked to $LINKED_PATH"
fi

# ── 步骤 6: 更新 shell / 系统 profile（新开终端持久生效）─────────────────────
PATH_ENTRY="export PATH=\"${ARTEMIS_BIN}:\$PATH\""

add_to_profile() {
    local profile="$1"
    local create="$2"
    if [ -f "$profile" ] && grep -qF "$ARTEMIS_BIN" "$profile" 2>/dev/null; then
        return 0
    fi
    # 兼容旧版标记（仅含 .artemis/bin 字样）
    if [ -f "$profile" ] && grep -qF '.artemis/bin' "$profile" 2>/dev/null; then
        return 0
    fi
...
_write_profile_d() {
    ...
    local dest="/etc/profile.d/artemis.sh"
    local content="# Artemis CLI
export PATH=\"${ARTEMIS_BIN}:\$PATH\"
"
...
if [ -f "$ARTEMIS_VENV/bin/python" ] && [ -d "$ARTEMIS_HOME" ]; then
  CONFIG_PATH="$ARTEMIS_HOME/config.json"
  if [ ! -f "$CONFIG_PATH" ]; then
    echo "{}" > "$CONFIG_PATH"
  fi
  "$ARTEMIS_VENV/bin/python" -m artemis.infra.mobile "$CONFIG_PATH" || true
fi

# ── 完成 ──────────────────────────────────────────────────────────────────────
echo ""
printf "${GREEN}${BOLD}Artemis installed successfully!${RESET}\n"
echo ""
printf "  Location:          ${BOLD}%s${RESET}\n" "$ARTEMIS_HOME"
printf "  Python:            ${BOLD}%s${RESET}\n" "$("$ARTEMIS_VENV/bin/python" --version 2>&1)"
...
if [ "$IMMEDIATE_OK" = true ]; then
    info "artemis is ready to use in the current shell (no source needed)"
elif [ -n "${BASH_SOURCE[0]:-}" ] && [ "${BASH_SOURCE[0]}" != "$0" ]; then
    export PATH="$ARTEMIS_BIN:$PATH"
    info "PATH updated; artemis is available now"
else
...
echo "Then run:"
echo ""
printf "  ${BOLD}artemis run${RESET}       # Run in the foreground (API + Web console)\n"
printf "  ${BOLD}artemis service start${RESET}  # Install and run in the background (systemd / launchd)\n"
printf "  ${BOLD}open http://127.0.0.1:8088${RESET}\n"
echo ""
printf "Upgrade: re-run this installer. Cleanup: ${BOLD}artemis clean${RESET}\n"
printf "Playwright Chromium (optional): ${BOLD}$ARTEMIS_VENV/bin/python -m playwright install chromium${RESET}\n"
