...
# 避免 find_python312 / fix_ownership_and_perms / free_artemis_ports 重复维护。
#
set -u

# ---------------------------------------------------------------------------
# 释放 Artemis 端口（8088=Docker 版，8089=本地版）并清理本应用残留进程。
# 仅清理：(1) 占用 Artemis 端口的进程；(2) 本安装目录（TRIM_APPDEST）下的
# artemis 服务进程。不使用宽泛的 `pgrep -f artemis`，避免误杀其它用户/其它
# 安装路径下的同名进程。
# ---------------------------------------------------------------------------
free_artemis_ports() {
    ...
    for port in 8088 8089; do
        ...
        for pid in $pids; do
            ...
            if kill -TERM "$pid" 2>/dev/null; then
                echo "[artemis] 已发送 TERM 给占用 ${port} 的进程 ${pid}" > "${TRIM_TEMP_LOGFILE:-/dev/null}" 2>/dev/null || true
            fi
        done
        sleep 1
        for pid in $pids; do
            ...
            if kill -0 "$pid" 2>/dev/null; then
                ...
                echo "[artemis] 已强制 KILL 占用 ${port} 的进程 ${pid}" > "${TRIM_TEMP_LOGFILE:-/dev/null}" 2>/dev/null || true
            fi
        done
    done

    # 兜底：只按本安装目录精确匹配，防止误杀其它实例。
    appdir="${TRIM_APPDEST:-/var/apps/artemis-native}"
    for pat in "$appdir/bin/artemis" "$appdir/app/bin/artemis"; do
        pids="$(pgrep -f -- "$pat" 2>/dev/null | tr '\n' ' ')" || true
        [ -z "$pids" ] && continue
        echo "[artemis] 发现本应用残留服务进程（$pat）: $pids，准备清理" > "${TRIM_TEMP_LOGFILE:-/dev/null}" 2>&1 || true
        for pid in $pids; do
        ...
        for pid in $pids; do
            if kill -0 "$pid" 2>/dev/null; then
                kill -KILL "$pid" 2>/dev/null || true
                echo "[artemis] 已强制 KILL 残留服务进程 ${pid}" > "${TRIM_TEMP_LOGFILE:-/dev/null}" 2>&1 || true
            fi
        done
    done
}
...
# 服务（artemis-native）启动时 `. "$PKGVAR/.env"` 会 Permission denied。
# 若目录曾带 ACL，单纯 chmod 会把 mask 压成 ---，需 setfacl -b 清除。
# ---------------------------------------------------------------------------
fix_ownership_and_perms() {
    local pkgvar="$1" envfile="$2"
    local artemis_user="artemis-native"
    id "$artemis_user" >/dev/null 2>&1 || {
        echo "[artemis] 警告：${artemis_user} 账户不存在，跳过数据目录 chown（服务将回退以 root 运行）" > "${TRIM_TEMP_LOGFILE:-/dev/null}" 2>&1 || true
        return 0
    }

    # 1) 应用数据目录与 .env 改属主为运行用户
    chown -R "$artemis_user:$artemis_user" "$pkgvar" 2>/dev/null || true
    ...
    if [ -n "${TRIM_DATA_SHARE_PATHS:-}" ]; then
        ...
        chown -R "$artemis_user:$artemis_user" "$ds" 2>/dev/null || true
        local share_root="$ds"
        while [ "$share_root" != "/" ] && [ "$(basename "$(dirname "$share_root")")" != "@appshare" ]; do
            share_root="$(dirname "$share_root")"
        done
        chown "$artemis_user:$artemis_user" "$share_root" 2>/dev/null || true
    ...
    echo "[artemis] 已修正数据目录/.env 属主与权限（${artemis_user}:${artemis_user}）" > "${TRIM_TEMP_LOGFILE:-/dev/null}" 2>&1 || true
}
...
# 管理员密码：生成 / 校验 / 凭据回落保存。
#
# 背景（issue #502）：src/artemis/infra/users/password.py 的弱密码黑名单包含
# "artemis123"，而 FPK 旧版把初始密码写死为 Artemis123，导致 artemis init 在首次
...
artemis_validate_password() {
    ...
    if [ -z "$pw" ]; then
        ...
        -e 'welcome1' -e 'letmein1' -e 'changeme1' -e 'artemis123' \
...
- 也可以随时在飞牛应用中心 → Artemis → 「设置」窗口中直接查看当前密码。
...
artemis_render_config_wizard() {
    ...
    content="${content//<artemis-current-username>/${username}}"
    content="${content//<artemis-current-password>/${password}}"
    tmp="${target}.tmp.$$"
