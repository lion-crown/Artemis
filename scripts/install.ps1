# Artemis Installer for Windows (PowerShell)
# Usage: irm <url>/install.ps1 | iex
#    or: .\install.ps1 [-Version X.Y.Z] [-FromSource] [-SourceDir DIR] [-Extras "browser"]
#
# Installs Artemis into ~/.artemis with a uv-managed Python environment.
...
$ArtemisHome     = if ($env:ARTEMIS_HOME) { $env:ARTEMIS_HOME } else { Join-Path $HOME ".artemis" }
$ArtemisVenv     = Join-Path $ArtemisHome "venv"
$ArtemisBin      = Join-Path $ArtemisHome "bin"
$PythonVersion = "3.12"
$ArtemisRepo     = if ($env:ARTEMIS_REPO) { $env:ARTEMIS_REPO } else { "https://github.com/TencentCloud/Artemis.git" }

function Write-Info { param([string]$Message) Write-Host "[Artemis] " -ForegroundColor Green  -NoNewline; Write-Host $Message }
function Write-Warn { param([string]$Message) Write-Host "[Artemis] " -ForegroundColor Yellow -NoNewline; Write-Host $Message }
function Stop-WithError { param([string]$Message) Write-Host "[Artemis] ERROR: $Message" -ForegroundColor Red; exit 1 }
...
Environment:
  ARTEMIS_HOME            Installation directory (default: ~/.artemis)
  ARTEMIS_REPO            Git clone URL for -FromSource without -SourceDir
"@
    exit 0
}

Write-Host "[Artemis] Installing Artemis into $ArtemisHome" -ForegroundColor Green
...
# TEMP: mcp 2.x breaks langchain-mcp-adapters; pin until the published Artemis distribution pulls harness-agent>=0.9.18.
...
function Test-Install {
    ...
    & $VenvPython -c "from artemis.infra.agents.manager import AgentManager" 2>$null
...
function Prepare-Console {
    ...
    $consoleDest = Join-Path $RepoDir "src\artemis\dashboard"
...
$VenvArtemis = Join-Path $ArtemisVenv "Scripts\artemis.exe"
...
if ($FromSource) {
    if ($SourceDir) {
        ...
        $cloneDir = Join-Path $env:TEMP "artemis-install-$(Get-Random)"
        try {
            git clone --depth 1 $ArtemisRepo $cloneDir
    ...
    # (artemis[browser]==x.y.z); artemis==x.y.z[browser] is an invalid requirement.
    $package = if ($Version) { "artemis${ExtrasSuffix}==$Version" } else { "artemis${ExtrasSuffix}" }
...
if (-not (Test-Path $VenvArtemis)) { Stop-WithError "Installation failed: artemis CLI not found in venv" }
Write-Info "Artemis installed successfully"

if (-not $script:ConsoleAvailable) {
    $check = & $VenvPython -c "import importlib.resources, artemis; p=importlib.resources.files('artemis')/'dashboard'/'index.html'; print('yes' if p.is_file() else 'no')" 2>&1
...
New-Item -ItemType Directory -Path $ArtemisBin -Force | Out-Null

$wrapperPs1 = Join-Path $ArtemisBin "artemis.ps1"
@'
$ErrorActionPreference = "Stop"
$ArtemisHome = if ($env:ARTEMIS_HOME) { $env:ARTEMIS_HOME } else { Join-Path $HOME ".artemis" }
$RealBin = Join-Path $ArtemisHome "venv\Scripts\artemis.exe"
if (-not (Test-Path $RealBin)) {
    Write-Error "Artemis environment not found at $ArtemisHome\venv"
    exit 1
}
& $RealBin @args
'@ | Set-Content -Path $wrapperPs1 -Encoding UTF8

$cmdWrapper = Join-Path $ArtemisBin "artemis.cmd"
@'
@echo off
set "ARTEMIS_HOME=%ARTEMIS_HOME%"
if "%ARTEMIS_HOME%"=="" set "ARTEMIS_HOME=%USERPROFILE%\.artemis"
set "REAL_BIN=%ARTEMIS_HOME%\venv\Scripts\artemis.exe"
if not exist "%REAL_BIN%" (
    echo Error: Artemis environment not found at %ARTEMIS_HOME%\venv >&2
    exit /b 1
)
"%REAL_BIN%" %*
'@ | Set-Content -Path $cmdWrapper -Encoding UTF8

Write-Info "Wrappers created in $ArtemisBin"

$targetPath = $ArtemisBin
...
Write-Host ""
Write-Host "Artemis installed successfully!" -ForegroundColor Green
Write-Host ""
Write-Host "  Install location:  $ArtemisHome"
...
Write-Host ""
Write-Host "If 'artemis' is not found in this session, run:"
...
Write-Host ""
Write-Host "Then run:"
Write-Host "  artemis init" -ForegroundColor White
Write-Host "  artemis run" -ForegroundColor White
Write-Host "  artemis service start" -ForegroundColor White
