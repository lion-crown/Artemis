...
GITHUB_REPO = "TencentCloud/Artemis"
...
def desktop_name(os_name: str, arch: str, version: str, ext: str) -> str:
    return f"Artemis-desktop-{os_name}-{arch}-{version}.{ext}"


def portable_name(os_name: str, arch: str, version: str) -> str:
    return f"Artemis-portable-{os_name}-{arch}-{version}.zip"
...
def render_download_section(raw_version: str) -> str:
    ...
    fnos_docker = md_link("Docker", tag, f"Artemis-fnos-docker-{version}.fpk")
    fnos_native = md_link("Native", tag, f"Artemis-fnos-native-{version}.fpk")
    return "\n".join(
        [
            "",
            "## Downloads",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Print Artemis GitHub Release download markdown.")
