...
将 Artemis 安装到隔离虚拟环境 `~/.artemis/venv`，并创建 `~/.artemis/bin/artemis` 包装脚本加入 PATH。

### 远程一键安装（推荐）

| 平台 | 命令 |
|------|------|
| macOS / Linux | `curl -fsSL https://finnie-1258344699.cos.ap-guangzhou.myqcloud.com/artemis/install.sh \| bash` |
| Windows (PowerShell) | `irm https://finnie-1258344699.cos.ap-guangzhou.myqcloud.com/artemis/install.ps1 \| iex` |
| Windows (cmd) | 下载 `…/artemis/install.bat` 后运行，或使用下方本地脚本 |
...
| 变量 | 说明 |
|------|------|
| `ARTEMIS_HOME` | 安装根目录，默认 `~/.artemis` |
| `ARTEMIS_REPO` | `--from-source` 无本地目录时的 git 克隆地址 |
| `ARTEMIS_PYPI_MIRROR` | PyPI 镜像（与 `--mirror` 等效） |
...
```bash
artemis init    # 初始化数据库与管理员
artemis run     # 启动服务 → http://127.0.0.1:8088
```
...
先构建前端（产物写入 `src/artemis/dashboard/`，与 `dashboard/vite.config.ts` 的 `outDir` 一致），再打包 wheel。
