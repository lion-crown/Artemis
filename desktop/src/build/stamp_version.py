#!/usr/bin/env python3
"""Stamp the Artemis version into Wails desktop metadata copies."""
...
_MANIFEST_IDENTITY = re.compile(
    r'(<assemblyIdentity\b[^>]*\bname="com\.tencent\.artemis"[^>]*\bversion=")[^"]+(")',
    re.IGNORECASE,
)
...
def stamp_manifest(src: Path, dest: Path, version: str) -> None:
    ...
    if n != 1:
        raise SystemExit(f"artemis assemblyIdentity not found in {dest}")
