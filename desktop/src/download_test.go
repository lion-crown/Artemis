...
func TestEnsurePortableUsesEmbeddedPackage(t *testing.T) {
	home := t.TempDir()
	t.Setenv("ARTEMIS_HOME", home)
	t.Setenv("ARTEMIS_DESKTOP_PORTABLE_ZIP", "")
...
func TestEnsurePortableUsesBundledPackage(t *testing.T) {
	...
	zipPath := filepath.Join(t.TempDir(), "Artemis-"+greenPlat()+".zip")
	t.Setenv("ARTEMIS_DESKTOP_PORTABLE_ZIP", zipPath)
	writeTestGreenZip(t, zipPath, "1.0.0")
...
func TestEnsurePortableReplacesOlderRuntime(t *testing.T) {
	...
	t.Setenv("ARTEMIS_DESKTOP_PORTABLE_ZIP", newZip)
	var statuses []string
...
func TestEnsurePortableUpgradesBundledVersionAfterDatabaseBackup(t *testing.T) {
	...
	database := filepath.Join(home, "artemis.db")
	...
	backups, err := filepath.Glob(filepath.Join(home, "backups", "artemis-desktop-pre-upgrade-*.db"))
...
func TestEnsurePortableKeepsRuntimeWhenDatabaseBackupFails(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(home, "artemis.db"), []byte("database"), 0o600); err != nil {
		t.Fatal(err)
	}
	newZip := filepath.Join(t.TempDir(), "new.zip")
	writeTestGreenZip(t, newZip, "0.9.32")
...
func TestEnsurePortableKeepsNewerExistingRuntime(t *testing.T) {
	...
	t.Setenv("ARTEMIS_DESKTOP_PORTABLE_ZIP", oldZip)
	if err := ensurePortable(LocaleZH, func(string) {}); err != nil {
		t.Fatal(err)
	}
...
func TestEnsurePortableKeepsCurrentRuntimeWhenReplacementIsInvalid(t *testing.T) {
	...
	t.Setenv("ARTEMIS_DESKTOP_PORTABLE_ZIP", invalidZip)
	var statuses []string
	if err := ensurePortable(LocaleZH, func(status string) { statuses = append(statuses, status) }); err != nil {
...
func TestBundledPortableZipRequiresMatchingPackage(t *testing.T) {
	t.Setenv("ARTEMIS_DESKTOP_PORTABLE_ZIP", filepath.Join(t.TempDir(), "missing.zip"))
...
func TestFormatHealthWaitErrorUsesEnglishWhenLocaleIsEn(t *testing.T) {
	...
	for _, needle := range []string{
		"Artemis did not become ready within",
		"1 minute",
		"http://127.0.0.1:8088",
		"make sure Artemis is running",
	} {
		if !strings.Contains(msg, needle) {
...
func TestWaitHealthTimesOutWithFriendlyMessage(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "Artemis did not become ready within") {
...
func writeTestGreenZip(t *testing.T, path, version string) {
	...
	w := zip.NewWriter(f)
	files := []string{
		"Artemis-test/launch.py",
		"Artemis-test/VERSION.txt",
		"Artemis-test/packages/artemis-" + version + ".dist-info/METADATA",
	}
	if runtime.GOOS == "windows" {
		files = append(files, "Artemis-test/runtime/python.exe")
	} else {
		files = append(files,
			"Artemis-test/runtime/bin/python3",
			"Artemis-test/runtime/bin/python3.12",
		)
	}
...
func writeMetadataOnlyZip(t *testing.T, path, version string) {
	...
	w := zip.NewWriter(f)
	entry, err := w.Create("Artemis-test/packages/artemis-" + version + ".dist-info/METADATA")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := entry.Write([]byte("Name: artemis\nVersion: " + version + "\n")); err != nil {
		t.Fatal(err)
	}
	if err := w.Close(); err != nil {
		t.Fatal(err)
...
func writeVersionOnlyZip(t *testing.T, path, version string) {
	...
	w := zip.NewWriter(f)
	entry, err := w.Create("Artemis-test/VERSION.txt")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := entry.Write([]byte("artemis_version=" + version + "\n")); err != nil {
		t.Fatal(err)
	}
	if err := w.Close(); err != nil {
		t.Fatal(err)
