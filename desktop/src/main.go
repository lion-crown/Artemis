...
func (a *App) applyDashboardPrefs(s Settings) {
	...
	js := fmt.Sprintf(
		`(function(){try{localStorage.setItem('artemis:ui-locale',%s);}catch(e){}})();`,
...
func (a *App) boot() {
	...
	if url := os.Getenv("ARTEMIS_DESKTOP_URL"); url != "" {
	...
	a.mu.Lock()
	stopArtemis(a.cmd)
	cmd, err := startArtemis(root, s.Port)
	a.cmd = cmd
	a.mu.Unlock()
	if err != nil {
		a.setStatus(err.Error())
