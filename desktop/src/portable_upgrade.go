...
func desktopSQLitePath(home string) (string, bool, error) {
	...
	if os.Getenv("ARTEMIS_DATABASE_URL") != "" {
		driver = "postgresql"
	}
	if value := strings.TrimSpace(os.Getenv("ARTEMIS_DATABASE_DRIVER")); value != "" {
		driver = strings.ToLower(value)
	}
	if value := os.Getenv("ARTEMIS_DATABASE_SQLITE_PATH"); value != "" {
		sqlitePath = value
	}
...
func backupSQLiteBeforeUpgrade(root, currentVersion, nextVersion string) (string, error) {
	...
	name := fmt.Sprintf(
		"artemis-desktop-pre-upgrade-%s-to-%s-%s.db",
