...
const desktopDragRegionClass = "artemis-desktop-drag"

func dragOverlayJS() string {
	return `(function(){
		if (!document.body || !window._wails || typeof window._wails.invoke !== 'function') return;
		if (document.documentElement.dataset.artemisDragReady === '1') return;
		document.documentElement.dataset.artemisDragReady = '1';
		var armed = false, startX = 0, startY = 0;
		var noDrag = 'button, a, input, textarea, select, [role="button"], [role="menuitem"], [data-artemis-no-drag], .artemis-desktop-no-drag';
		function targetEl(t) {
