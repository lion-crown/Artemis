"""Green portable entry: wire packages/ onto sys.path then run Artemis.
...
def _bootstrap() -> Path:
    ...
    # resolve ``python3`` to the same interpreter Artemis runs with.
...
def main() -> None:
    _bootstrap()
    # The retained package module keeps portable archives compatible.
    sys.argv = [sys.argv[0], *sys.argv[1:]]
    runpy.run_module("artemis", run_name="__main__", alter_sys=True)
