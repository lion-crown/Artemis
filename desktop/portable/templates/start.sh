#!/usr/bin/env bash
# Artemis green portable launcher (macOS / Linux).
# Usage:
#   ./start.sh
...
export ARTEMIS_HOME="${ARTEMIS_HOME:-${ROOT}/data}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<EOF
Artemis green portable launcher

Usage: ./start.sh [--home DIR] [--host HOST] [--port PORT] [artemis run args...]

Defaults:
  ARTEMIS_HOME / --home   ${ROOT}/data
  --host                127.0.0.1
  --port                8088

Environment:
  ARTEMIS_HOME            User data directory (overridden by --home)
EOF
      exit 0
      ;;
    *)
