...
# Public GitHub Release names: Artemis-<kind>-<os>-<arch>-<version>.<ext>
# Zip payload directory is Artemis-<plat>/; the desktop unzip strips the first
# path component generically. Internal wheels keep their package metadata.
artemis_version() {
...
portable_zip_basename() {
  echo "Artemis-portable-${1}-$(artemis_version).zip"
}
...
desktop_pkg_basename() {
  ...
  ver="$(artemis_version)"
  case "$plat" in
    darwin-*) echo "Artemis-desktop-${plat}-${ver}.dmg" ;;
    windows-*) echo "Artemis-desktop-${plat}-${ver}.exe" ;;
    linux-*) echo "Artemis-desktop-${plat}-${ver}.tar.gz" ;;
    *)
...
write_green_overrides() {
  ...
  case "$plat" in
    windows-arm64)
      {
        echo "psycopg-binary ; sys_platform == 'artemis-unsupported'"
        echo "sqlite-vec ; sys_platform == 'artemis-unsupported'"
