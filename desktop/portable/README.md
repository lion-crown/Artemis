...
解压即用：内置便携 CPython + Artemis 及依赖，通过 `start.sh` / `start.bat` 启动。  
...
- [Artemis Desktop Package](../../.github/workflows/artemis-desktop.yml)（多平台 CI）
...
公开文件名：`Artemis-portable-<plat>-<version>.zip`。为兼容既有桌面更新器，zip 内目录仍是：

```
Artemis-<plat>/
  runtime/       # python-build-standalone
  packages/      # Artemis + 依赖（site-packages，可搬迁）
...
ARTEMIS_GREEN_OFFLINE=1 bash desktop/portable/package.sh
```
...
[Artemis Desktop Package](../../.github/workflows/artemis-desktop.yml) 在 6 个 runner 上出 zip：
...
壳只消费 `Artemis-portable-<plat>-<version>.zip`（zip 内为 `Artemis-<plat>/`），不要把绿包编进 asar。步骤见
