...
#   ARTEMIS_GREEN_OFFLINE=1 bash desktop/portable/package.sh   # require local wheels
#
# Layout of each zip:
#   Artemis-<plat>/
...
#     README.txt
#
# Public filename is Artemis-portable-<plat>-<version>.zip (see portable_zip_basename).
set -euo pipefail
...
build_artemis_wheel() {
  local wheel_dir="$1"
  mkdir -p "$wheel_dir"
  # Drop previous local internal wheels so we pick the fresh build.
  find "$wheel_dir" -maxdepth 1 -type f -name 'artemis-*.whl' -delete 2>/dev/null || true
  if [[ ! -f "${REPO_ROOT}/src/artemis/dashboard/index.html" ]]; then
    echo "[package] WARNING: dashboard not built (src/artemis/dashboard/index.html missing)." >&2
    echo "  Run: make build-frontend" >&2
  fi
  echo "[package] building Artemis wheel → ${wheel_dir}" >&2
  uv build --wheel --out-dir "$wheel_dir" "$REPO_ROOT" >&2
  local whl
  whl="$(ls -1 "${wheel_dir}"/artemis-*.whl 2>/dev/null | sort | tail -1 || true)"
  if [[ -z "$whl" ]]; then
    echo "[package] failed to build Artemis wheel" >&2
    exit 1
  fi
...
wheel_cache_usable() {
  ...
  # Need the internal wheel + a reasonable set of dependencies.
  [[ "${count:-0}" -ge 10 ]]
}
...
assemble_one() {
  ...
  local staging="${GREEN_RELEASE}/Artemis-${plat}"
  ...
  local artemis_whl
  artemis_whl="$(build_artemis_wheel "$wheel_dir")"
  ...
  # often Xcode 3.9, which cannot satisfy Artemis's requires-python >=3.12.
  if [[ "$plat" == "$host" ]]; then
  ...
  if [[ "${ARTEMIS_GREEN_OFFLINE:-0}" == "1" ]]; then
    if ! wheel_cache_usable "$wheel_dir"; then
      echo "[package] ARTEMIS_GREEN_OFFLINE=1 but wheels cache incomplete: ${wheel_dir}" >&2
  ...
  # Install locked dependencies first, then the local wheel without re-resolving deps.
  ...
  if [[ ${#offline_args[@]} -gt 0 ]]; then
    ...
    status=$?
    if [[ $status -eq 0 ]]; then
      uv pip install "${common_args[@]}" "${offline_args[@]}" --no-deps "$artemis_whl"
      status=$?
    fi
  else
    ...
    status=$?
    if [[ $status -eq 0 ]]; then
      uv pip install "${common_args[@]}" --no-deps "$artemis_whl"
      status=$?
    fi
  fi
...

  {
    ...
    sed -n 's/^version[[:space:]]*=[[:space:]]*"\([^"]*\)".*/artemis_version=\1/p' \
  ...
  (
    ...
    if command -v zip >/dev/null 2>&1; then
      ...
      "$py" -c "import shutil; shutil.make_archive('${zip_stem}', 'zip', '.', 'Artemis-${plat}')"
    fi
  )
