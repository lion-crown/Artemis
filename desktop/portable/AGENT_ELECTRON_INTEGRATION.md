# Artemis Electron green-package integration

This document is the contract for embedding Artemis's **green portable zip**
...
```
desktop/portable/release/Artemis-portable-<plat>-<version>.zip
```
...
4. Set `ARTEMIS_HOME` to Artemis's default data dir (`~/.artemis`, or `$ARTEMIS_HOME`
   if already set). Do **not** use the zip's `./data` folder when launching
   from the Wails desktop shell (`desktop/src`).
   Extract the zip under `~/.artemis/portable/` so runtime files stay next to
   user data without overwriting `artemis.db`.
...
8. First run uses the **normal Artemis setup wizard** (create admin). The
