...
| `INIT_VERSION_FILE` | `src/artemis/__init__.py` | 含运行时常量 `__version__` 的文件（缺失则跳过并提示） |
...
2. `README_FILE`（`README.md`）— shields.io 版本徽标：
   ...
   ```
   文件不存在则跳过并提示（不中止）。
3. `INIT_VERSION_FILE`（`src/artemis/__init__.py`）— 运行时常量 `__version__`：
   ```bash
   grep -n '__version__' src/artemis/__init__.py
