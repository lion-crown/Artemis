...
DASHBOARD_DEST := $(REPO_ROOT)/src/artemis/dashboard
...
help:
	...
	@echo "  dev-backend      Start artemis run only"
	@echo ""
	@echo "Online-deps targets (local Artemis source + PyPI harness components):"
	@echo "  install-online   Create .venv-online: Artemis editable + harness-* from PyPI"
	@echo "  test-online      pytest against .venv-online (not live)"
	@echo "  run-online       Start artemis run from .venv-online"
	@echo ""
	...
	@echo "  typecheck        mypy --strict src/artemis"
...
# ─── Online-deps dev (local Artemis source + PyPI harness components) ─────────
# Keeps a dedicated .venv-online alongside .venv. Artemis itself is installed
...
install-online:
	@echo "[install-online] Creating $(VENV_ONLINE) (local Artemis + PyPI harness deps)..."
	uv venv $(VENV_ONLINE) --python 3.12
	uv pip install --no-sources --python $(PY_ONLINE)/python -e ".[dev]"
	@echo "[install-online] Done. Artemis=editable(local), harness-*=PyPI."
