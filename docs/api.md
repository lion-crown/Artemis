...
> `enable_api_docs=true` (or `ARTEMIS_ENABLE_API_DOCS=1`). The schema
...
Tokens expire after `ARTEMIS_ACCESS_TOKEN_TTL` seconds (default 24 h).
Rotating the JWT secret (`artemis admin rotate-jwt-secret`) invalidates
every outstanding token immediately. Login attempts are rate-limited
(`ARTEMIS_LOGIN_MAX_ATTEMPTS` / `ARTEMIS_LOGIN_LOCKOUT_SECONDS`); the
...
| `GET`    | `/agents/{aid}/cron/examples` | agent access | `{task_examples}` from workspace `.artemis/manifest.json` (`{zh,en}` string arrays, display-normalized to 3 or 6); `null` if the field is absent (dashboard keeps default cards). Prefer `GET /agents/{aid}/chat/welcome` which includes the same field. |
...
Persona content lives in `src/artemis/infra/agents/mbti_profiles.py` —
...
Bundled experts live in `src/artemis/infra/agents/experts/library/`
...
Custom MCP OAuth (streamable HTTP, public HTTPS URL only): Artemis discovers the authorization
...
| `POST` | `/browser/shutdown` | user | stop the current user's Artemis-managed Chrome |
...
## Ollama, i18n, plugins, slash, preferences

| Method | Path | Auth | Notes |
|--------|------|------|-------|
| `GET`/`POST` | `/admin/service/status`, `/admin/service/restart` | user / TLS admin | service runtime capabilities and managed restart |
...
`code` matches the `ErrorCode` enum in `artemis.infra.errors`; the
server-localized `message` is rendered by `ArtemisError.to_envelope`
