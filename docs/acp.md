...
Artemis integrates [ACP](https://agentclientprotocol.com/) in two directions:

| Direction | ACP server | ACP client | Typical use |
|-----------|------------|------------|-------------|
| **Inbound** | Artemis (`artemis acp`) | Zed, OpenCode, … | External IDE drives your Artemis agent |
| **Outbound** | OpenCode, CodeBuddy, … | Artemis (`acp_runner` tool) | Artemis agent delegates coding tasks |

Both use **stdio** JSON-RPC today. Artemis does not expose an HTTP ACP endpoint.
...
Install the CLI on the host where `artemis run` executes, ensure it is on `PATH` (or set an absolute `command` in the runner drawer). Built-in runners cannot be deleted; custom runners can be added from **Add runner**.
...
After changing global runners, Artemis reloads your agents automatically. Changing only `tool_enabled` reloads that agent.

---

## CLI: inbound — expose Artemis as an ACP server

Let external clients (e.g. Zed) use an Artemis agent as their coding agent:
...
This starts a **standalone** Artemis server (reads `~/.artemis`), boots the agent, and speaks ACP on stdin/stdout. It does **not** require `artemis run` to be running; it is a separate process.

Pin the default agent for convenience:

```bash
artemis user login --username you
# set default_agent in ~/.artemis/cli_state.json, or:
artemis --agent main acp
```
...
```json
{
  "agent_servers": {
    "Artemis": {
      "command": "uv",
      "args": ["run", "artemis", "acp", "--agent", "main"],
      "env": {}
    }
  }
}
...
Create an agent thread in Zed and prompt as usual. Sessions map to `thread_id`; the agent workspace remains under `~/.artemis/agents/<agent_id>/`.
...
- **harness-agent `[acp]` extra** — pulls in `agent-client-protocol` (included in the Artemis wheel dependency on harness-agent).
- **Outbound:** install external CLIs (`opencode`, `codebuddy`, …) on the machine running `artemis run`.
...
| `acp_runner` lists runner but start fails with “Unknown runner” | Restart `artemis run` after adding a custom runner (ACP service cache is process-wide) |
| External runner returns no text | Check CLI auth/quota (e.g. `codebuddy auth status`) |
| `artemis acp` fails immediately | Agent not running or invalid `--agent`; create/start agent in dashboard first |
