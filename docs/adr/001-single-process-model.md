...
Artemis needs to run a web server, a CLI, per-user Agent runtimes, IM channel connections, and cron schedulers simultaneously. The question was whether to split these into separate services (worker + API server) or keep everything in one process.
...
- **Self-hosted target:** Operators install with `pip install artemis && artemis init`. Adding Redis or a process supervisor doubles the ops burden for the primary audience.
...
- **Restart semantics are simple:** `ArtemisServer.start()` rebuilds the entire runtime tree from the SQLite file. No external state to reconcile.
