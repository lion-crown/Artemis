...
> 目标：Artemis 服务端对 agent workspace **内容文件**的读写，一律经 `agent.backend`（或 `resolve_harness_backend`），不再用 `Path(...).write_text` / `read_text` 直连磁盘。  
...
当前部分代码在 agent 已配置远程 backend（S3/COS 等）时，仍向本地 `~/.artemis/agents/<agent_id>/` 读写内容文件，导致：

- 用户配置了 backend，agent 工具读的是远程存储；
- Artemis 服务端 seed / 写 SOUL / 同步插件 skills 等仍写本地；
- Dashboard 部分读路径优先本地，展示与真实存储不一致。

默认未配置 backend 时，harness 使用 `filesystem` + `virtual_mode`，`root_dir` 由 `workspace_dir`（即 `~/.artemis/agents/<id>/`）在创建 backend 时挂载。此时直连磁盘与经 backend 写入**同一物理文件**，但远程 backend 场景下必须统一走 backend。
...
改造前后**操作的是同一个文件**，路径字符串**保持项目既有写法**，Artemis 不做路径改写。
...
- **禁止**在 Artemis 层把 `~/.artemis/agents/<id>/SOUL.md`「转换」为 `/SOUL.md`、`SOUL.md` 等另一套命名。
...
- `~/.artemis/plugins/` 安装、`~/.artemis/security/` tool guard
...
- `init_workspace=False`：避免 harness 再次写本地造成双轨；seed 由 Artemis 经 backend 完成。
...
``{ARTEMIS_HOME}/agents/<agent_id>/``；创建时可覆盖）。**所有 backend 类型**共用这
...
``artifacts_root`` 收到工作区（Artemis：``.artemis/conversation_history/``）。
...
```json
{
  "backend": {
    ...
    "sandbox_scope": "agent",
    "sandbox_prefix": "artemis_sandbox",
    "allow_network": false,
    "memory": "512m"
  }
}
...
| `sandbox_prefix` | 容器名前缀；Artemis 默认注入 `artemis_sandbox`；库默认 `sandbox` |
| `sandbox_id` | `fixed` 必填 |
| `username` | `user` 用；缺省时 Artemis 注入专家所有者用户名 |
| `previewable` | 仅影响 Admin「我的存储」浏览按钮；默认仅 `fixed` 为 true。专家工作区始终可预览 |
| `volumes` | 用户自行配置的挂载，原样透传；**不**自动 named volume |

容器命名：

- `agent` → `{prefix}_agent_{agentId}`（如 `artemis_sandbox_agent_ZE6GR2`）
...
- **同名路径、两处落盘（不自动挂载）**：
  - 专家 ``workspace_dir``（创建时写入配置/库；Artemis 默认 ``{ARTEMIS_HOME}/agents/<id>/``）：宿主上放 sessions / memory / checkpoints
