...
`SOUL.md` (via `artemis.infra.agents.persona.render_soul_template`) at
...
If `persona_mbti` is `NULL` or the empty string, Artemis falls back to
...
| Resource | Location |
|----------|----------|
| 16 `MBTIProfile` records | [`src/artemis/infra/agents/mbti_profiles.py`](../src/artemis/infra/agents/mbti_profiles.py) |
| Public API (`get_profile`, `get_all_profiles`, `MBTIProfile`, `MBTIDimensions`, `MBTIBehaviorMapping`) | same module |
| SOUL.md rendering | [`src/artemis/infra/agents/persona.py`](../src/artemis/infra/agents/persona.py) |
| Persisted agent field | `agents.persona_mbti` (string, e.g. `"INTJ"`) |

The legacy `src/artemis/infra/agents/personas/*.md` directory is gone —
...
The CLI exposes the same path via `artemis agent create --persona-mbti INTJ`.
