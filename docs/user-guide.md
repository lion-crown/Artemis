...
> 所有运行时数据默认存放在 `~/.artemis/`（可通过 `ARTEMIS_HOME` 覆盖）。
...
**Artemis** 是一个开源、自托管的 AI 助手平台，支持多用户、多 Agent。它在单进程中同时提供 Web 控制台、CLI、IM 通道（飞书、钉钉、QQ、Discord、企业微信等）和定时任务，所有数据都保存在你自己的机器上。

![图 1.1 — Artemis 产品总览](assets/overview.png)
...
- **无需** 预先安装 Python —— 安装脚本会通过 [uv](https://docs.astral.sh/uv/) 在 `~/.artemis/` 下自动创建隔离的 Python 3.12 虚拟环境。
- 需要可访问外网，用于下载安装脚本与依赖。
...
安装脚本会把 `artemis` 命令放入 `~/.artemis/bin` 并加入 PATH，并在 `~/.artemis/venv` 创建隔离环境；**不会改动系统 Python**。
...
若提示 `command not found: artemis`，请确认已重新加载 shell 或检查 `~/.artemis/bin` 是否在 PATH 中。
...
docker run -d \
  -p 8088:8088 \
  -v artemis-data:/data/.artemis \
  -e HOME=/data \
  -e ARTEMIS_DEFAULT_PASSWORD="<自定义强密码，留空则自动生成随机密码>" \
  artemis:latest
```
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `ARTEMIS_PORT` | `8088` | HTTP 监听端口 |
| `ARTEMIS_DEFAULT_PASSWORD` | _(空)_ | 首次运行管理员密码（Docker 引导；≥8 位且含字母+数字；留空则自动生成随机密码并写入 credential.txt） |
| `ARTEMIS_ADMIN_USERNAME` | `admin` | 首次运行管理员用户名 |
| `ARTEMIS_DATA` | `~/.artemis` | 宿主机数据目录（compose 挂载） |
...
```bash
artemis run       # 前台启动 API + Web 控制台
```

若希望服务在后台常驻，可注册为系统服务：

```bash
artemis service start   # Linux(systemd) / macOS(launchd) / Windows 服务
```
...
- 密码打印在启动终端，并写入服务器上的引导文件（常见为 `~/artemis-login.txt`）。
- 此步**不依赖**控制面数据库是否已打开。

**步骤 2：选择控制面数据库**

- 默认使用本地 **SQLite**（路径相对 `~/.artemis/`，通常为 `artemis.db`），点击「保存并继续」即可。
- 「展开更多」可配置 **PostgreSQL**（内测）：填写主机等并先「测试连接」，再「使用 PostgreSQL 并继续」。
- 向导会把选择写入 `config.json` 的 `database` 段，并在服务进程内**首次绑定**连接池、跑迁移。
- 也可事先用环境变量指定（见 [configuration.md](configuration.md) 的 `ARTEMIS_DATABASE_*`）；已有库文件的升级安装不会延后建连。
...
```bash
export ARTEMIS_ADMIN_USERNAME=admin
export ARTEMIS_ADMIN_PASSWORD="<你的强密码，≥8 位且含字母和数字>"
artemis run
```
...
Artemis 通过 **供应商（Provider）** 接入大模型。每个 Agent 可使用不同的供应商与模型。支持 OpenAI 兼容 API、Anthropic、AWS Bedrock、DashScope（通义千问）、Ollama 本地模型等。
...
```bash
artemis models              # 查看供应商预设与模型解析
artemis provider list       # 列出已配置供应商
artemis provider --help     # 供应商增删改查帮助
```
...
> ⚠️ **安全提醒**：Docker 首次初始化若未设置 `ARTEMIS_DEFAULT_PASSWORD`，会自动生成随机管理员密码（写入 `/data/.artemis/credential.txt`，可用 `docker exec <容器> cat /data/.artemis/credential.txt` 查看）。无论哪种方式，都请尽快在 **个人设置 → 修改密码** 中更换，避免服务暴露到公网时被未授权访问。
...
Artemis 支持两个方向的 ACP 集成：

1. **入站** —— 让外部工具（Zed、OpenCode 等）使用你的 Artemis Agent：

   ```bash
   artemis acp --agent main
   ```
...
- **用户**：管理账号、角色、修改密码。
- **安全**：工具审批、Shell 命令护栏（`~/.artemis/security/tool_guard/`）。
- **TLS**：配置 HTTPS（自签或 Let's Encrypt）。
- **系统**：监听地址 / 端口、日志级别、定时任务时区等。

> 手动编辑配置文件：运行时参数保存在 `~/.artemis/config.json`，可用环境变量覆盖（如 `ARTEMIS_PORT`、`ARTEMIS_BIND_HOST`）。详见 [docs/configuration.md](docs/configuration.md)。
...
| 命令 | 说明 |
|------|------|
| `artemis run` | 前台启动 Artemis |
| `artemis run --host 0.0.0.0 --port 8088` | 自定义监听地址与端口 |
| `artemis service start` | 安装并启动系统服务 |
| `artemis service stop` | 停止系统服务 |
| `artemis agent` | 创建、列出、启停 Agent |
| `artemis channel` | 安装与管理 IM 通道 |
| `artemis chats` | REPL 与会话管理 |
| `artemis acp` | 为 IDE 提供 stdio ACP 服务 |
| `artemis cron` | 管理定时任务 |
| `artemis models` | 供应商预设与模型解析 |
| `artemis provider list` | 列出已配置供应商 |
| `artemis skills` | 按 Agent 启用 / 禁用 Skill |
| `artemis user list` | 列出用户（管理员） |
| `artemis backup` | 导出 / 恢复备份 |
...
- 确认已执行 `artemis run` 且终端无报错。
- 若改过端口，请访问对应地址（如 `http://127.0.0.1:8088` 或自定义端口）。
- 用 `artemis service status`（Linux / macOS）确认服务状态。
...
**Q：如何修改监听地址让局域网访问？**
- 启动时：`artemis run --host 0.0.0.0 --port 8088`；或设置环境变量 `ARTEMIS_BIND_HOST=0.0.0.0`、`ARTEMIS_PORT=8088`。
...
├── artemis.db                 # 默认 SQLite 控制面 — 用户、Agent、通道、定时任务 …
├── secrets/                 # JWT 密钥、通道 Token
├── agents/<agent_id>/       # 各 Agent 工作区（SOUL.md、skills …）
├── security/tool_guard/     # Shell 命令允许 / 拒绝规则
├── logs/                    # 运行日志
└── bin/artemis              # PATH 包装脚本 → venv/bin/artemis
```

**Q：如何升级？**
- 按你的部署方式安装新的 Artemis 构建，并在完成后手动重启服务。
...
| 编号 | 位置 | 文件名 | 状态 | 内容说明 |
|------|------|--------|------|----------|
| 图 1.1 | 一、简介 | `overview.png` | ✅ 已就位 | Artemis 品牌 Banner |
