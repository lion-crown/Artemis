...
harness-memory 和本次 Artemis 读取修正。原有 inline 格式仍可读取。
PostgreSQL 不进入 SQLite 读取入口，继续经 graph 恢复消息；这不改变下述分段归档的 SQLite-only 限制。

## 启用

适用范围：SQLite 主库、单个 Artemis 服务进程。PostgreSQL 开启此功能会明确拒绝启动。
上线需同时包含本次 Artemis 变更及 harness-agent 流式协议变更；后者提供模型消息 ID 和来源关联。
...
3. 在现有 `config.json` 增加 `"history_v2_enabled": true`，或设置
   `ARTEMIS_HISTORY_V2_ENABLED=true` 后重启。默认值为 false。
4. 新文件位于 Artemis 数据根目录下的 `history_v2.sqlite`，旁边有
