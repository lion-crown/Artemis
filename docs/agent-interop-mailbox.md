...
**状态：** 已实现（harness `teams/` + Artemis `GlobalProcessor` 作为 `TeamProcessor`）
**范围：** `harness-agent`（主实现）+ Artemis（宿主适配）
...
- 属性 `team_enabled: bool`；`set_team_processor(...)` 支持启动后注入（Artemis 装配顺序需要）。
...
- 仅当调用方（Artemis）显式取用时加入 agent 工具；**不写进 HarnessAgent 默认工具**。
...
（仅 Artemis 使用 manager，符合「使用 manager 时才启用」）。
...
5. **Artemis 接线**：`DelegationProcessor`、装配、工具注册、删旧路径；回归 `uv run pytest -m "not live"`。
