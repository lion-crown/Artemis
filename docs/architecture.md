...
Artemis is a self-hosted AI assistant platform for multiple users and agents,
...
`ArtemisServer.start()` wires the runtime tree in dependency order. The
composition root lives in `src/artemis/infra/server.py`; FastAPI/uvicorn
is attached from `src/artemis/launch.py` (the only module that may import
...
ArtemisServer.start()
 ...
 ├─ PluginManager.seed_bundled() then load_installed()  (~/.artemis/plugins/*; bundled default off)
...
The dashboard always talks to Artemis over `/api` HTTP/WebSocket; the
...
| CLI | Embedded `ArtemisServer` (REPL) or WebSocket | `<aid>:cli:<user_id>:dm` | `artemis chats send` / `chats repl` |
...
* SQLite — default file under `~/.artemis/` (legacy `artemis.db` or
  `config.json` → `database.sqlite_path`), or
* PostgreSQL — `config.json` / `ARTEMIS_DATABASE_*` / Setup wizard database step.

Migrations: `src/artemis/infra/db/migrations/NNN_*.sql` (SQLite) and
...
(`artemis.infra.backend` → `harness_agent.backends`). The default
backend is a `filesystem` adapter rooted at
`~/.artemis/agents/<agent_id>/`; remote backends (S3, COS) are mounted
on top of the same root_dir and the Artemis service never reads
...
`src/artemis/i18n/{en,zh}.json` and helpers under
`src/artemis/i18n/domains/`. Locale resolution lives in
...
entry points. Artemis's `GlobalProcessor` implements the
...
- [Configuration](configuration.md) — `~/.artemis/` layout + env vars
