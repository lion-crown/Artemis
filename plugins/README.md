# Artemis Plugin Demos

[中文版](./README_CN.md)

Sample plugins for Artemis / harness-agent `kind` values, plus a **dual
frontend + backend** demo that renders tool results in chat.
Product plugins shipped with the wheel live in
`src/artemis/infra/agents/plugins/bundled/`. `artemis init` and `artemis run`
copy missing ones into `~/.artemis/plugins/` and set them **globally off**
...
| [`demo-ui-card`](./demo-ui-card/) | `tool` + `ui/` | Backend returns `artemis_ui` JSON; frontend renders an interactive card |
...
Ship a self-contained ESM; React is provided as `window.__ARTEMIS_REACT__` /
`__ARTEMIS_JSX__`.
