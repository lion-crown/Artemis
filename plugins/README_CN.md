...
本目录提供插件 demo，对应 Artemis / harness-agent 支持的 `kind`，以及**前后端一体**的 UI 渲染示例。
随 Artemis 安装包一并分发的产品插件在 `src/artemis/infra/agents/plugins/bundled/`。
`artemis init` 与 `artemis run` 启动时会把缺失的插件复制到 `~/.artemis/plugins/`，并在 `config.json` 里写成 **全局关闭**；管理员在 Dashboard 插件页打开后再给 Agent 用。卸载过的 id 不会自动装回。
...
| [`demo-ui-card`](./demo-ui-card/) | `tool` + `ui/` | 后端返回 `artemis_ui` JSON；前端在聊天页渲染可交互卡片 |
...
插件 ESM 为自包含文件；React 由 Dashboard 注入 `window.__ARTEMIS_REACT__` / `__ARTEMIS_JSX__`。

## 本地安装

```bash
# 从目录安装（开发时常用）
artemis plugin install ./plugins/demo-toolkit --force
artemis plugin install ./plugins/demo-greeting-skill --force
artemis plugin install ./plugins/demo-turn-logger --force
artemis plugin install ./plugins/demo-ui-card --force
artemis plugin list
```
...
artemis plugin install ./demo-ui-card.zip --force
```
